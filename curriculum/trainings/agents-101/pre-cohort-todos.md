...
- **Mechanical (tmux-runner) coverage for a101, PARTIAL.** Scenarios now exist for prework through M6 (`a101-m4a` + `a101-m4b` split the two M4 sessions), and the seam/plant/trajectory regression suite passed 2026-08-23. The previously validated live floor remains prework→M2; run the full M3→M6 live chain after the three-persona sim sweep. M7 has no scenario; M8 remains deferred because it needs a synthetic peer room. `check_prompts.md` rule 42 (AUQ-suppression) is mechanically exercised only where the scenario and assertions make it observable. Harness is headless-Code only, not Cowork.
