...
- `judges/writing.md` — writing-class judge prompt template. Reads `memory/check_*.md` compendiums tagged `eval_classes: writing` (writing / sales_copy / prompts / student_facing surfaces). Catches what the auto-fix in `eval-class-router.sh` can't (register, atmospheric phrasing, value-prop leak, over-hedge).
- `judges/story.md` — storytelling-class judge prompt template. Reads compendiums tagged `eval_classes: storytelling` (pedagogy / strategy_tie_in / lectures). Reads **Class A persona-reader** sim trace from `sim-cache/<training>--<surface-type>--<slug>.persona.json`; regenerates per-phase slices if per-phase SHA stale.
- `judges/technical.md` — technical-class judge prompt template. Reads compendiums tagged `eval_classes: technical` (platform / research_claims). Static checks: capability claims, citation evidence ladder, skill availability, URL liveness.
- `judges/prompt-behavior.md` — behavior-class judge prompt template. Per-prompt risk against the 15-pattern catalog at `curriculum/evals/simulation-behavior.md`. Reads **Class B prompt-behavior** sim trace from `sim-cache/<training>--<surface-type>--<slug>.behavior.json`; regenerates per-prompt slices if per-prompt SHA stale. Reasons over the *distribution* of Claude responses for files mechanical doesn't cover.
- `judges/pedagogy.md` — pedagogy-class judge prompt template. Reads compendiums tagged `eval_classes: pedagogy`. Module architecture and dynamics, statically. Cross-module rules are stub redirects here; the judge returns N/A on those numbers.
- `judges/strategy.md` — strategy-class judge prompt template. Reads `check_strategy_tie_in.md` against the training's strategy doc: Big Idea, Key Concepts shape, What-You'll-Learn register, mood contract, strategic-vocab gate.
- `judges/slides.md` — slides-class judge prompt template. Splits the body at `##` (one slide per chunk, matching `site/layouts/slides.js`) and scores every rule against every chunk in isolation. A slide must resolve its own referents cold.
- `judges/cross-module.md` — cross_module judge prompt template. **Module-set scope, not per-file:** ≥2 modules from one training, walked as adjacent pairs. Artefact contracts, session boundaries, between-module reading, bring-or-scramble shape. Records as its own `- cross_module` Quality row; touching any member of the pinned set degrades the row for all of them.
- `judges/voice-panel.md` — voice_panel, and it is a panel rather than a judge: six subagents, five author personas plus Sami, the cautious reader whose flinch vetoes a unanimous author panel. Scores taste (virtues absent) where the writing judge scores compliance (violations present), so it fires AFTER writing passes. AE101 student-facing surfaces; findings are a punch list, never a gate.
- `exercise.md` / `lecture.md` — manifests. Name which classes + scopes run per artefact type. Not megajudge templates.
- `sim-cache/` — content-SHA-keyed JSON traces, the evidence PASS instances cite. This tracked link points into the sibling private `agents-102-core/evals/sim-cache/`, so paired clones retain the traces in Git. One file per slug per class: `<training>--<surface-type>--<slug>.persona.json` (Class A) + `<training>--<surface-type>--<slug>.behavior.json` (Class B). Training and surface type disambiguate same-name files across trainings and directories.
...
- Subagents inherit the applicable `CLAUDE.md` hierarchy, including `.claude/rules/content-rules.md`. Do not prepend that routing page again. Eval dispatchers supply the matching full `memory/check_*.md` compendiums at T3; rules outside the auto-loaded hierarchy, such as `continuous-research/research-rules.md` on research surfaces, still require explicit injection. Surface-specific extras (mood contracts for seam judges, Builder scoping for path-consistency, etc.) travel alongside the class brief. See prior runs in `instances/` for prompt patterns.
