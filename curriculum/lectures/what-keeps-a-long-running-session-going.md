...
**The fourth slide (2026-08-14, Antti-directed) inverts the lecture's frame: three slides answer *what keeps a session going*, this one asks what ends it ahead of done-done — the question a student can act on while their own send-off is in flight. Roster is a maintainer call:** stops-and-waits (Antti's own: the agent's belief that information is out of reach is the stopper, not the reachability, and re-judging it is the student's move on return) · declares-done-on-the-part-it-managed · blocks-on-a-non-returning-command · spends-the-run-on-one-obstacle. *The environment ends it* (sleeping laptop, dropped network) stays out: obvious to a working engineer, covered operationally by `set-the-markers-send-it-off` § *Nudge sparingly while it runs*. Context-window management belongs only on `## Durable state keeps the place`, not on this roster. **The kicker is the slide's one move and comments on bullet 1 (2026-08-14, Antti-worded):** *"When your agent stops for missing information, check whether there was a way for it to uncover that. Usually there was."* Kicker shape, not a `**Note**` widget: a Note box would out-weigh the bullet, and this is a trainer's aside, not a hazard. The check it points at is whether a connector, a file, a command or a question to a colleague would have dissolved the belief. Placement fixed to this slide; anywhere else it reads as generic advice.
...
- costa-backpressure `[checked:2026-07-02 result:OK due:2026-11-23]` https://www.lucasfcosta.com/blog/backpressure-is-all-you-need — [practitioner direct] Lucas F. da Costa, 2026-05-23. Byline, date and the quoted sentence verified verbatim. Due is publication+6mo — an argument is not a §2a completed event, so no durable-account carve-out; dated check owned by `earn-the-trust.md`, this due matches it. fallback: paraphrase as "human review can become the bottleneck stage" and drop the name.
