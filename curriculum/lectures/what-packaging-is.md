...
- `[checked:2026-08-01 result:OK due:2027-02-01]` https://ampcode.com/notes/200k-tokens-is-plenty — [practitioner direct, vendor venue] Counter-evidence the closer should keep honest: *"Agents get drunk if you feed them too many tokens… Long threads are not just worse, they also cost more. Way more."* Amp's bet is short ~80k threads plus explicit handoff, which cuts at the triad's premise — a pattern for surviving long runs is only needed if long runs are the goal. Sustained into the window (Handoff Oct 2025 → *Handoff, Please* Jan 2026 → agent-to-agent Jul 2026), so not a one-off. fallback: the existing two-camp framing already carries this; keep it.
