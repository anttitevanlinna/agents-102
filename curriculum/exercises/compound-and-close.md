...
**The `Esc` → narrow → `continue` recovery callout appears in this file and in `orient-and-introspect.md`, both inside M1, and both stay.** The final clause is verbatim in the two, which is what an arc reader notices; the failures underneath are different, and `check_pedagogy.md §48` requires a recovery move on each. Ex1 guards a repo read that fans out or stops short; this file guards a sweep that runs long. Cutting either loses a required guard, and the only other cure spends fresh phrasing to preserve the same structure. A per-file judge cannot see the pair, so the disposition lives here rather than in a chat window: an arc reader meeting the duplicate reads this note and files nothing.
...
- judges @64d17eea: writing PASS, story PASS, technical PASS, behavior PASS (4 findings see instances/ae101--exercise--compound-and-close.behavior.json), pedagogy PASS, strategy PASS, slides PASS (2 findings see instances/ae101--exercise--compound-and-close.slides.json)
**Meta (trainer):**
...
- **`check_prompts.md` §43 declines on `compound-and-close-1`; its resume tail is discharged in body (2026-09-09).** The fence reviews scrollback already in the window and writes one file, and in-fence progress narration cuts against its deliberate 3–5 line summary shape. §43's tail — a mid-run interrupt yields report-then-continue, not restart — sits one line above the fence in the *Long session, long read* callout, which hands the student the narrowing move (`Esc`, narrow to the orient and introspect phases, `continue`). That callout is §48's reactive half, so the pairing both rules require holds here, with the steer on the student's side rather than inside a prompt copied once. Dose: the body names a couple of minutes as the stall to interrupt, not the expected duration, and a restart is loud and re-steerable in its own beat. This is not §43 switched off on this fence — a fence here going silent through a genuinely long default run is still in scope.
