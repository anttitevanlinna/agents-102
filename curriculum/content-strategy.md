...
Prework (as of 2026-04-20): **Three-walls pre-read** bundling absorption bottleneck (L4), Access-Trust Gap (Pattern 47, L4), announcement-to-deployment gap (Pattern 31, L3) — three walls practitioners hit when they try to scale past themselves. Plus Rumelt on *crux / Good Strategy Bad Strategy*, Roger Martin on *strategy as assumptions / what would have to be true?*, Gary Klein on *pre-mortem* (migrated from M8 — the strategy skills are now M7 material). The 8 practitioner stories originally TODO'd are superseded by the patterns catalog (`strategy/personal-to-team-patterns.md`) which students consult live during the exercise.
