...
1. **Verify capabilities BEFORE asserting, not after.** The trigger is the assertion — any sentence of the form *"Claude Code supports X"* or *"the file Y does Z,"* whether spoken in strategic conversation, written in a plan, drafted in prose, or proposed as a reshape. Invoke the `claude-code-guide` agent with specific questions — feature name, GUI click-path in the desktop app, MCP dependencies, Git assumptions for cloud variants, current-as-of-date — before the assertion goes out. Training-data memory of Claude Code goes stale within months; features land mid-year. Do not assert from memory. **Trust-but-verify the capability-check agent too** — WebFetch the URL yourself when the assertion is load-bearing; agents can hallucinate credible-sounding official answers (caught 2026-04-23 on the `CLAUDE.local.md` check — the agent's summary included a suspicious *"docs updated today"* detail that warranted direct verification before action).
