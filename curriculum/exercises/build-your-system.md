...
Open Cowork on the cohort SharePoint workspace. You will save your own files in your named folder under `participants/`, for example `participants/maija-koskinen/`. The task itself lives in `shared/m1-task.md`, prepared by your organisers before the workshop.
...
```
Read `shared/m1-task.md`. Draft a one-page IT-team response we can send to an internal user.

Ask me for my participant folder under `participants/` if you do not already know it. Then save the draft as `response.md` in that folder.
```
...
```
Read the `response.md` you saved in my participant folder. Propose 3 to 5 rules for a folder CLAUDE.md that would catch the most generic and the most off claims you made. Show me the rules in chat first, one sentence each. Don't save yet.

After I push back, save the final list as `CLAUDE.md` in the same participant folder. Open the file with this line as rule 1, exactly:
...
```
Re-read the `response.md` and `CLAUDE.md` in my participant folder. Now redraft `response.md` from scratch in that same folder, applying the rules in CLAUDE.md to the same user question. Before you save, show me in 3 lines what you're changing and why.
```
...
```
Add ONE rule to the `CLAUDE.md` in my participant folder that would have caught a specific wrong claim from the Phase 1 response.md. The rule should fire on similar future claims, not on everything. After adding the rule, re-read `response.md` in the same folder, regenerate it, and tell me whether the new version still contains the wrong shape. The wrong claim from Phase 1:
```
...
```
Final pass. Re-read `response.md` and `CLAUDE.md` in my participant folder. Iterate the response until three specific things in it are unmistakably about MY team and not generic. After you regenerate, name those three things in chat. I will confirm or push back on each one.
```
...
- draft 2026-04-29 (remote-resilient architecture edit: Phase 1 task now comes from `shared/m1-task.md`; prompt-chain tests not rerun)
