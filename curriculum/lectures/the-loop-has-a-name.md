...
- `[found:2026-08-02]` The Intercom numbers are unrevised. The newest post at the venue is 2026-07-15 and does not touch the topic; the original re-reads consistent, and adds two figures the lecture does not use (497 PRs fully autonomous in the first four weeks, ~60% of all PRs evaluated). One honest gap: Curran's own X account returned 402 and the documented workarounds are degraded, so that is an unopened source rather than a checked-clean one.
