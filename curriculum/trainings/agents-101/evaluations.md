...
**Sources:**
- `[checked:2026-08-23 result:OK due:none]` https://www.oneusefulthing.org/p/the-bitter-lesson-versus-the-garbage — [practitioner direct] Mollick (pub 2025-07-28), framing of bitter-lesson scaling against the organisational "garbage can" problem used in the opener. Outside the 6-month window by maintainer decision — named framing piece cited for its question, not for currency; dated check owned by `run-the-first-experiment.md`. fallback: retain the question and remove the named attribution.
