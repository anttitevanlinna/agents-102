...
A plain prompt file for use during pre-work. The participant pastes this into a fresh Cowork session connected to a safe personal folder, with the O365 calendar connector enabled if the customer's environment supports it. Claude builds the participant a small custom agent that produces a one-page daily plan from their calendar.
...
- One short instruction on what the run produces (a one-page plan in plain text, max 200 words, saved as today-plan.md)

After morning-planner.md is saved, run the agent: read morning-planner.md, pull today's calendar from the O365 connector, produce today-plan.md per the rules in the agent file. Show me the plan in chat after saving.

I'll push back if it doesn't sound like me. After my push-back, sharpen morning-planner.md (not just today-plan.md — the rules file) and run again.
```
...
- The two artifacts (`morning-planner.md`, `today-plan.md`) live in the participant's safe personal folder. They survive past the cohort. The participant can keep running the agent after the training
