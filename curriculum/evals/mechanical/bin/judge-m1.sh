...
# by the prior LLM-Judge run at instances/bootstrap-m1-verbatim-judge-report.md
...
#   judge-m1.sh
#
# Writes report to: instances/bootstrap-m1-verbatim-judge-report.md
# Exit 0 PASS, 1 FAIL.
...
SCRATCH="$MECH_DIR/scratch/bootstrap-m1"
SCROLL="$MECH_DIR/instances/bootstrap-m1-verbatim-actor-scrollback.md"
ACTOR_REPORT="$MECH_DIR/instances/bootstrap-m1-verbatim-actor-report.md"
PRIOR_JUDGE="$MECH_DIR/instances/bootstrap-m1-verbatim-judge-report.md"
REPORT="$MECH_DIR/instances/bootstrap-m1-verbatim-judge-report.md.script"
...
rm_out=$(bash "$SCRIPT_DIR/runner-mapping-check.sh" "$MECH_DIR/runners/bootstrap-m1.verbatim.actor.md" /Users/anttitevanlinna/Projects/agents-102/curriculum/exercises/personal-site-with-guardrails.md 2>&1) || true
...
{
  echo "# Judge report — Bootstrap M1 verbatim (script-only)"
  echo
  echo "## Summary"
  echo
  echo "Verdict: **$verdict_line $pass_count/$total**. Generated by bin/judge-m1.sh — no LLM."
  echo "INHERITED rows reference prior LLM-Judge PASS at instances/bootstrap-m1-verbatim-judge-report.md (run 2)."
  echo
  echo "## Assertions"
  echo
