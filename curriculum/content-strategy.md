...
**The plugin-authoring module — taught once, here.** No pre-shipped plugins anywhere in Bootstrap. M4 is the canonical place plugin-authoring is introduced. Skills arrive as the mechanism — and as **expertise the student authors and plugs in.** Two lenses, one plugin the student writes during the exercise: a company-policy lens (using the customer's actual rules where the engagement provides them, or a Nordic-baseline reference set as the fallback) plus an agent-security lens (access-control analysis + agent-STRIDE with agentic mitigations).

Prework: two reads. (1) Plugins — what they are, how they package an expertise into the agent, why authoring one is the move. (2) Agent security vs. classical software security — why the threat model shifts when the system is non-deterministic and tool-using, and why mitigations are agent-shaped (split / filter / review / gate) rather than firewall-shaped.
...
- Lecture: **"The practice of risk"** — absolute certainty never exists. The practice is the loop: assess → mitigate → reassess residual risk → decide. Access control (what can it reach vs. should reach) and agent-STRIDE (spoof / tamper / repudiate / disclose / DoS / elevate, in agent shapes) are the two lenses. Agentic mitigations (split, filter, review, scope, gate) — not firewalls. And the best mitigation is the one you don't need: don't open the biggest risks in the first place. The plugin you author tells you which doors not to open.
- Exercise: **Audit your agent** — author a security plugin and run it against your Module 3 system. The plugin carries two lenses: a policy lens (compliant / violating / "I can't tell" per rule) and an agent-security lens (access-control analysis + agent-STRIDE with ranked agentic mitigations). Read both reports. Pick ONE risk. Apply the mitigation. Re-run. See the residual risk shift — not vanish. **Caveat:** the exercise body is a surgical pass at present; full reshape into plugin-authoring (per-runtime install paths) is owed in a separate session.
...
- Concept (emergent): Plugins as expertise injection, authored once in M4, not just scoping. Residual risk as a first-class artifact. The best mitigation is not opening the door.
...
- **Module 4 (Security):** module file reframed (mood: epistemic humility, not guilt; big idea: *you can't tell if your agent is safe by looking at its output — the practice is running the check*). **No pre-shipped plugins anywhere in Bootstrap (Antti, 2026-04-25); M4 is the canonical plugin-authoring module — taught once, here.** Surgical wrapping pass 2026-04-25 stripped pre-shipped skill framing; full M4 plugin-authoring exercise reshape (per-runtime install paths: Cowork *Save plugin* button, Desktop plugin loader, CLI `~/.claude/skills/<name>/SKILL.md`) is owed in a separate session. Scaffold `curriculum/scaffolds/module-4-starter/` cleared of pre-built skill folders 2026-04-25; the policy reference content (Nordic-baseline `.md` for self-study, customer-distilled for engagements) is kept as **input material the student reads while authoring**, not as a pre-built plugin. Prework with two sections: plugins + agent-vs-software security. In-room lecture **The practice of risk**. Exercise **Audit your agent** — three phases on the student's Module 3 system, produces `module-4/policy-report.md`, `module-4/security-report.md`, `module-4/residual.md` with a "doors I'd rather not open" scoping rule. Lecture eval APPROVE; exercise eval APPROVE WITH TODOs (length soft-fails; reshape pass also owed). Simulation 8/10 "this is me" — mood lands. Pass-1 patches applied (Phase 3 stop language, re-check granularity). **Blocking before first delivery:** M4 reshape session (plugin-authoring exercise), plus capability check on per-runtime plugin install paths.
...
5. **Module 4 reshape + customer-input checklist** — the M4 exercise body needs reshape into plugin-authoring (per-runtime install paths). Plus the out-of-repo process and input checklist for distilling a customer's policies into reference `.md` material the student reads from while authoring their plugin. Referenced from `scaffolds/module-4-starter/README.md`. Needed to scope the separately-billable prep work and run a first customer engagement.
