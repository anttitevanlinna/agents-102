...
- **Outcome convergence (~~L3~~ **L1–L2, corrected 2026-08-01** — Faros and DX are both vendors reading telemetry off their own paying customers, so the "independently-verifiable" framing does not hold; DORA is the one non-vendor leg):** At the organization level, the independently-verifiable measurements converge near **~10%** (Faros: org DORA flat despite +98% PRs; DORA: adoption correlates *negatively* with throughput/stability; DX: ~5 hrs/week saved ≈ ~12%). A secondary analysis claims ~6 efforts cluster here but does not itemize all six — the ~10% holds on the named components alone. The absorption bottleneck (review +91%, bugs +9%) explains the gap between 2X individual throughput and ~10% org outcome.
...
5. **No org that scaled throughput AND outcome past ~10%** (consistent with absorption-bottleneck.md, now L2; ~~CircleCI: only 1 in 20 teams scaled both~~ **[UNVERIFIED STAT, struck 2026-08-01]**).
