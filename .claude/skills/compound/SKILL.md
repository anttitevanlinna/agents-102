...
**Then rebuild the rule index and repin the ledger.** `node curriculum/evals/scripts/build-rule-index.js && node curriculum/evals/scripts/compendium-drift.js --repin` — the rebuild carries the amendment into the tier the next session actually reads at generation time (T1 leads; T0 if you added the rule number to `metadata.tiers.diamond`). Skip the rebuild and the hooks detect the drift by content hash and fall back to full compendiums: safe, but the amendment reaches the next generator only if that session pays the full-file cost. The repin — the amendment you just landed changes what a judge would say, and every pinned verdict older than the amendment was taken against the previous wording. The repin dates the moved rule so `scan-stale-classes.js` can bill the re-judge as `rule-drift`. Skip it and the amendment is invisible to the eval machinery: the rule fires at generation time and nothing re-checks the corpus already shipped against it.
