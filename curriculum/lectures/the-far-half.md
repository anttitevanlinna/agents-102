...
Format → `curriculum/backing-format.md`. Figure provenance: derived from `the-whole-map.md`'s canvas — if that parent changes, re-derive. Sibling copies: `the-loop-half-filled.md`, `composing-the-workflow.md` (M6, since 2026-09-02).
...
- decided: **the per-run framing stays, and it is the personal-tier reading on purpose, 2026-08-02.** Osmani's sampling loop means some runs are never read by a person, but at a student's scale every run is, so nothing here is wrong for its audience. The scale shift landed at M6 until 2026-09-02, when the Absorption slide carrying the climb was cut; this file is now the only home of the climb (*"your own runs speed up before the team's numbers move"*), rather than loading the M4 container with a team-tier caveat.
