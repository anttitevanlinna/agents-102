...
**Control-loop diagram (2026-07-04):** the canonical closed-loop block diagram relabeled in student words, inlined as `{{figure:student-closed-loop}}`. No SVG ids, so no collision with the engine map's `mf-` ids on the theory-handbook page. The caption carries "robotics and autopilots have run on for decades" once.
