...
A plain prompt file for Module 3 Phase 1 of Claude Basics. Each participant pastes this in their own Cowork session with their participant folder and the `shared` folder added as working folders. Claude conducts a one-question-at-a-time interview about top three rollout difficulties and saves notes in the participant working folder.
...
- The save destination is the participant working folder, not the `shared` working folder. Only group drivers write to `shared`
