...
1. **Using capabilities to the full** — multi-agent orchestration, self-improving loops, benchmarking, source-watching, background execution, agents-spawning-agents, live-system tool use. The agent does the heavy lifting.
...
**Understandable magic, not wizardry.** The student must be able to explain what happened and why it worked. *"Five detectors ran in parallel on the same briefing, a scorer measured them against a five-claim benchmark I wrote myself, detector X won because Y, now I have a judge file I trust for this shape of output."* That is understandable magic. *"The agent just figured it out"* is mystification — and mystification trains dependency, the opposite of competence-first. Awe through clarity (Risto's epistemic directness), not awe through opacity. Mechanism must be legible in the artifacts the exercise leaves behind.

**Canonical magic patterns** (menu — more exist; these are known-good, Antti has run all of them):
- **Orchestrator + fixed judge + self-improving generator** — orchestrator runs N rounds against an unchanged judge; between rounds, the generator's strategy file gets rewritten from per-claim judge feedback. Score trajectory under a yardstick that never moves.
- **Benchmarking / method benchmark** — N judges or detectors run in parallel on the same input; a scorer measures and picks the winner (or ensemble). Teaches empirical selection over intuition.
...
| M5 | Watching a bounded benchmark / loop run |
| M6 | Walk-away — loop runs to score trajectory under fixed judge |
...
- **Pick your runtime — Claude Code or Cowork.** Both work. The exercises are runtime-agnostic; the site has a switcher.
  - **Claude Code Desktop** (macOS/Windows GUI) or **CLI** (terminal) — file-system-native flow.
  - **Claude Cowork** — the *Cowork* tab in Claude Desktop, connect your training-dir folder once. Best if you don't want to think about CLI plumbing; plugin install is one button.
  - **Same engine underneath**, different UI. `CLAUDE.md` and subagents work identically; the same `.plugin` files install in either.
...
**The plugin-authoring module — taught once, here. Split into two exercises.** No pre-shipped plugins anywhere in Bootstrap. M4 is the canonical place plugin-authoring is introduced; the move is split into two exercises so first-time authoring gets real time rather than a three-minute warmup. The student authors a plugin in the first exercise, then runs it against their Module 3 system in the second. Skills arrive as the mechanism — and as **expertise the student authors and plugs in.** Two lenses, one plugin: a company-policy lens (using the customer's actual rules where the engagement provides them, or a Nordic-baseline reference set as the fallback) plus an agent-security lens covering four named attack classes (prompt injection direct + indirect, secrets-in-context-and-scrollback, tool-confusion, plugin supply-chain) plus access-control analysis. The named classes are external grounding — without them, the lens self-grades against a rubric Claude wrote and the threat model collapses into taxonomy.

Prework: two reads. (1) Plugins — what they are, how they package an expertise into the agent, why authoring one is the move. (2) Agent security layered on classical software security — same residual-risk vocabulary and CVSS-style probabilistic reasoning, faster cadence (residual moves while you watch), four new attack classes by name, and new mitigation shapes (scope, split, filter, gate, review) layered ON TOP of classical perimeter / IAM / mTLS / audit, not as a replacement.
...
- Lecture: **"The practice of risk"** — absolute certainty never existed in classical security either. The practice is the loop: assess → mitigate → reassess residual risk → decide. Two lenses: access control (what can it reach vs. should reach) and named attack classes (prompt injection, secrets-in-context, tool-confusion, plugin supply-chain). Agentic mitigations (scope, split, filter, gate, review) layer on top of classical controls — additive, not substitutive. And the best mitigation is the one you don't need: don't open the biggest risks in the first place.
- Exercise 1: **Author a security plugin** (~25 min) — student dictates 3–5 lines of what matters in their own voice (not pre-reading the policy files); Claude reads `module-4/policies/` and proposes the plugin; student steers. The agent-security lens MUST cover the four named attack classes by name. Per-runtime install: Cowork *Save plugin* button (most scaffolded — Cowork users have less terminal fluency); Desktop plugin loader; CLI `~/.claude/plugins/<name>/`. Verify with one quick invocation against a single file in a fresh session. Plugin lives on disk; carries forward to every agent the student builds.
- Exercise 2: **Audit your agent** (~45 min) — run the plugin from Exercise 1 against the full Module 3 system. The plugin carries two lenses: policy (compliant / violating / "I can't tell" per rule) and agent-security (access-control analysis + named-class pass with ranked agentic mitigations layered above classical controls). Read both reports. Pick ONE risk. Apply the mitigation. Re-run. See the residual risk shift — not vanish.
...
> CUSTOMER: The customer's actual policies (data-usage, security, AI-use, sector-specific — GDPR, NIS2, DORA, MIFID, sector codes) feed the plugin the student authors during M4. Where the customer provides the source policies as inputs, Antti pre-distils them into a reference `.md` the student reads from while authoring (separately billed prep). Where there is no customer engagement (self-study, open enrolment), a Nordic-baseline reference set is the fallback input. **No pre-built plugin ships.** The plugin is the student's, authored in the exercise.
- Concept (emergent): Plugins as expertise injection, authored once in M4, not just scoping. Prompt injection as a named class (direct + indirect). Layered mitigations — agent surfaces above classical controls, not in place of them. Residual risk as a first-class artifact. The best mitigation is not opening the door.
- Reflection: "What's one 'I can't tell' row you're leaving 'I can't tell' today — and what would it take to close it?"
- Homework: extract one rule from a company policy and author it as a small plugin yourself (e.g., customer-data classification, retention boundaries). Every student produces one before Module 5. Plus: run the plugin you wrote in M4 against the other agents built since M2 (they won't come back clean — unaudited agents stacking up feeds the deepened-unease mood, doesn't steal it). Plus: agent-sprawl reading (shadow agents, the 82%-think-protected / 24%-have-visibility pattern).
...
- Exercise: **Hallucination benchmark** — Five detectors run on the same output (your Module 3 briefing): source triangulation, entailment, citation integrity, self-consistency (four blind regenerators with different framings, collated), counter-evidence search. A scorer agent measures each against a 5-claim benchmark you annotate in 2 minutes. Student watches the scoreboard fill in — doesn't classify claims by hand. Winner saves to `judges/groundedness-judge.md` with a mandatory "Known limit:" line naming what it misses. M6 picks this judge up and evolves it. **Magic beat (understandable):** five methods, one corpus, the scoreboard IS the mechanism — *"ahh, this is actually fixable, and here's empirically WHICH detector caught what."* Not triumph: the judge is plain about its blind spots, M3's strategic uncertainty and M4's security residual still stand.
...
- Classic-way contrast (between Phase 2 scoreboard and Phase 3 winner-save): Claude prints a three-line comparison in chat — what the classic way would have been, faster or slower, why. No file, no ask-the-student mechanic. Locates the benchmark against the classic default without adding a pause beat or an artifact.
- Take-home (Close): a stealable prompt that names the five techniques (source triangulation, entailment, citation integrity, self-consistency, counter-evidence search) and asks Claude to set up the system for the student's chosen output, with an adaptation instruction to swap techniques that don't fit the output shape. Short, technique-first, no exercise-specific references. Operationalizes the "this pattern transfers" assertion without adding a phase.
...
- Exercise: **Eval loop** — Four phases. **Phase 0 (~3 min):** single prompt generates a fresh briefing, runs the M5 judge against it, saves judgment + per-claim feedback to `module-6/first-run-judgment.md`, and prints a three-line chat summary. Gives the student one "I ran my judge end-to-end" rep before the loop starts. Transition: *"that's the manual version, now automate it."* **Phases 1-3:** Orchestrator session runs **one generation track × 3 rounds** against a fixed judge. The winner judge from M5 (`judges/groundedness-judge.md`) grades each round and writes per-claim feedback alongside the score. Between rounds, the orchestrator reads the judgment (+ optional student `deltas.md`) and rewrites `module-6/generator.md` — the generator's strategy file — so the next round's generator learns from the feedback. The judge file is never touched. **Student walks away** for ~25–30 min while rounds run — first true walk-away on the trust-distance ramp. Returns to `strategy-diff.md` per round (legibility artifacts), a score trajectory (flagged-claim count dropping 11 → 6 → 3), and an integrity check confirming the judge file is byte-identical to its M5 state. **Magic beat (understandable):** mechanism legible in the diff files — student points at specific strategy rules added and reads the feedback that produced each one; judge didn't move. *"We can automate the loop — the yardstick stays still and the work gets better."*
...
- Take-home (Close): a stealable prompt that names the four moves (generation via subagent, scoring + per-claim feedback, strategy rewrite between rounds with the judge file untouched, rounds + dashboard) and asks Claude to set up the loop around any judge the student already owns. Short, move-first, no M6-specific scaffolding. Operationalizes "the loop is a pattern, not a one-off."
...
Bootstrap deserves the same check, adapted to its audience (builder leaders making the chat-to-systems leap — not software engineers). Expected shape of results:

- **Similar meta-pattern**: a handful of gaps that become homework / pre-read / one-line beats; a handful of competitive archetypes (vendor copilot courses, AI-for-execs cohorts, change-management consultancies, conference workshops, YouTube deep-dives, corporate L&D); a Nordic-field whitespace read.
- **Different practitioners** to blind-OODA. Candidates: Ethan Mollick (Wharton; everyday.ai), Dan Shipper (Every), Mikko Alasaarela (Agentics Helsinki), Steph Smith (a16z), Lenny Rachitsky. Pick two with 6-month public output that matches builder-leader framing.
- **Different landscape scan**: publishing/cohort curriculums targeted at builder leaders, not engineers. Maven courses in the exec/GM tier, Wharton Interactive, IMD Lausanne, Section, Reforge's AI tracks, internal L&D programs written up publicly.
- **Different competition scan**: who sells agent training to SVP-marketing / SVP-ops / product leaders in the Nordics.

**Expectation: similar results in method, possibly similar in shape** (homework-pairs as the right slot; a few permission beats; whitespace around "how the team learns to use this" that Bootstrap already sits in). Don't assume the gap list transfers verbatim — Bootstrap's buyer is different and the convergence layer is different. Re-run the method, read the data.

**Do NOT start until:** (a) first AE101 cohort runs and the method gets a feedback loop from live use; or (b) Antti explicitly decides Bootstrap needs the audit ahead of AE101 cohort feedback. Low urgency, high confidence the method will hold.
...
**2026-04-20 session — agenticness-contract pass.** Three modules rewritten to honor the Agenticness Contract (walk-away test + 50% magic rule + understandable magic + trust-distance ramp): M5 benchmark, M6 orchestrator+self-improving eval loop, M7 pattern-catalog-driven with strategy skills absorbed. Magic beat count across M3–M8 now 4 of 6 (M3 three-retrievers, M5 benchmark, M6 eval-loop, M8 flywheel) — comfortably over the 3-of-6 threshold. Source-watcher/scheduled pattern confirmed already shipped via M2's personal-agent-homework. M8 live-deliberation direction flagged tentatively pending runtime capability (Cowork or successor). Personal-to-team patterns catalog scaffolded at `strategy/personal-to-team-patterns.md` — needs practitioner-grade examples filled in. Newsletter draft for the catalog lives in `~/Projects/bosser/drafts/newsletter-personal-to-team-patterns.md` (private — bosser article system).
...
- **Module 5 (Output Quality):** module file reframed (center of gravity: GROUNDED as the discipline; fabrication as the failure mode when there is no ground to stand on). Exercise rewritten 2026-04-20 to **Hallucination benchmark** (see State of play → REWRITTEN entry below for canonical detail). Previous exercise (**Ground your output** — five-category classification by hand) is kept as optional reference, scheduled for delete at next sweep if unused. Lecture `grounded.md` — original shape leads with GROUNDED as discipline + compound reliability math (85%×10=20%) + three detection techniques (citation re-verification, adversarial pass, consistency probe); **needs reshape** to frame the three as candidates-to-measure in the benchmark rather than methods-to-know. Canonical detector panel is five (source triangulation, entailment, citation integrity, self-consistency, counter-evidence search). Lecture eval dispatched before reshape; redo after reshape.
- **Module 4 (Security):** module file reframed (mood: epistemic humility, not guilt; big idea: *you can't tell if your agent is safe by looking at its output — the practice is running the check*). **No pre-shipped plugins anywhere in Bootstrap (Antti, 2026-04-25); M4 is the canonical plugin-authoring module — taught once, here, split into two exercises so first-time authoring gets real time.** Plugin-authoring reshape landed 2026-04-25: split *Audit your agent* into *Author a security plugin* (~25 min — Phase 1 student dictates, Phase 2 author both lenses with named attack classes, Phase 3 install + verify with paired `.rt-cli` / `.rt-desktop` / `.rt-cowork` divs and Cowork-fork-needs-MORE-scaffolding) plus *Audit your agent* (~45 min — runs the plugin against the full Module 3 system). Scaffold `curriculum/scaffolds/module-4-starter/` carries policy reference content only (Nordic-baseline `.md` for self-study, customer-distilled for engagements) — input material the student reads while authoring, never a pre-built plugin. Prework with two sections: plugins + agent-security-layered-on-classical-security (re-written 2026-04-25 to drop the "if you have a security background half of this will sound wrong" framing the security-skeptic sim flagged as flattering a caricature; classical security has CVSS / residual / NIST — agent security adds non-determinism + four named attack classes on top, not in place). In-room lecture **The practice of risk**. Audit produces `module-4/policy-report.md`, `module-4/security-report.md`, `module-4/residual.md` with a "doors I would rather not open" scoping rule. Substantive audit-finding fixes 2026-04-25: prompt injection now named class in Key Concepts; "not firewalls" rhetorical foil reframed as **layered**; AGENT-SECURITY lens grounded in four named attack classes (prompt injection direct + indirect, secrets-in-context, tool-confusion, plugin supply-chain) so it cannot collapse into closed-loop self-grading. Lecture eval APPROVE; exercise evals owed re-run after the split. Simulation 8/10 "this is me" — mood lands. **Blocking before first delivery:** capability check on per-runtime plugin install paths (`claude-code-guide` Cowork *Save plugin* + Desktop loader + CLI `~/.claude/plugins/<name>/`); em-dash sweep across module + both exercises; lecture Pass 3 polish.
...
- **Module 5 (Output Quality) — REWRITTEN 2026-04-20 as magic beat:** Hallucination benchmark exercise (5 detectors × scorer × five-claim benchmark × winner judge saved). Supersedes `ground-your-output.md` (kept as optional reference; delete at next sweep if unused). Lecture `grounded.md` reshaped same session — reframed as "five candidates to measure" (empirical method selection), matches exercise's five detectors, ~900 words.
- **Module 6 (Evaluations) — REWRITTEN 2026-04-20, reframed 2026-04-24 as magic beat:** Eval loop exercise — orchestrator + fixed judge + one generation track × 3 rounds; generator's strategy file rewritten between rounds from per-claim judge feedback; judge file byte-identical across the run (a yardstick you can rewrite is not a yardstick). Canonical artifact of Antti's own practice. Supersedes `groundedness-eval.md` and `steering-eval.md`. Steering-eval kept as supplementary for variants.
- **Module 7 (Personal to Team) — REDESIGNED 2026-04-20 as canonical, surgical strip 2026-04-25:** `share-your-work.md` exercise applies three thinking-disciplines inline (Rumelt's crux, Martin's assumption-test, Klein/Kahneman's pre-mortem — named in prompt bodies, not pre-shipped plugins; per Antti 2026-04-25 strict no-pre-shipped-plugins rule, M4 is the canonical plugin-authoring module). M7's job is *from-personal-to-team* sharing; the move is sharing prompts / `.md` files / shared `CLAUDE.md` rules, not plugin-shipping. Uses patterns catalog as reference, two scenario branches (cloud agent infra / personal-Claudes-only), technical + people plan both required, hardness-is-a-feature (incomplete endings = Monday's work). Supersedes `promote-your-challenge-memory.md`. Pattern catalog (`strategy/personal-to-team-patterns.md`) is scaffolded; patterns need Antti's practitioner detail.
...
- Module 8 — module file exists with Extend + Joint Double Diamond exercises (inline in module file, no separate exercise files yet). The three thinking-disciplines (crux / assumption-test / pre-mortem) are named inline in M7 and re-applied inline in M8 — no pre-shipped plugins, no skill re-invocation language (per Antti 2026-04-25 strict rule). Live Deliberation direction flagged tentatively in M8 file (networked personal agents on shared runtime, orchestrator-as-agent, humans at decision layer — Cowork is real now, not a future hedge).
...
4. **Module 4 capability check** — verify per-runtime plugin authoring + install (Cowork *Save plugin* button, Desktop plugin loader, CLI `~/.claude/plugins/<name>/`) and plain-language invocation reliability across all three runtimes; pre-bake `/skill use` fallback into prompts if not. Blocking before first delivery.
5. **Module 4 customer-input checklist** — the out-of-repo process and input checklist for distilling a customer's policies into reference `.md` material the student reads from while authoring their plugin. Referenced from `scaffolds/module-4-starter/README.md`. Needed to scope the separately-billable prep work and run a first customer engagement. (M4 plugin-authoring reshape itself: landed 2026-04-25 — split into two exercises, named attack classes, layered framing.)
