#!/usr/bin/env bash
# arrange-agents-101.sh — reset the Agents 101 training dir to a clean
# prework baseline and stage the selected case's source material.
#
...
#
# Usage: arrange-agents-101.sh [--case nordveil|finnish-psychologist] [--cwd DIR] [--material DIR]
set -euo pipefail
...
# ---- Stage the case source material (M2 ingest pulls from here) -----------
...
for source_dir in wiki docs; do
  while IFS= read -r source_file; do
    strip_comments "$source_file" > "$material_dir/$source_dir/$(basename "$source_file")"
  done < <(find "$KIT/sources/$source_dir" -type f -name '*.md' | sort)
done
strip_comments "$KIT/$A101_LOCAL_SOURCE" > "$material_dir/local/$(basename "$A101_LOCAL_SOURCE")"
strip_comments "$KIT/$A101_WEB_BASE_SOURCE" > "$material_dir/web/$(basename "$A101_WEB_BASE_SOURCE")"
strip_comments "$KIT/$A101_M2_HELD_SOURCE" > "$material_dir/new/$(basename "$A101_M2_HELD_SOURCE")"
strip_comments "$KIT/$A101_M3_HELD_SOURCE" > "$material_dir/new-m3/$(basename "$A101_M3_HELD_SOURCE")"
...
if a101_case_supports_module m8; then
...
echo "[arrange] staged case=$A101_CASE material in $material_dir:"
