...
**CYCLES 175–177 — LEARNING CODING AGENTS: THE PIECES ARE REAL; THE COMPLETE HUMAN-GATED, CONTROLLED LOOP IS NOT YET A PRACTITIONER STANDARD (L2).** The closest field report converts accepted human review comments into version-controlled behavioral rules: one microservices team observed zero recurrence across nine ruled-against error classes and 74 post-rule session exposures, but over only 11 recorded sessions, four weeks, one primary language, and no A/B control. Every's current implementation adds current and historical session mining plus a grounded `docs/solutions/` store; ReasoningBank adds measured automatic trace → memory → later SWE-bench reuse; Ruflo DREAM adds a durable baseline/candidate ledger with real human rejection and remediation. No implementation in this sweep joins automated session harvest, explicit human rule promotion, and a controlled comparable-task rerun. ([Accumulated Behavioral Rules](https://arxiv.org/abs/2607.13091) — [academic]; [Every `ce-compound`](https://github.com/EveryInc/compound-engineering-plugin/blob/b159e1fa4c70efa995742269d38269bcc7524dd2/skills/ce-compound/SKILL.md) — [practitioner direct]; [ReasoningBank implementation](https://github.com/google-research/reasoning-bank/blob/ed80611788292ea739f1effd31f16c53823b8a0d/third_party/src/minisweagent/run/extra/swebench.py) — [academic/research]; [Ruflo DREAM ledger](https://github.com/ruvnet/ruflo/blob/277c7bc03ad192eef6d6f57e59ab7bab69a5728d/docs/dream-cycle/LEDGER.md) — [practitioner direct])
...
**CYCLE 143 UPDATE — Fable 5 Day 12 still offline; Polymarket June 26 at 10% (collapse from 41%), July 31 at 73% — market has abandoned near-term restoration thesis; Legion filed first legal challenge to ban; Ronacher "The Coming Loop" (June 23) crystallizes practitioner belief shift; Osmani (June 15) — review is now the leveraged engineering skill; ServiceNow IT Specialists 26th absence (6 days to June 30 deadline); Cursor/SpaceX date correction: Q3 2026, not July 12.** No Fable 5 restoration as of June 24. Trump verbally softened post-G7 Amodei meeting but Commerce Dept directive unchanged. Legion (legal-tech startup) filed suit against US government — first formal legal challenge. **Critical practitioner belief convergence:** Ronacher published "The Coming Loop" (June 23). **TWO CORRECTIONS APPLIED 2026-07-31 — this paragraph previously carried both errors; do not restore.** (1) *"I don't prompt Claude anymore… My job is to write loops"* was quoted here as Ronacher's own belief statement. **It is Boris Cherny's**, used by Ronacher as an epigraph and attributed to Cherny directly beneath the blockquote. Cycle 145 flagged this on 2026-07-14 with an explicit back-sweep instruction naming this very section; the flag went unactioned for seventeen days. Ronacher's own claim is structural: *"work is put into a queue of sorts, a machine picks it up, attempts it, stops, and then some harness decides whether that was actually the end."* His stance is also **skeptical**, not evangelical — *"I have not had much success with this way of working for code I deeply care about"* — so the "identity shift crystallized" reading that ran here inverted his mood. Consequence for the convergence claim below: Ronacher remains a genuine independent voice on harness loops, but **not** on Cherny's sentence; do not count him twice. (2) The 93.4% figure was presented as Osmani's empirical finding with methodology unknown. **It is not Osmani's data and the methodology is published.** Osmani says so himself — *"The most useful result I have seen this year is not from a vendor"* — pointing at v.j.k.'s experiment ([dev.to/_vjk](https://dev.to/_vjk/best-ai-code-reviewer-in-2026-we-ran-4-in-parallel-for-3-weeks-146-prs-679-findings-1c0f) — [practitioner direct]): 146 merged PRs over 3 weeks, four AI reviewers in parallel, 679 findings collapsed to **617 distinct (file, line) coordinates, which is the denominator** — 576 of 617 caught by exactly one tool. Single team, single experiment → **L2**, and the axis is tool-vs-tool, not human-vs-agent. Osmani's own contribution here is the framing, not the number: "The hard part of engineering moved from writing code to deciding whether to trust it" — review is now the leveraged skill. Defensive code failure mode named by Ronacher: models are "mortally terrified of exceptions" (citing Karpathy), generating fallbacks rather than enforcing invariants. 4x code output → ~10% delivered value; gap is hidden review work. Ronacher and Osmani do still converge independently on the substantive point: expertise concentrates at architecture/judgment level, not generation. Cursor/SpaceX Q3 2026 close (not July 12 — prior cycle inference, not confirmed deal term; Bloomberg + SEC 8-K say "Q3 2026"). ([lucumr.pocoo.org/2026/6/23/the-coming-loop/ Jun 23 2026](https://lucumr.pocoo.org/2026/6/23/the-coming-loop/) — practitioner direct; [addyosmani.com/blog/agentic-code-review/ Jun 15 2026](https://addyosmani.com/blog/agentic-code-review/) — practitioner direct; [letsdatascience.com Jun 2026](https://letsdatascience.com/news/legion-sues-us-over-lost-access-to-anthropic-models-2e697bd4) — domain trade publication; [Bloomberg Jun 16 2026](https://www.bloomberg.com/news/articles/2026-06-16/spacex-cements-60-billion-deal-to-take-over-ai-startup-cursor) — general press; [Polymarket](https://polymarket.com/event/claude-fable-5-restored-for-us-customers-by-20260613193753196) — prediction market)
...
- MCP Tool Search: lazy loading defers tool definitions until needed — enables running many MCP servers simultaneously. ~~reduces context usage by 95%~~ **[UNVERIFIED STAT — do not cite, 2026-08-01]**: this line carried a bare 95% with no citation, the only entry in this list without one. Anthropic's own current docs were fetched directly and **decline to quantify the benefit at all** — the nearest language is qualitative: *"Tool search keeps MCP context usage low by deferring tool definitions until Claude needs them… adding more MCP servers has minimal impact on your context window."* No percentage, no benchmark, no workload, no sample size on either the MCP or tools-reference page. Round number, no source, contradicted by tracing it to the vendor's own documentation — the zombie shape. Until an original with methodology surfaces, use the qualitative version. ([Claude Code Docs — mcp](https://code.claude.com/docs/en/mcp) — [vendor documentation])
...
**Opus 4.8 limitations (from practitioner-direct testing, Zieminski, May 28 — [SOURCE NEEDED]: no URL on file for the Zieminski account):**
...
**August 2026 — non-model control flow is now shipped product, not a DIY pattern (documentation sweep, 2026-08-01):**

The interesting delta is not a new tool; it is *who decides whether the run continues*. Through 2025 the answer was always the model: it reads context and tool results, and it chooses the next step or stops. Four shipped, documented Claude Code mechanisms now put some part of that decision outside the working model. All four are opt-in and off by default — the significance is that they exist as product surface, not that they fire unasked.

- **`/goal`** — *"adds a separate evaluator that checks your condition after every turn, so completion is decided by a fresh model rather than the one doing the work."* Still a model deciding, but a **different** one, with its own context. ([Claude Code Docs — /goal](https://code.claude.com/docs/en/goal) — [vendor documentation])
- **`/loop` at a fixed interval** — a literal cron-style scheduler outside any model, plus a harness-authored default when the model does not act: *"If an iteration ends without either rescheduling or stopping, Claude Code schedules one fallback wakeup about 20 minutes later and ends the loop when that iteration doesn't reschedule either."* This is the clearest instance of a non-model component choosing the next action. ([Claude Code Docs — scheduled tasks](https://code.claude.com/docs/en/scheduled-tasks) — [vendor documentation])
- **`Stop` hooks** — an arbitrary script, not a model, *"Prevents Claude from stopping, continues the conversation."* `PreToolUse` can symmetrically deny a tool call the model chose. ([Claude Code Docs — hooks](https://code.claude.com/docs/en/hooks) — [vendor documentation])
- **Agent teams** — dependency bookkeeping is mechanical (*"when a teammate completes a task that other tasks depend on, it unblocks the dependent tasks"*), though which unblocked task a teammate claims remains its own choice. ([Claude Code Docs — agent teams](https://code.claude.com/docs/en/agent-teams) — [vendor documentation])

**What does NOT belong in this bucket, tested and excluded.** Context compaction looks like harness takeover and is not: the trigger is a token threshold the harness owns, but the summary itself is a model-authored request (*"/compact reads the conversation it summarizes"*). Subagent delegation is a model tool-choice — the parent decides to delegate, and each subagent runs its own model-driven loop. Both were the obvious candidates and both fail the test. ([costs](https://code.claude.com/docs/en/costs), [sub-agents](https://code.claude.com/docs/en/sub-agents) — [vendor documentation])

**That "token threshold" is not one fixed number — re-checked against the live docs 2026-08-31.** Sonnet 5 on the API compacts "at about 967K tokens by default" of its 1M context window; the 200K-boundary default list reads Sonnet 4.6 and Opus 4.6 without extended context, joined by Opus 4.8/5; cloud sessions compact "as the conversation approaches the model's limit" (the fixed 200K-boundary phrasing for cloud is off the page); other local sessions fire "when the conversation reaches the model's context limit." `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` (1–100) exists and can only lower the threshold, never raise it; model-config also names `CLAUDE_CODE_AUTO_COMPACT_WINDOW`. There is no single percentage or fixed token buffer that applies across models and surfaces — ~~an earlier "~83.5% / 33K buffer" figure~~ **[SUPERSEDED — do not cite, 2026-08-01]**: the source is known and is the problem. It traces to https://claudefa.st/blog/guide/mechanics/context-buffer-management, a page carrying no byline and no date, and the figure does not match the per-model/per-surface picture above. Retired on the docs, not for want of a source. Do not go looking for a better citation for it. ([env-vars](https://code.claude.com/docs/en/env-vars), [model-config](https://code.claude.com/docs/en/model-config) — [vendor documentation])

**Why it might matter for the CTO question — on one practitioner's account, pending corroboration.** Armin Ronacher described this layer roughly five and a half weeks before the docs sweep found it shipped (Jun 23 → Aug 1), and framed it as the coming architecture rather than a feature: *"some harness decides whether that was actually the end. If not, the harness continues the same session, injects another message, starts a fresh session with modified context, or sends the task to another machine. The task stays alive beyond the point where the model by itself would normally have said: 'I am done.'"* He is explicit that this is an **outer** loop distinct from *"the agent loop inside every coding agent"*, and equally explicit that he does not welcome it, since it is what reduces the engineer's role *"to that of a messenger."* ([lucumr.pocoo.org, Jun 23 2026](https://lucumr.pocoo.org/2026/6/23/the-coming-loop/) — [practitioner direct])

**His account does not cover all four mechanisms, and should not be read as though it did.** It maps cleanly onto `/loop` (the harness continuing a session on its own schedule) and onto `Stop` hooks (a non-model component deciding the run was not over). It fits `/goal` weakly — that is still a model deciding, only a different one — and it is tangential to agent teams, which is bookkeeping about which task gets claimed rather than about whether a run continues. He never enumerated this bucket; we assembled it.

**Classification — two separate claims, deliberately not averaged into one.**
- **Capability** (do these four mechanisms exist and behave as described): **confirmed.** Vendor documentation is L0 as *evidence* but authoritative for *capability*; every quote above was fetched from the live page on 2026-08-01 and all four re-confirmed verbatim on the live pages 2026-08-31. One capability drift on that re-check: agent teams are now explicitly experimental and disabled by default behind `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`.
- **Significance** (is this a direction the field is moving): **L1.** One named practitioner asserts it. The vendor docs contribute nothing here — an L0 source carries zero weight on a trend question, so there is no second leg to average with, and an earlier draft of this entry called the pair L2 by doing exactly that.

Not higher: no second platform confirmed shipping equivalent non-model control flow in this sweep, and no practitioner accounts of running these mechanisms in production were located. **Watch:** whether any of the four moves from opt-in to default, and whether Codex / Cursor / Gemini CLI ship an equivalent. That flip is the signal worth catching, because it changes the answer to *who is accountable for the run* without changing anything a user sees.

**MCP tool search is default-on, and the consequence is under-recorded.** The benefit is real but not quantified by anyone (see the struck 95% above — Anthropic declines to put a number on it). The cost is that a session's tool set is two-tier — only tool names and server instructions load at session start, with full definitions fetched on demand (mechanism holds on the 2026-08-31 re-fetch; the two sentences this entry once quoted verbatim are off their pages as of that check, so the claim rides the documented behaviour, not a quote). Practical effect for anyone auditing what an agent can reach: asking the agent is not an inventory. ([mcp](https://code.claude.com/docs/en/mcp), [tools-reference](https://code.claude.com/docs/en/tools-reference) — [vendor documentation])

**Re-check cadence for this entry: `[checked:2026-08-31 result:OK due:2026-09-28]`.** Every source above is an undated live documentation page carrying no on-page revision date, so none can be freshness-checked the normal way. Vendor docs go stale on product time — weeks — not on the six-month evidence clock, and a snapshot with no stated expiry rots silently. Four weeks. 2026-08-31 re-fetch (env-vars, model-config, mcp, tools-reference, plus goal, scheduled-tasks, hooks, agent-teams for the four control-flow quotes) — holds: all four control-flow quotes verbatim on their pages; 967K default; 200K for Sonnet 4.6/Opus 4.6 sans extended context; `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` (1–100) lower-only; tool search default-on (stated on the current mcp/tool-search pages). Drifted: cloud sessions now compact "as the conversation approaches the model's limit", not at the 200K boundary; Opus 4.8/5 join the 200K list; model-config now names `CLAUDE_CODE_AUTO_COMPACT_WINDOW`; the mcp "Only tool names and server instructions load at session start" sentence and the tools-reference "conversational summary / run `/mcp`" sentence are off their pages. The entry body above states the re-fetched picture.
