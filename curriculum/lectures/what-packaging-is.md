...
- The weights hold the written record: the setup posts, the plan-file templates, the verifier write-ups, the reversals. More of the field than you will read in a career.
...
**Current shape (2026-08-14, Antti-directed):** nine slides. The three-pattern lands first; `Theory 1`, `Theory 2`, and `Theory 3` deliberately reserve the unsettled theory passage; hooks and re-feed retain the implication end. The placeholders carry no body until the theory is settled and are not shippable final content. The Intercom tier case has its home in M6's `the-loop-has-a-name.md`, so M5 carries no duplicate. Context-window management and the subagent-isolation callout live in M4's durable-state slide.
...
**Slide progression:** chart → reference and plan → verifier → three reserved theory slides → hooks → re-feed. The opener remains recognition-before-naming; the theory passage is being designed between the kit and its implications.
...
- `weights-hold-the-written-record` · detail · "The weights hold the written record: the setup posts, the plan-file templates, the verifier write-ups, the reversals." ← amp-handoff, amp-neo
...
- `Theory 1`, `Theory 2`, and `Theory 3` are deliberate blanks while the theory passage is still being designed. Do not improvise body copy into them during delivery.
