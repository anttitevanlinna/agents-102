...
**The unsolved problem — performance evaluation:** DORA 2025 (n=5,000) confirms traditional engineering metrics can't separate AI-generated from human-authored work. Shopify's blunt approach: add AI usage to performance reviews (measuring input, not output quality). StrongDM's approach: holdout scenarios evaluate system output. Intercom's approach: resolution rate, CSAT, automation coverage evaluate system output. **Nobody has published a framework for evaluating the human spec-writer's quality.** This is the biggest open question for AI-native leadership. Cycle 93 searched 10 different queries and fetched 6+ pages — confirmed: no such framework exists in the public record.

**Cycle 93 update — closest available evidence on spec craft (not evaluation):**
- **Osmani's 5-principle framework** (Jan 19, 2026 — practitioner direct, Google Chrome engineering): (1) high-level vision first, (2) PRD structure with 6 areas (commands/testing/project structure/code style/git workflow/boundaries), (3) modular prompts not monolithic, (4) three-tier boundaries (Always/Ask First/Never), (5) iterative evolution. GitHub analysis of 2,500+ agent config files confirms "too vague" is the #1 failure mode. [addyosmani.com/blog/good-spec/](https://addyosmani.com/blog/good-spec/) — [practitioner direct]
- **Mollick's implicit criterion:** "knowing what good looks like" (domain expertise) is the prerequisite. Experts can immediately scope problems, define deliverables, and assess output quality. Craft advice, not evaluation rubric. [oneusefulthing.org/p/management-as-ai-superpower](https://www.oneusefulthing.org/p/management-as-ai-superpower)
- **Unicrew synthesis (Feb 2026):** Three elements of good spec: "clear acceptance criteria, edge cases documented, input/output contracts defined." Framed as craft advice.
- **Gap confirmed:** Osmani provides WHAT makes a good spec. Nobody has published HOW to evaluate whether a HUMAN consistently produces them. No scoring rubric, no interview framework, no performance review criteria. This is original research white space — opportunity to be first.
...
0. **A working framework for evaluating spec-writer quality in AI-native teams** (added cycle 74, hardened cycle 88, partial signal cycle 118). Shopify measures AI usage (input metric). DORA 2025 (n=5,000) explicitly confirms traditional metrics cannot separate AI-generated from human work. StrongDM and Intercom evaluate system output, not the human's contribution. **Nobody has published "here's how we evaluate whether a person is good at directing AI" at deployment scale.** DORA's independent confirmation upgrades this from KB gap to field-wide gap. **Level 3 absence finding** after 118 cycles. **Partial signal (cycle 118):** Optimum Partners (Dec 2025) proposes Centaur Pod + AI Reliability Engineer (ARE) role with four new metrics: (1) Defect Capture Rate (% AI-generated errors caught before staging), (2) MTTV — Mean Time to Verification (velocity of human review/merge of AI PRs), (3) Change Failure Rate AI-Specific (regression/rollback frequency), (4) Interaction Churn (prompt iterations per usable output — high churn = poor spec quality). These are proposed frameworks, not deployed ones. No named company uses these metrics. Gap status: vocabulary now exists; deployment evidence absent. ([Optimum Partners](https://optimumpartners.com/insight/engineering-management-2026-how-to-structure-an-ai-native-team/) — [practitioner analysis], Dec 18, 2025)

1. **Non-engineering AI-native teams — two named cases now, pattern emerging** (updated cycle 118, May 2026).
   - **F-Secure finance (March 2026, cycle 88):** Finance person coding with Claude for 9 months, automating bookkeeping, building in-house tool replacing Excel. Self-directed, sustained. Level 2 (single practitioner, direct observation).
   - **Atlassian HR + NORA (May 6, 2026, cycle 118):** HR staff built onboarding agent NORA on Atlassian Rovo without engineering. 70%+ of new hires used it in first month, 2,000+ questions, top 5 internal agent by WAU. Not a one-off — NORA is operational infrastructure. Level 2 (single company self-report; vendor platform — note confirmation bias risk, specific metrics mitigate). ([SiliconAngle](https://siliconangle.com/2026/05/06/ai-native-teamwork-spotlighted-atlassianteam/) — [domain trade publication], May 6, 2026; [Atlassian Community](https://community.atlassian.com/forums/Atlassian-AI-Rovo-articles/How-to-Create-a-High-Impact-Rovo-Agent-Lessons-from-NORA/ba-p/3157915) — [practitioner direct], Dec 4, 2025)
   - **Pattern emerging:** Finance and HR are the non-engineering beachheads. Both have verifiable output correctness — bookkeeping balances, onboarding completion — consistent with Pattern E (verification gap) prediction. Marketing and operations show high tool adoption but near-zero team-level agent building.
   - **Atlassian stat (May 2026):** "85% of knowledge workers use AI, but only 29% have embedded it in actual flows of work." Quantifies the embedding gap we've observed qualitatively.
   - The gap is narrowing: this was a confirmed zero in cycle 74. Now at two named cases in different geographies (F-Secure/Finland, Atlassian/global) and different functions (finance, HR). Not convergence yet — pre-convergence. Watch for a third case from marketing or operations.

2. **Candid failure retrospectives from AI-native teams.** UPDATED: Klarna is now a documented failure case — CEO admitted "too much efficiency, not enough quality." Forrester/Orgvue data (55% of companies regret AI layoffs) provides macro confirmation. Still need: team-level post-mortems with specific "what broke" detail beyond the Klarna narrative.
