...
- `sim-cache/` — content-SHA-keyed JSON traces (gitignored, user-local). One file per slug per class, prefixed by training short key: `<training>--<slug>.persona.json` (Class A) + `<training>--<slug>.behavior.json` (Class B). The prefix disambiguates same-slug files across trainings (`getting-going` exists in both Agents 101 and AE101).
