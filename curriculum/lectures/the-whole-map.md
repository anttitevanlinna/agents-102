...
<path d="M866,244 C858,230 998,244 1034,260 C1070,276 1110,340 1082,338 C1054,335 874,257 866,244 Z" stroke="url(#wm-wall)" stroke-width="2.4" opacity="0.8"/>
</g>
<g fill="#2f6b6b">
...
**Sources**
- groundwork-value-cycle `[checked:2026-07-31 result:ATTESTED due:none]` `theory-plan.md` § The Field Map — [maintainer-attested] The six columns are the house's own spine, imported from the groundwork pattern language and banked as a ★ backbone law. No external source is owed and — checked this cycle across four rosters — none exists: **nobody publishes this decomposition.** That is the honest position. The map is an assembly, and the assembly is ours.
...
**Flagged**
- `[found:2026-07-31, re-read and re-framed 2026-07-31]` **A machine can stand in Absorption's place — and Ronacher is the witness against that, not for it.** *The Coming Loop* (23 Jun 2026) names *"the harness level loop: the loop outside the agent loop"* and says what it costs: *"Even the 'done' signal loses all meanings and just becomes communicated to yet another machine that judges. My role is reduced to that of a messenger."* **This entry first read as though that were Ronacher's own model. It is not — corrected on a second pass.** He is contrasting two architectures and stating his own practice in the other one: *"In the agent loop, the model eventually says 'done' and I review. Even before that, I usually steer along the way. I am involved."* The harness loop is the trajectory he expects and resents, and his closing question is how to prevent exactly the outcome quoted above: *"how do we don't abdicate judgment, how we can retain rules of good engineering, how we can ensure that responsible human can continue to supervise."* → So the quote is the **stakes** of the phase, not a refutation of it, and it is citable as such. What we still cannot claim is that the field has settled on human-owned Absorption; a respected practitioner expects it to erode. The far-half lectures inherit this, and `the-map-filled-in` at M6 is where the answer shows. Same piece sits in `the-machine-you-just-met`'s Flagged for the sibling worry — one source, two columns, likely one decision.
- `[found:2026-07-31, sharpened 2026-07-31]` **Osmani samples runs rather than reviewing each one — the challenge is scale, not ordering.** *Software Factories* (20 Jul 2026): *"The Factory — Multiple harnessed loops running in parallel, feeding a shared review gate."* One gate, many loops. And in *Own the Outer Loop* (15 Jul 2026) the mechanism is explicit: *"the human doesn't need to be in the inner loop. We want them in the constraints loop… the sampling loop (how much output should we sample and review?), the audit loop…"* **A sampling loop means some runs are never read by a person.** This map draws Absorption as a column every run passes through. → Does Absorption bind per-run, or is it a policy set over a portfolio? At a student's scale the per-run reading is the true one and the column teaches correctly; the question is whether the map should say that its per-run framing is the personal-tier reading of a phase that changes shape at team scale. Note this entry originally also charged Osmani with *fusing* Verification and Absorption — **that was wrong and is retracted**; his Quality → Verdict is sequential and corroborates the split. See the `osmani-outer-loop` source entry for the correction.
