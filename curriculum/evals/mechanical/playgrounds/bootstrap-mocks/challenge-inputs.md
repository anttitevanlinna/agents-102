# Maija's substituted student inputs — bootstrap M2 mechanical run

These are the answers the harness pastes verbatim when the Actor's exercise prompts ask the student a question. The persona is Maija Lehtinen, SVP Engineering at Kaleva, rolling out agentic ways of working to ~300 engineers across two quarters.

## name-your-challenge prompt-001 — Q1, Q2, Q3 (paste one per turn)
...
**Q3 — Where are you stuck right now?**
...
## name-your-challenge prompt-002 — connectors and practitioners

When Claude proposes Confluence terms / OneDrive folders / practitioners, accept this as the source-preview list:
> Confluence ENG and SEC spaces; OneDrive `/Kaleva/Platform/`; practitioners Karpathy, Alasaarela, Klaassen.

## build-your-challenge-memory prompt-001 — Beat 1 curate

When Claude asks where work material lives:
...
## build-your-challenge-memory prompt-005 — Phase 2 author agent (Q1, Q2)
...
## build-your-challenge-memory prompt-006 — Phase 2 use agent (the task)
...
## build-your-challenge-memory prompt-008 — Phase 4 self-maintain (Maija's six decisions)

> Approve: fix the contradiction between the sub-team-structure page and the OKRs page on whether Infra can move in Q3 (OKRs are newer and correct).
> Approve: add the missing citation on the classification-policy claim in the security-platform page.
> Reject: the "stale" claim about the vendor session failing — it's not stale, it's a reference point we're still using as counter-example; keep it with a date-marker instead.
> Approve: the other three.

## build-your-challenge-memory prompt-009 — Close (the question)
