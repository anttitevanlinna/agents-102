...
- `[checked:2026-07-31 result:CAVEAT due:asap]` https://github.com/humanlayer/advanced-context-engineering-for-coding-agents/blob/main/ace-fca.md — [practitioner direct, vendor venue] Horthy is the only named practitioner attaching a number to this, and it is a **range held continuously** — *"keeping utilization in the 40%-60% range"* — not a trigger to wait for. A 60% cue reads his ceiling as his signal to act, which inverts him. Full stamp + freshness caveat lives at `the-loop-half-filled.md`, which already rules: do not present 40–60% as a current measured constant. **Do not repair the slide by swapping 60% for 40–60%** — that keeps the number and adds a misreading. fallback: name no number.
