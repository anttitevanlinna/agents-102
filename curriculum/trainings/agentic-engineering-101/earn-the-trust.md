...
Optional pre-reads before this module: Simon Willison, [The lethal trifecta for AI agents](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/) and [Designing agentic loops](https://simonwillison.net/2025/Sep/30/designing-agentic-loops/); [OWASP Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/) for a deeper scan.
...
Module 3's compound runs against the skill (its file content carries the conventions the side-quest window *m3-quality* encoded) and your `./CLAUDE.local.md` (where any rule about how you work with security skills, ADRs, or the access-map → STRIDE → test-strategy sequence proved worth keeping). Skills carry the codebase convention; rules carry your working style. The skill sharpen is mandatory; the rules-file update is opportunistic. The ADR you wrote this module compounded the moment it landed in the repo: a decision the next session reads without anyone prompting it.
...
- `[checked:2026-08-15 result:OK due:cohort]` https://code.claude.com/docs/en/headless — [platform docs] `claude -p` runs Claude Code programmatically and supports structured output for downstream programs. fallback: run the same bounded task interactively and save a machine-readable artifact.
...
- `[checked:2026-07-30 result:OK due:2026-11-23]` https://www.lucasfcosta.com/blog/backpressure-is-all-you-need — [practitioner direct] (Lucas F. da Costa, 2026-05-23; due is publication+6mo). Byline, date, and the claim that a human error-catching stage limits the system were verified against the original. The `## Pre-read before Module 4` entry paraphrases that limit. **This is the module that owns the check** — `run-the-first-experiment.md` echoes the same URL in its `## Prework` and delegates here; `plan-mode-done-right.md` also assigns this essay directly, in its `no-module:earn-the-trust` `## Pre-reads before Module 4` block for cuts that drop M3. fallback: drop the pre-read; M4's closing lecture still names the term from the student's live session.
- `[checked:2026-07-30 result:OK due:none]` https://simonwillison.net/2025/Sep/30/designing-agentic-loops/ — [delegated stamp] Willison, *Designing agentic loops*, the second `## Prework` pre-read echo. Dated check owned by `plan-mode-done-right.md`, which assigns it in the M2→M3 gap and carries the out-of-window decision. `due:none` — a delegation does not expire, the delegate's stamp does. fallback: drop the echo; the assigning file still names it.
- `[checked:2026-07-02 result:OK due:none]` https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/ — [delegated stamp] Willison, the `## Prework` optional pre-read that named the threat class. Dated check owned by `supplementary/the-lethal-trifecta.md`, itself `due:none` as a durable named framing. `due:none` here for the same reason a delegation carries no clock of its own. fallback: the supplementary carries the threat class in full.
...
**§38 fires on L49 and is not owed a fix (2026-09-08, `check_prompts §38b`).** The sentence immediately preceding the fence is the grab-quotes guard, and its verbs share nothing with the prompt's opening *"Read the test-strategy skill file at…"*. That is structural: the guard says what to do **before sending the prompt**, so it sits against the fence because that is where it fires. The rhyme it is owed is one sentence earlier, and L49's first sentence carries it against the prompt's *"find the one section… rewrite that section in place."* The reorder that would satisfy §38 literally was drafted and rejected on its own merits: it puts the opportunistic rules-file move ahead of the mandatory sharpen, inverting the priority L47 states outright, and moves the guard away from the beat it guards. Unfixed, this reads as mild drafting drift nobody is blocked by. Do not re-file, and do not reorder the paragraph.
...
| Module body, `ae101-m3-clear-session` — typing `/clear` | Fired on autopilot as the beat's full stop, before the decide-or-accept move has actually happened — or skipped at the break, carrying M3's security scrollback forward | Trainer holds the room one beat before the keystroke. If the clear slips entirely nothing downstream breaks: M4's first exercise opens its own new session (`walk-and-send-off.md`'s `**Session** *(new, …)*` widget), so the cost is carried context, not a lost artefact |

**Module-body rows added 2026-09-05 (eval-triage adjudication — not an Antti ruling; reverse it if he disagrees).** §47's unit is the forcing function, not the linked exercise, so this module's own three `{{prompt:}}` fences owed rows the exercise-phase rows never covered. Two pairs were lifted from body prose that already carried them (`## Read the sharpen with a skeptical eye`, and the decide-or-accept line under `## Clear the session`); the `/clear` row is the one that had no pair anywhere in the file, and it now states one. Nothing about §47 is declined here — a judge re-raising it against these three beats should read the rows, not this note.
...
**Trifecta pre-leash beat: added to `## Next` 2026-07-02, removed 2026-08-12 (Antti).** The three-question check + prompt-injection naming + supplementary pointer sat in M3's close aimed at M4's autonomy, and M4 carried a second firing of the same beat before its send-off. Both are gone: the trifecta belongs to M3's own body and to `supplementary/the-lethal-trifecta.md`, reachable from the training index's supplementary row (the pre-read links Willison's original piece, not the supplementary). Do not re-add a forward-pointing copy; a threat class taught once in the module that teaches it does not owe a reminder in the module that uses it.
