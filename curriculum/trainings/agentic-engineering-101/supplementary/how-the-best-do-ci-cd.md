...
Sources
- klaassen-definitive-guide `[checked:2026-09-03 result:OK due:none]` https://every.to/source-code/compound-engineering-the-definitive-guide — [practitioner direct, vendor venue] Klaassen, 2026-02-09. Confirms the term and the make-the-next-one-cheaper philosophy. **Cite for the term, never for a step count** — the loop's step names live elsewhere and are not fixed. fallback: attribute ordinal-free.
- curran-2x `[checked:2026-05-25 result:CAVEAT due:2026-10-16]` https://ideas.fin.ai/p/2x-nine-months-later — [practitioner direct, vendor venue] Curran, 2026-04-16: 19.2% / 14.6 vs 75.8 min / 86% ≤20 lines. CAVEAT ×2: vendor-self-reported (body flags it as Intercom's own numbers), and the company renamed to Fin on 2026-05-12 (Salesforce acquisition since signed) — the memo predates the rename and says Intercom throughout, accurate for April 2026; do not silently swap the name (record: kb:observations/intercom.md). fallback: keep the numbers, attribute the telemetry, flag self-report.
...
- cursor-bugbot `[checked:2026-08-01 result:OK due:2026-10-08]` https://cursor.com/blog/bugbot-learning — [vendor blog — Level 0] **Verified live 2026-08-01.** Michael Zhao, 2026-04-08. All three figures hold verbatim: *"When we launched Bugbot out of beta in July 2025, 52% of the bugs it identified were resolved"*, *"Today, the resolution rate is nearing 80%"*, and *"more than 110,000 repos have enabled learning, generating more than 44,000 learned rules."* **Still L0 and outside the ladder** — verified does not mean promoted, and these are the vendor's own product metrics. The page also carries a competitor comparison table (78.13% across 50,310 PRs versus Greptile's 63.49%) which is marketing and must not be imported. fallback: cut the numbers, keep the mechanism (useful review comments becoming promoted rules), which is the transferable part and needs no telemetry.
- cloudflare-ai-review `[checked:2026-08-01 result:OK due:2026-10-20]` https://blog.cloudflare.com/ai-code-review/ — [vendor blog — Level 0] **Verified live 2026-08-01.** Ryan Skidmore, 2026-04-20. Every figure holds: 131,246 review runs, 48,095 merge requests, 5,169 repositories, median 3m39s, and the override rate stated as *"engineers have only needed to 'break glass' 288 times (0.6% of merge requests)"*. The 30-day window is 2026-03-10 to 2026-04-09; body says "30 days" without dating it, which is fine for a supplementary and would not be for a claim about now. **Still L0** — the vendor's own instrumentation of its own stack. fallback: keep "an org runs a fan-out of specialist reviewers with a coordinator", drop every figure.
- stripe-minions `[checked:2026-09-03 result:OK due:2026-08-09]` https://stripe.dev/blog/minions-stripes-one-shot-end-to-end-coding-agents — [vendor blog — Level 0] Alistair Gray, 2026-02-09; due = pub+6mo. Verbatim: *"Over a thousand pull requests merged each week at Stripe are completely minion-produced"*, *"while they're human-reviewed, they contain no human-written code"*. Isolation = devboxes *"isolated from production resources and the internet"*. fallback: cut the count, keep the shape.
- taylor-fintechbrainfood `[checked:2026-08-30 result:OK due:2026-10-19]` https://www.fintechbrainfood.com/p/ramp-cracked-ai — [practitioner analysis] Simon Taylor, *Ramp Cracked Enterprise AI. Here's The Playbook*, 2026-04-19. Analysis of Geoff Charles's April 2026 X thread, not Charles's own words — same caveat as cherny-via-jadhav. Carries the Dojo/marketplace shape verbatim ("Ramp then built a marketplace of over 350 reusable skills"; "anyone in the company can now use AI as well as the best person"), independently corroborated by Patrick Wheeler, Tuck Center for Digital Strategies, 2026-04-22 ("350+ shared workflows"). The primary (x.com/geoffintech/status/2042002590758572377, snowflake-decodes to 2026-04-08) is 402 direct and via oEmbed as of 2026-08-30; the search index reaches the thread's headline and status URL only, and agents.ramp.com/skills is Ramp's external product-skills catalog, not Dojo (checked 2026-08-30). The body cites the marketplace shape only — the 350+ figure stays out of body as vendor self-report. Do not cite the Yang/creatoreconomy interview: verified 2026-08-29 that it never mentions Dojo, a marketplace, or skill-sharing. Due = Taylor's publication (2026-04-19)+6mo — the reachable source this stamp records; the unreachable primary is a standing re-check condition on this line, not a due (`asap` is reserved for blocking results). Re-check the primary when the X thread becomes reachable. fallback: drop the Dojo sentence and let Larson's skill-catalogue point carry §5 alone.
- majors-charity-wtf `[checked:2026-08-29 result:OK due:none]` https://charity.wtf/p/your-data-is-made-powerful-by-context — [practitioner direct] Majors, 2026-03-09, confirmed by direct fetch: "How do you expect your agents to validate each change, if the consequences of each change cannot be found?" — near-verbatim match for the paraphrase body carries. Durable argument essay, not a decaying metric — due:none. fallback: the paraphrase carries the point; quotation marks may be restored against this live URL if ever wanted.
...
- holds: the shape and the figures the stamps above carry. The shape (CI as feedback, fan-out review, tiering by blast radius, guardrails versus evaluators) survives every figure being cut.
- contested: the evidence *level*, not the figures. The vendor-blog sources report their own product metrics: **L0, outside the ladder, and verifying one does not promote it.** Body flags each as the org's own numbers, which is the correct treatment.
- decided: **a swapped URL is a new citation and owes a fresh check** — the swap is the moment a claim silently loses its source. Ramp's figures stay out of body until Charles's own April 2026 X thread is reachable; routes tried and what each returns are on the taylor-fintechbrainfood stamp.
...
OODA
- question: is Charles's Ramp thread reachable again by any route?
- roster: Cursor blog, Cloudflare engineering blog, Stripe dev blog, Geoff Charles, Charity Majors, Adam Wolff, Darragh Curran
- last-run: 2026-08-01

<!-- /backing -->
**Org-diversity check:** Anthropic is the only organisation cited twice (Cherny, move 4, secondary; Wolff, move 6, secondary) — accepted: distinct practitioners, distinct claims about different layers of the loop. `check_writing.md` §11's cap is per practitioner; each is one mention.
