...
- judges @324b81d7: writing PASS (2 findings see instances/ae101--supplementary--workflow-composition-lineages.writing.json), story PASS, technical PASS (2 findings see instances/ae101--supplementary--workflow-composition-lineages.technical.json), behavior N/A (no prompt blocks on this page), pedagogy PASS, strategy PASS, slides PASS (1 finding see instances/ae101--supplementary--workflow-composition-lineages.slides.json)
...
**Supplementary meta:** *Reading material beside AE101 M6. Pairs with `lectures/composing-the-workflow.md`, which carries the pointer, not the walk. One named framework (Klaassen), shapes (Pocock / Yan-Cognition / Metcalf-Amp / Cherny-moves), one counter-position (Ronacher), one worked-example case study (Dino, AE101 trainer); closes on filters for picking what to study first. Intro states the scope: partial by design.*
...
- amp-feedback-loopable `[checked:2026-05-25 result:OK due:2026-08-05]` https://ampcode.com/notes/feedback-loopable — [practitioner direct, vendor venue] Metcalf, 2026-02-05 (due = pub+6mo): three components plus *"making it feedback loopable"* verbatim. fallback: vendor-venue methodology — treat as practitioner-coined, never as industry standard.
...
- amp-neo `[checked:2026-08-01 result:OK due:2026-11-06]` https://ampcode.com/news/neo — [practitioner direct, vendor venue] Amp, 2026-05-06: *"So handoff is out. Compaction is in."* · *"Handoff is gone. As described above, compaction made it obsolete."* Compaction now runs automatically at 90% fill — a current-mechanism claim, so the due is publication (2026-05-06)+6mo rather than the durable-account `due:none` the reversal event itself carries (`what-packaging-is.md` amp-neo). fallback: cite as one team's reversal, never as a trend.
...
- howborisusesclaudecode `[checked:2026-05-25 result:OK due:cohort]` https://howborisusesclaudecode.com/ — [practitioner analysis] due:cohort — a living page with no publication date; re-walk at delivery like the other living sources on this list. CarolinaCherry on Cherny, a fan-compiled aggregator. **Not Cherny-direct: use only as an aggregator, and never attribute its unifications to Cherny.** This is the source-laundering failure mode `check_research_claims.md §1` was written for, kept here deliberately labelled rather than dropped. fallback: use the thread above.
- ronacher-bottleneck `[checked:2026-08-31 result:OK due:2026-08-13]` https://lucumr.pocoo.org/2026/2/13/the-final-bottleneck/ — [practitioner direct] re-confirmed on page 2026-08-31; due stays pub+6mo, now past — the counter-position section cites him live, so the overdue is a real stance-re-hunt owing (lecture-ooda), not a re-stamp. Ronacher, 2026-02-13, quote verbatim. fallback: none needed.
- ronacher-pi `[checked:2026-08-31 result:OK due:2026-07-31]` https://lucumr.pocoo.org/2026/1/31/pi/ — [practitioner direct] Ronacher, 2026-01-31: *"I throw skills away if I don't need them"* verbatim. Cited as live stance → overdue is a stance-re-hunt owing, as ronacher-bottleneck. fallback: none needed.
- ronacher-psychosis `[checked:2026-08-31 result:OK due:2026-07-18]` https://lucumr.pocoo.org/2026/1/18/agent-psychosis/ — [practitioner direct] Ronacher, 2026-01-18, paraphrase faithful (dopamine hit real / "massive slop machines if you turn off your brain" / review asymmetry). Cited as live stance → overdue is a stance-re-hunt owing, as ronacher-bottleneck. fallback: none needed.
...
- **Re-laundering.** Future OODA cycles must apply the fan-digest source-laundering check (`check_research_claims.md §1`) before citing any third-party-aggregated practitioner site.
