...
- `judges/story.md` — storytelling-class judge prompt template. Reads compendiums tagged `eval_classes: storytelling` (pedagogy / strategy_tie_in / lectures). Reads **Class A persona-reader** sim trace from `sim-cache/<training>--<slug>.persona.json`; regenerates per-phase slices if per-phase SHA stale.
...
- `judges/prompt-behavior.md` — behavior-class judge prompt template. Per-prompt risk against the 15-pattern catalog at `curriculum/evals/simulation-behavior.md`. Reads **Class B prompt-behavior** sim trace from `sim-cache/<training>--<slug>.behavior.json`; regenerates per-prompt slices if per-prompt SHA stale. Reasons over the *distribution* of Claude responses for files mechanical doesn't cover.
...
- `sim-cache/` — content-SHA-keyed JSON traces, the evidence PASS instances cite. A link into private `agents-102-core/evals/sim-cache/`, so traces are committed there (end-of-turn autocommit) and a fresh clone of both repos has them. One file per slug per class, prefixed by training short key: `<training>--<slug>.persona.json` (Class A) + `<training>--<slug>.behavior.json` (Class B). The prefix disambiguates same-slug files across trainings (`getting-going` exists in both Agents 101 and AE101).
...
- Subagents do NOT read CLAUDE.md. Use the rule-injection model: prepend `.claude/rules/content-rules.md` (or `continuous-research/research-rules.md` for research surfaces) plus the matching `memory/check_*.md` compendium verbatim. One edit to the rule file propagates to every future subagent; no copy-paste drift. Surface-specific extras (mood contracts for seam judges, Builder scoping for path-consistency, etc.) get inlined alongside the rule injection. See prior runs in `instances/` for prompt patterns.
