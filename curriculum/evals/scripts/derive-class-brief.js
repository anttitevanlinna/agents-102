...
function build(fileArg, cls) {
  ...
  const header = [
    ...
    '',
    '**A numbered rule that is not in this brief is already answered. Do not emit a row for it.**',
    'Author numbered-rule rows only for rules in this brief, plus every explicit judge-owned criterion in your class template.',
    'Those criteria are not compendium rules, so neither this brief nor the sidecar can carry them.',
