...
- `judges/prompt-behavior.md` — behavior-class judge prompt template. Sonnet 4.6. Per-prompt risk against the 15-pattern catalog at `.claude/skills/content-creation/simulation-behavior.md`. Reads **Class B prompt-behavior** sim trace from `sim-cache/<training>--<slug>.behavior.json`; regenerates per-prompt slices if per-prompt SHA stale. Reasons over the *distribution* of Claude responses for files mechanical doesn't cover.
