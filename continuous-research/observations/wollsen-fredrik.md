...
**Bottlenecks are organisational, not technical.** *"We are blocked a lot of the time… Almost none of the bottlenecks are technical. It's nearly always that somebody at some point decided a process has to work a certain way."* On adoption: *"most people go quiet. They don't tell us something is broken, they just stop using the thing and quietly find another way."* And *"the development speed moved by an order of magnitude and the expectations around it barely moved at all."* **L1.** This is **thematically consistent with, but adds no evidence to,** the convergence tracked at `synthesis/patterns.md` Pattern 47 (Access-Trust Gap, L4 meta-pattern) and Pattern 50 ("The Last Mile Problem," L2, June 2026 — patterns.md numbers two different patterns "50"; this is the organisational-stall one, not "The Productivity Paradox") — a single unaudited practitioner statement resonating with a pattern does not increment that pattern's count. What is genuinely new is the vantage point: **as far as this corpus currently shows**, every Nordic leg feeding those patterns is a survey house or an executive, and this is the first from inside the engineering function, first-hand. That composition claim is checkable against `synthesis/nordic-landscape.md`; it has not been re-counted this pass.
...
- [LinearB 2026 benchmarks](https://linearb.io/blog/engineering-metrics-benchmarks-what-makes-elite-teams) `[vendor press release]` — median **12.4** merged PRs/month, elite **>20**/month, over a stated 8.1M PRs / 4,813 teams / 163,820 contributors / 42 countries. **Re-fetched 2026-08-29: the live page no longer surfaces these figures** — it is now titled "2025 Software Engineering Benchmarks Report" (dated 2024-12-12) and states 6.1M+ PRs / 3,000+ teams / 167k+ contributors / 32 countries instead, with no extractable median/elite PR-per-month comparison. Either the cited figures are chart-only content a text fetch cannot read, or the body cites a superseded edition. Treat the 12.4 / >20 / 8.1M / 4,813 / 163,820 / 42 figures as **unconfirmed** pending a re-traceable source.
...
**Citation mismatch, correctable in place.** He attributes Anthropic's 8× to [anthropic.com/research/how-ai-is-transforming-work-at-anthropic](https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic) `[vendor case study]`, Level 0. This corpus's own trace (`findings/by-pattern/productivity-gains-2026-06-25/`) puts the 8× in *When AI builds itself* (2026-06-04), and the post he links carries the cleaner **67% increase in merged PRs per engineer per day**. Anthropic itself calls 8× *"almost certainly an overstatement of the true productivity gain."* **`[checked:2026-08-29 result:OK due:none]`** — re-fetched 2026-08-29: the linked post contains only the 67% merged-PRs-per-engineer-per-day figure (quoting Pragmatic Engineer's corroboration), no 8×; confirmed via `productivity-gains-2026-06-25/a-anthropic-8x-artifact.md` that the 8× and its "almost certainly an overstatement" qualifier both originate in the separate Anthropic Institute post *When AI builds itself* (2026-06-04). The citation-mismatch description holds. `due:none` — durable fact about a specific document's fixed contents; revisit only if the page is edited.
...
**Kieran Klaassen**, [every.to](https://every.to/source-code/the-folder-is-the-agent), 2026-04-13 `[practitioner direct, vendor venue]` — *"more agents didn't make me faster."* **L1.** Date and URL independently verified 2026-08-29 (previously inherited from the `coding-engineering.md` citation).

## Corroborating pattern from vendor data — weaker, and not peer to the above

**Faros AI: +98% PRs merged per developer with org DORA flat**, [faros.ai](https://www.faros.ai/blog/ai-software-engineering) `[vendor press release]`, Level 0, **`[UNVERIFIED STAT]`** — round number; sample size and methodology ARE disclosed (10,000+ developers, 1,255 teams, Spearman rank correlation, p<0.05, per-company standardization — confirmed 2026-08-29), but no named organisation backs the aggregate dataset and it remains vendor self-report on its own telemetry: Level 0. Already logged in `findings/by-pattern/productivity-gains-2026-06-25/SYNTHESIS.md` and `findings/by-pattern/absorption-bottleneck.md`. It is the exact shape of the trap this file is about — PR count doubles, outcome does not move — which is why it is kept, and it is deliberately **not** filed with Osmani, Klaassen and Asay above: a vendor reporting on its own customers is not counter-evidence of the same standing as a named practitioner writing on his own venue.
...
**No review or verification process at volume.** Who reviews 490 PRs, how quality is verified, what broke, what fraction was sibling-fix rework. Rework is named as a category during his self-audit and never quantified. **A later post fills part of this gap**: [*Code review when four engineers merge 844 PRs a month*](https://agentherder.com/blog/code-review-at-844-prs-a-month) `[practitioner direct, commercial interest]`, 2026-07-27, states directional (not line-by-line) review, self-inflicted rework at **16%** of active-workstream PRs, only **2 reverts** across 844 merges, and a cross-check against measured codebase line growth. **L2** — same practitioner, same self-audit method as the June count; still entirely self-reported and unaudited, but no longer unquantified.
...
- `[checked:2026-08-29 result:OK due:2026-10-13]` https://every.to/source-code/the-folder-is-the-agent — `[practitioner direct, vendor venue]` opened directly 2026-08-29: publication date 2026-04-13 confirmed on-page (previously only inherited from `coding-engineering.md`); quote confirmed verbatim — "But more agents didn't make me faster... agents coordinating with each other, deciding what to work on, producing output I hadn't shaped." Leaves the 6-month window 2026-10-13.
- `[checked:2026-08-29 result:CAVEAT due:2026-09-29]` https://linearb.io/blog/engineering-metrics-benchmarks-what-makes-elite-teams — `[vendor press release]` fetched 2026-08-29 (three passes): live page is titled "2025 Software Engineering Benchmarks Report" (dated 2024-12-12), reporting 6.1M+ PRs / 3,000+ teams / 167k+ contributors / 32 countries — none of the body's cited figures (8.1M PRs / 4,813 teams / 163,820 contributors / 42 countries; median 12.4 / elite >20 merged PRs/month) were found in extractable page text. Either these are chart-only numbers a text fetch cannot read, or the body cites a superseded edition; if the 2024-12-12 date is accurate the source is also ~20 months outside the 6-month window. `due:2026-09-29` — chase the original figures with a rendering browser or drop them.
- `[checked:2026-08-29 result:OK due:none]` https://graphite.com/research/commit-frequency — `[vendor press release]` confirmed directly 2026-08-29: page states "the median active developer merges a median of 2 PRs a week" over a dataset spanning "1/2023-now." Confirms the body's multi-year-blend caveat exactly; stays historical context only, never a current comparator. `due:none` scopes to the **methodology fact** (the dataset's 1/2023-onward window, as the article states it) — the throughput number itself is historical context and must not be re-cited as current without a fresh dated check.
- `[checked:2026-08-29 result:CAVEAT due:none]` https://www.faros.ai/blog/ai-software-engineering — `[vendor press release]` fetched directly 2026-08-29: post dated 2025-07-23 states developers "merge 98% more pull requests" on high-adoption teams, from telemetry on "over 10,000 developers across 1,255 teams," via Spearman rank correlation (p<0.05), metrics standardized per company — sample size and methodology ARE disclosed, correcting the body's earlier "no methodology surfaced." Still Level 0 (vendor self-report on its own telemetry, no named org behind the aggregate). CAVEAT accepted, `due:none`: outside the 6-month window permanently since 2026-01-23 — cite only as a dated vendor claim, never as current evidence.
- `[checked:2026-08-29 result:OK due:none]` https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic — `[vendor case study]` re-fetched directly 2026-08-29: post (2025-12-02) contains only the 67% merged-PRs-per-engineer-per-day figure, no 8×; the 8× and its "almost certainly an overstatement" qualifier confirmed to live in the separate *When AI builds itself* post (2026-06-04, anthropic.com/institute/recursive-self-improvement) per `productivity-gains-2026-06-25/a-anthropic-8x-artifact.md`. Citation-mismatch description holds. `due:none` — durable fact about this document's fixed contents; revisit only if the page is edited.
- `[checked:2026-08-29 result:OK due:2027-01-27]` https://agentherder.com/blog/code-review-at-844-prs-a-month — `[practitioner direct, commercial interest]` Wollsén, *Code review when four engineers merge 844 PRs a month* (2026-07-27): directional review, 16% self-inflicted rework, 2 reverts across 844 merges — the review-process account the body's "later post fills part of this gap" note cites. Confirmed directly 2026-08-29. Self-report, not independent verification.
...
- `[checked:2026-08-29 result:OK due:2026-09-29]` — critique or replication of the June-2026 count. Re-searched 2026-08-29 (web): still no third-party critique or replication. New first-party material surfaced instead — Wollsén's own agentherder.com review-process post (stamped above), self-report that does not close this item. `due:2026-09-29` — recheck cadence held at ~1 month.
- `[checked:2026-08-29 result:CAVEAT due:none]` — `synthesis/patterns.md` Pattern 47 (Access-Trust Gap, Level 4 meta-pattern, durable structural finding) confirmed present 2026-08-29; Pattern 50 is ambiguous — patterns.md's own housekeeping note flags two patterns sharing the number "50", and the content cited here matches "The Last Mile Problem" (June 2026), whose HBR leg exits its window 2026-09-09, before the 2026-09-30 cohort date. `findings/by-pattern/absorption-bottleneck.md` carries `updated: 2026-08-01` (fresh). `due:none` — durable citation-currency snapshot; recheck only if patterns.md renumbers or absorption-bottleneck.md's date moves. The duplicate-numbering defect itself lives in patterns.md, out of this file's scope.
