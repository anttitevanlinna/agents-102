...
|---|---|---|---|---|---|---|
| Stack Overflow Developer Survey | 2025 ed. (data 2025, published Dec 2025) | 84% use or plan to use; 51% of pros use daily | **~31% use AI agents at some cadence** (14.1% daily + 9% weekly + 7.8% monthly); 13.8% use copilot/autocomplete mode *exclusively*; ~~agent usage **doubled** vs prior survey~~ **[UNVERIFIED — struck 2026-08-01, see §(g)]** | ~49,000 total; 26,004 pro devs | [survey.stackoverflow.co/2025/ai](https://survey.stackoverflow.co/2025/ai/) | [survey, commercial venue] (SO is vendor-run; large + reputable) |
...
1. **Stack Overflow's own daily-use cut is the cleanest survey split.** Among developers, AI-agent use breaks out as 14.1% daily / 9% weekly / 7.8% monthly (~31% at some cadence), against 13.8% who use plain copilot/autocomplete mode *exclusively*. So genuine agent use is already *larger than* autocomplete-only use. (~~and SO reports **agent usage doubled** vs the prior edition~~ — **struck 2026-08-01, unverifiable; see §(g)**.) The mainstream of AI-using developers, though, still sits in assistant/chat/autocomplete territory — the agent-daily cohort is one in seven. [survey, commercial venue] — [survey.stackoverflow.co/2025/ai](https://survey.stackoverflow.co/2025/ai/)
...
**Convergent estimate of the split (with uncertainty):** Of developers "using AI" in mid-2026, roughly **30–40% are using a genuine agentic coding tool** (multi-step, tool-using, runs the loop) at some cadence; the remaining ~60–70% are still primarily in chat/assistant/autocomplete mode. The *trajectory* is toward agentic (Claude Code 6x, Cursor 2.5:1→1:2, Anthropic GitHub-repo agent activity >2x — note all three are **vendor telemetry**, L0–L1; the SO "doubled" leg that used to head this list was struck 2026-08-01, see §(g), and "unambiguously" went with it), but the *current stock* is still assistant-majority. Uncertainty is wide: "use an agent" conflates tried-once with daily-driver, and the vendor telemetry (Cursor, Anthropic, GitHub) over-weights agent-forward populations vs the median enterprise developer the DORA/JetBrains panels capture. The honest framing: **software is the sector where agentic genuinely escapes chat — but even here, agentic is the fast-rising challenger, not yet the incumbent.**
...
- **Newer editions exist and shift the headline:**
  - **Stack Overflow 2025** (data 2025, pub Dec 2025): 84% use/plan AI; 51% of pros daily; ~~**agent use doubled**~~ **[struck 2026-08-01, see §(g)]**; trust collapsed (only 3% "highly trust" AI code; 46% distrust). Note: the **Stack Overflow 2026 survey opened June 23, 2026** (human-developers-only) — *fielding now, no results yet*; the "2025" edition remains the latest published.
...
**Recommended KB update:** ~~replace the single bundled line with → *"~84–90% of developers use AI tools (DORA 2025: 90%; SO 2025: 84%); ~30–35% use a genuine coding agent at some cadence (SO 2025 agent-cadence split; JetBrains Jan-2026 per-tool shares); agentic use doubling YoY but still minority of AI use."*~~ **Superseded 2026-08-01 — this wording was applied to the KB and three of its four clauses then failed re-verification against the primaries.** The doubling clause is unverifiable (§(g)); "~84–90% of developers use AI tools" misstates both legs (SO's 84% is *use-or-plan-to-use*, DORA's 90% is *technology professionals*, not developers); and the ~30–35% band blends two instruments. Only the SO cadence split (~31%) survives verbatim. Current wording → `findings/by-domain/coding-engineering.md` § Level 3, which carries the corrected version. **Lesson for this file's genre: a "Recommended KB update" is a draft, not a finding, and shipping one unverified is how a trace launders itself into the knowledge base.**
...
- **Evidence-ladder classifier:** Whole-file = **L3 convergence** — five independent survey/telemetry sources agree on direction (agentic rising, doubling YoY) and bracket the magnitude (~30–40%). Individual vendor-telemetry items (Cursor reversal, GitHub first-week, Anthropic hours) flagged L0–L1 and used only as corroborating directional signal, not as the load-bearing claim. PASS.
  - **Addendum 2026-08-01 (second correction): "doubling YoY" is struck** — it was one of the two legs this verdict rested on, and it does not exist on the cited page. See §(g). A convergence verdict resting on a direction claim that is itself unsourced is weaker than the PASS above records.
  - **Addendum 2026-08-01 (record left standing, verdict narrowed):** when this file's recommendation was applied to `findings/by-domain/coding-engineering.md`, a re-audit downgraded the surviving KB bullet to **L2**. The distinction the original verdict blurred: survey *instruments* agreeing is not the 10–20 independent *practitioners* the ladder requires for convergence. The direction (agentic rising) is well corroborated; the **magnitude is a convergent estimate assembled across instruments**, and the JetBrains per-tool shares are non-summable (one developer can report Cursor + Claude Code + Codex, so 18+18+3 is an overlap artifact, not a population share — this file already says so at (f)/5, which the L3 verdict did not carry through). Cite the KB bullet at L2.

## (g) Correction 2026-08-01 — the "agent usage doubled" claim is struck

**What was claimed, in four places in this file:** that Stack Overflow's 2025 survey reports agent usage *doubled* against its prior edition. It was carried into the KB as a headline trajectory claim and survived a four-persona audit at §(e).

**What the page says:** nothing of the kind. `survey.stackoverflow.co/2025/ai/` was fetched whole on 2026-08-01 — ~985 KB, server-rendered, 241 occurrences of "agent" — and the string `doubl*` **does not occur anywhere in it**. This is a real absence, not a JS-rendering or fetch artifact. The only year-over-year comparison SO publishes on that page is for general AI-tool use: 76% → 84%.

**What does verify, verbatim, on the same fetch:** the cadence split. *"Yes, I use AI agents at work daily 14.1% / weekly 9% / monthly or infrequently 7.8%"* — and materially, *"No, but I plan to"* (17.4%) is a **separate answer option on the same question**, so the ~31% is genuinely current use with intent excluded. That figure is sound and stays.

**Also corrected on the same fetch:** SO's 84% is *"using **or planning to use** AI tools in their development process"* — current-plus-intent, not a usage rate. This file said "84% use/plan AI" at §(c), which was right, and the KB line it recommended said "84–90% of developers use AI tools", which was not. The trace was more careful than the summary built from it.

**Why it matters more than one struck number.** The claim was unsourced from the start and propagated *upward* in confidence: trace → recommended KB update → KB headline → a correction block whose own subject was killing an unsourced stat. At the last hop it acquired a hedge — *"directional, not a computable rate"* — which read as rigour while making it no more traceable. `check_research_claims.md §14(c)`: the audit at §(e) confirmed the annotation existed; nobody opened the page and searched for the word.

**Standing instruction:** do not restore any doubling/trajectory claim attributed to Stack Overflow without an on-page quote. If SO's agent-cadence question has no prior-edition counterpart — and the 2025 edition appears to be the first to ask it — then no year-over-year agent comparison from SO is possible at all, which would make this claim not merely unsourced but unsourceable.
