...
- `[checked:2026-07-30 result:OK due:2026-11-23]` https://www.lucasfcosta.com/blog/backpressure-is-all-you-need — [practitioner direct] (Lucas F. da Costa, 2026-05-23; due is publication+6mo). Byline, date, and the claim that a human error-catching stage limits the system were verified against the original. The `## Pre-read before Module 4` entry paraphrases that limit. **This is the module that owns the check** — `run-the-first-experiment.md` echoes the same URL in its `## Prework` and delegates here. fallback: drop the pre-read; M4's closing lecture still names the term from the student's live session.
