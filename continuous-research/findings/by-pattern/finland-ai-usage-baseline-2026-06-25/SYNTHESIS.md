...
- **Refresh `coding-engineering.md`'s global dev line** per `i-`: ~~replace the dated "85% use AI / 62% use a coding agent" with "~84–90% use AI; ~30–35% use a genuine coding agent at some cadence; agentic doubling YoY but still minority."~~ **DONE 2026-08-01, but this recommended wording did not survive re-verification and must not be reused.** The "doubling YoY" clause is struck as unverifiable (`i-developer-adoption-depth.md` §(g) — the word does not occur on the SO page), and "~84–90% use AI" misstates both legs (SO's 84% is *use-or-plan-to-use*; DORA's 90% is *technology professionals*, not developers). Only the SO cadence split (~31%) verified verbatim. Live wording → `findings/by-domain/coding-engineering.md` § Level 3.
