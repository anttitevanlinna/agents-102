...
**The §52c sweep of the sixteen AE101 lead-ins is now IN the ledger** (`gate_triage.batch = wyb-52c-2026-09-06`), persisted 2026-09-08 from a session scratchpad that would not have survived. Eight want a rewrite, seven stand, a skeptic overturned three proposed rewrites. Two are suppressed with their reasons recorded on the row — `spot-gaps-build-the-loop` grows the line, `diagnose-and-resend` proposes *"in minutes, not hours"*, a quantified claim with no source. `read-your-stack` STANDS. Still open: `author-test-strategy-skill` alone — and **the sweep's proposed wording for it does not survive the tangibility boundary.** It trades `SKILL.md` in a named folder for *"a skill"*, the direction §52c bans, so the card owes a rewrite that keeps the filename. A SKILL.md is a container, so unlike the ADR row it does still owe the *for what*.

**The batch's own hit rate is the finding.** Of the eight rows the sweep marked for rewrite, two were applied close to as proposed, two were declined or withdrawn on tangibility, two were suppressed at persistence, and one — `threat-model-with-stride` — was applied as a **cut against the sweep's premise**: it had been filed for a missing enablement clause, but a *hardening decision* is not a bare artifact noun and never owed one. §52c now carves that out, so the same misfire cannot be filed again. The two that landed near-as-proposed share the one shape a reader can confirm cold: the build slot restating the do-slot.
