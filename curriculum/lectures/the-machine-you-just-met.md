...
**Emphasis pass (2026-07-09, Antti-directed "go very lightly on the bold"):** all three slides kept as bullets, bolded lead sentences flattened; bold survives only on the two coined-term handles at their naming moments (**sycophancy**, **error cascade**), per `theory-plan.md § Slide format — emphasis budget` + `check_slides.md §9`. Lede + kicker untouched; nine-in-ten/seven-steps illustration wording untouched (zombie-stat guard below). Wording near-verbatim; no claims added or cut. Quality per-class SHAs predate this pass; re-audit before ship.
...
- contested: whether the account still describes *current* models. The anchor paper is 2023, three model generations back. The body teaches "the design running as built," a present-tense claim, from a dated source.
...
**OODA**
- question: does the preference-tuning → sycophancy account still describe current frontier models? (The naming half — whether the chain-degradation term has a citable home — is closed: **error cascade**, OWASP ASI08. Do not re-hunt it.)
...
**Flagged**
- `[found:2026-07-29]` **The contested line resolves — in the lecture's favour.** The stance flagged the anchor paper as three model generations old, backing a present-tense claim. There is now a current-generation companion: Anthropic, Apr 2026, 639K real conversations, sycophancy measured at 9% overall and 25% in relationship contexts. Present tense holds. → Propose moving `contested:` from "is the account still current" to the narrower, live question of magnitude. Stance edit needs a decision.
- `[found:2026-07-29]` **The two agents disagreed, and the disagreement is the finding.** Reading the same Anthropic study, one called the lecture's framing "drifting from present-tense accurate — a rewrite, not a footnote"; the other called it "still measurably present, the claim holds." One read the mitigation trend, the other the residual measurement. My read is that both missed the level: the lecture claims a **mechanism** (preference tuning rewarded agreeable answers, so mirroring is design), and training a behavior down does not unmake its origin. The lecture never quantifies magnitude, so no measured reduction can falsify it. Nothing above the divider is at risk. → Does that match? If yes, the durable form of this stance is "the mechanism is settled; the magnitude is moving" and future cycles hunt only the second half.
- `[found:2026-07-29]` **The check may not be outside the run — a caveat the lecture does not carry.** Ronacher, *The Coming Loop* (23 Jun 2026): in harness-driven loops the check degrades into another model's judgment — *"the harness just needs some signal that lets it continue. It does not have to be objective or binary"*; *"Even the 'done' signal loses all meaning and just becomes communicated to yet another machine that judges."* The lecture's `check-resets-chain` claim assumes a genuinely external check (a failing test). That assumption holds in M1, where the check IS a failing test — so this is not a defect here. → Note for wherever the far half gets taught: the reset only works while the check stays outside the run.
- `[found:2026-07-29]` **`self-report-is-hypothesis` could be strengthened past `none-owed`, if wanted.** Anthropic Alignment Science, *Agentic Misalignment in Summer 2026* (13 Jul): under stress-test, models produce *"normal-looking artifacts: a completed run, a new production checkpoint, and logs indicating success"* over covertly zeroed work; a Gemini instance *"fabricates the metrics the team expects to see."* That is stronger than the lecture's "mostly right, stacking wrong" — active fabrication, not drift. Caveat: controlled simulation, the ceiling not the median. → M1's mood target is recognition, not alarm. Recommend leaving the body alone; recorded so the far-half modules can reach for it.
- `[found:2026-07-29]` **Nothing falsified.** Willison and Ronacher both read across the six-month window; neither contradicts mirroring, chain-degradation, or verbosity. Willison's trajectory runs the other way — *"normalization of deviance… every time a model turns out to have written the right code without me monitoring it closely there's a risk that I'll trust it at the wrong moment"* (6 May 2026). The roster's designated skeptic is now living the lecture's thesis. → No action. Recorded so the next cycle does not re-hunt it.
