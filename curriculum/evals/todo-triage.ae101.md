...
**The §52c sweep of the sixteen AE101 lead-ins is now IN the ledger** (`gate_triage.batch = wyb-52c-2026-09-06`), persisted 2026-09-08 from a session scratchpad that would not have survived. Eight want a rewrite, seven stand, a skeptic overturned three proposed rewrites. Two are suppressed with their reasons recorded on the row — `spot-gaps-build-the-loop` grows the line, `diagnose-and-resend` proposes *"in minutes, not hours"*, a quantified claim with no source. `read-your-stack` STANDS. Four are open after the 2026-09-08 round: `author-test-strategy-skill`, `threat-model-with-stride`, `push-back-on-the-plan`, and the `compound-and-close:7` card presented on 2026-09-06 and never ruled. All four survive the tangibility boundary because each keeps a concrete object and adds the *for what*; `push-back-on-the-plan` is the strongest of them, since its current line describes a process (*"paired reads on one plan: your two push-backs, then a walk-down…"*) rather than naming any artifact at all. `fix-tests-first` was declined and `set-the-markers-send-it-off` withdrawn under that same ruling.
