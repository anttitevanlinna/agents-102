...
| **workflow** | skills composed around one task; each skill carries the best instructions for its one step; which skills are in, and in what order, is composed per task and recomposed when the task changes (Antti 2026-09-05) — defined on the skills-to-workflow slide in `composing-the-workflow`; checks are triggers and instruments, not part of the definition | the concept is **skills-to-workflow composition**: variety first (called by hand / chained through files / authored wiring), one documented kit as specimen, never a closed field taxonomy | `composing-the-workflow` |
| **pilot** / **orchestrator** | one skill whose whole job is running the others in order, re-invoked until the task lands — carried by the slide's one clause (*Some workflows have a pilot; many do not*) and the Dino figure; no defining bullet owed after the 2026-09-05 composition frame | pilot = chart register, orchestrator = field register, bridged once; many workflows keep zero | `composing-the-workflow` |
| **leaf** | a skill that does one job and calls nothing — carried by the Dino figure; no defining bullet owed after the 2026-09-05 composition frame; body prose says *skill* | you invoke it by name, no chain | `composing-the-workflow` |
| **verifier** | the external check in the three-pattern | earns its full name at the M5 closer; `composing-the-workflow` § *Eval* earns *eval* from it; shapes menu under House coinages | `what-packaging-is` |
| **judge** (noun, and *judge* as the agent's verb) | the LLM-based check, sibling of the deterministic verifier and the CI-placed gate | **embargoed before the M5 closer** (Antti 2026-09-05). M5's three words are *judge*, *verifier*, *gate*; the split by check type is ours. Earlier modules describe an agent checking its work against something without naming the act *judging* — an M2-era bullet reads *shapes*, *proposes*, *works it into* rather than *judges*. Ordinary-English *judgement* (a person exercising taste) is unaffected; the ban is on the agent-evaluates-output sense that M5 defines | `what-packaging-is`, earned at the M5 closer |
...
| **the loop** / **compounding** | the compound loop (Klaassen lineage): taught in full once at the first module, varied after, never repeated as ceremony. **Not single-referent** — body prose may name a loop loosely at a beat that has not defined it (Antti 2026-09-10, on `fix-tests-first` *"The agent runs the loop; you read the result"*: *"Ok to name loop at that point without defining better"*). Do not file the M1 uses against the orient → fix → close → compound sequence | `check_student_facing.md §7`; `compound-and-close` |
...
| **eval** (the word) | the M6-earned name for the ranked check; definition under revision toward measurement-over-runs, not verdict-on-one (2026-09-02, `pre-cohort-todos.md`) | embargoed until `composing-the-workflow` § *Eval* earns it from the M5 verifier + ranked check-menu; don't pre-plant | `composing-the-workflow` |
| **backpressure** | downstream feedback that slows, stops, or redirects production when the next stage cannot safely accept more | earned once after the M4 send-off; later lectures prefer checks, gates, review bandwidth and reach | `what-keeps-a-long-running-session-going` |
