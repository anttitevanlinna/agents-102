...
- Homework: extract one micro-skill from your own reading of a company policy rule (e.g., customer-data classification, retention boundaries). Every student produces one skill before Module 5. Plus: run the two audit skills against the other agents built since M2 (they won't come back clean — unaudited agents stacking up feeds the deepened-unease mood, doesn't steal it). Plus: agent-sprawl reading (shadow agents, the 82%-think-protected / 24%-have-visibility pattern).
