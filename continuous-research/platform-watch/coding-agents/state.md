...
**That "token threshold" is not one fixed number — re-checked against the live docs 2026-08-01 (Claude Code v2.1.220).** Sonnet 5 on the API compacts "at about 967K tokens by default" of its 1M context window; Sonnet 4.6 and Opus 4.6 without extended context, plus cloud sessions, "compact at the 200K boundary by default"; other local sessions fire "when the conversation reaches the model's context limit." `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` (1–100) exists and can only lower the threshold, never raise it. There is no single percentage or fixed token buffer that applies across models and surfaces — ~~an earlier "~83.5% / 33K buffer" figure~~ **[SUPERSEDED — do not cite, 2026-08-01]**: the source is known and is the problem. It traces to https://claudefa.st/blog/guide/mechanics/context-buffer-management, a page carrying no byline and no date, and the figure does not match the per-model/per-surface picture above. Retired on the docs, not for want of a source. Do not go looking for a better citation for it. ([env-vars](https://code.claude.com/docs/en/env-vars), [model-config](https://code.claude.com/docs/en/model-config) — [vendor documentation])
...
- **Capability** (do these four mechanisms exist and behave as described): **confirmed.** Vendor documentation is L0 as *evidence* but authoritative for *capability*, and every quote above was fetched from the live page on 2026-08-01.
