...
- **Root cause is a doc convention, not a code key.** No code constructs the cache path; the judge subagent fills `{{trace_path}}` from its template. README:42 + `judges/story.md:147` + `judges/prompt-behavior.md:170` write `<slug>`/`<file-slug>`; instances + `eval-fire` SKILL.md use `ae101--<slug>`. FIX = tighten the convention to `<training>--<slug>` across those doc sites (approval-gated: SKILL.md + judge templates; README is infra-doc, ungated), then the bare orphans can be deleted (safe — gitignored, regenerate on fire). No code change. Adjacent doc-rot in the same edit: SKILL.md:150 names the path `<file-slug>.json` (missing `.persona`) and the dead megajudge / "Step 4/7 of the eval refactor" refs.
