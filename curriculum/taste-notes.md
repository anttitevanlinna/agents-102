...
The general shape: **a finding row ages well and its proposed cure ages badly.** The row points at a line that will still be there next week; the cure was derived against a rule whose text has since moved — §52c gained a boundary on 2026-09-06 and a carve-out on 2026-09-08, both after the sweep was written. Keep the row, re-derive the fix with the rule open in front of you. The tell that you skipped this: your AFTER matches the ledger's AFTER exactly, on a rule that has been amended since.

## 2026-09-08 — the slot that answers the slot beside it, and the noun that owes nothing

Four `What you build:` slots in one sitting, and the two cleanest defects were the same defect. `compound-and-close` said *"a rules file written from session evidence"* one line under a `What you do:` ending *"...write your `./CLAUDE.local.md` from session evidence"*, with the paragraph below closing on *"the session is the evidence"* — three tellings in eleven lines, and the build slot was the one not earning its go. `push-back-on-the-plan` said *"paired reads on one plan: your two push-backs, then a walk-down three questions at a time"* under an H1 reading *Sharpen the plan with two reads* and a do-slot reading *push back twice... run a second-pass grilling*. Same word count as a good line, no artifact in it at all. `author-test-strategy-skill` was subtler and worse: its line 9 was its own `What happened:` closer with the parenthetical removed, so the exercise opened and closed on one sentence.

None of these needs taste to catch. **A cold reader confirms all three in seconds without knowing what the exercise teaches** — which makes them the cheapest findings in a queue that mostly costs judgement. Worth separating from the expensive kind rather than draining in one undifferentiated list.

**The counterweight, and it is the entry that matters.** §52c's enablement clause — *name what the artifact ENABLES, not only the artifact* — was six days old and had already needed one boundary against abstraction. It needed a second on 2026-09-08: it does not fire at all on a slot whose noun already carries a judgment. *"One rules file"* owes the *for what* because a file is a container that any session can end with. *"One hardening decision"* owes nothing, because a decision is a call made under pressure and names its own job. A rule that fires on every slot of one shape will fire wrongly on the slots that were already doing the work, and the wrong fire looks exactly like the right one: correct rule, correct surface, real-sounding gap, and a cure that adds words to fix nothing.

## 2026-09-03 — four prompt-clause proposals, four refusals, and the shape they share

Every clause a rule asked to add to a prompt fence was declined, and the reasons are not interchangeable. §19's self-augment invitation on the M4 send-off died on **"M4 has deliberate room for mistakes and sub-par packaging"** — the strongest counterweight of the day, because the clause is genuinely good practice and already ships verbatim in `ae101-m5-rerun-packaged`. It fails *here* precisely because it works: an agent invited to fill its own gaps closes the gap M5 exists to open. A module whose job is to underdeliver visibly must refuse improvements that would make it deliver. §43's progress-streaming died on the same ground plus **"Logical suggestion but not enough value"**; the walk-away beat is the teaching, and a narrating agent buys the hovering the file already warns against.

The other two killed their rules' premises rather than the instances. §17's no-preamble: **"I see no reason the Claude would go outputting too much info."** §20's grill-before-save: **"there is no known correlation between grilling and team-readiness. Adds action without clear benefit."** Both rules paired a mechanically checkable trigger (4+ sub-steps; artefact-assembling prompt) with an empirical justification nobody had re-checked. The count never goes stale, so the rule fires forever at full confidence and its hits read as proof it is load-bearing. → `compounded/2026-09-03-prompts-a-rule-resting-on-a-model-behaviour-claim-expires-with-the-model.md`; §17, §20 and `check_student_facing.md` §31 all narrowed the same day.

## 2026-09-03 — "the wall", and the header that promised a beat the body never had

On M5's closing slide: **"wall → your biggest challenge."** The rename was refused on its own evidence — the figure on that same slide is labelled `↻ CROSSING THE WALL`, and two lectures use the term — so the framing landed *beside* the term instead: *"**The wall** is your biggest challenge from here. It is the line between what works for you and what works for someone else."* Two sentences, not one with an em-dash (§14). The lesson is cheap and general: **check what the picture says before renaming what the prose says.**

The trim on the same slide: **"start by cutting 'Everything built in these four sittings is on your side.' that's just fluff"** — correct, and it announces a list the reader is about to see anyway. Cutting it orphaned the list, which then re-attached as *"The rules file, the observations, the verifier, the reference, the plan. All of it on your side."* — meaning after the items, not before them, which reads better than the original.

And on `diagnose-and-resend`'s header, three passes to land one line. My proposal made Claude the actor twice over; Antti's counter kept the verb he wanted — *"I like the word 'dig' my grammar was just off a bit"* — and it settled at **`## Push for quotes, then make your agent dig deeper`**. Worth recording separately: the old header promised *show its work* **and** *dig*, while both bullets underneath do the same single thing, demand receipts. A header can carry a phantom beat for months and read fine. Cover the bullets and ask what is actually down there.

## 2026-09-03 — the singular claim, and the evidence that could not carry it

Promoting a bullet to a header surfaced an overclaim in the bullet: *"Skills are how agentic knowledge compounds across teammates."* Antti: **"Not so singular. Skills are one of the places. Not the only."** Then the stronger framing offered as a replacement — *skills are the practitioner-preferred current best practice* — was the one to refuse, and refusing it was the right call on the file's own sources: one company (Fin, self-reported, `CAVEAT`-stamped, cited twice by two authors at the same org, which the source note itself flags as not independent), one individual's MIT-licensed skill, and the vendor's docs. That is evidence skills work somewhere, not evidence of what the field prefers. Header shipped as `## A skill outlives the thread it came from` — vision-grade, no backing owed, non-singular. **A convergence claim needs more than a good hunch and a loud example; the claim row stays `vision · none-owed` until a real hunt says otherwise.**
