...
- `judges/story.md` — storytelling-class judge prompt template. Sonnet 4.6. Reads compendiums tagged `eval_classes: storytelling` (pedagogy / strategy_tie_in / lectures). Reads **Class A persona-reader** sim trace from `sim-cache/<training>--<slug>.persona.json`; regenerates per-phase slices if per-phase SHA stale.
