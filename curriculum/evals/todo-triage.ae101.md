...
One is held back. `klaassen-definitive-guide` on `push-back-on-the-plan.md` was ruled a theory-construct `due:none`, citing the sibling stamp at `learn-from-the-test.md:256`. That citation is accurate, but the corpus is split: five bodies carry `due:2026-08-09` (compound-and-close, run-the-first-experiment, push-back-on-the-plan, test-and-learn, the-machine-you-just-met) and two carry `due:none` (getting-going, learn-from-the-test). Applying the edit alone moves the count to 3-4 and leaves the contradiction standing in a new place. Every one of the seven stamps says in its own words to cite this URL for the term and never for a step count, which is the theory-construct shape — so the evidence points at `due:none` everywhere and the count points the other way. That is a seven-file convention call. `check_platform_and_boundaries §52` procedure: count first, then move the losing side, once.
...
- **`klaassen-definitive-guide` due-date convention**, split 5 to 2 across seven bodies. See *Applied* above.
