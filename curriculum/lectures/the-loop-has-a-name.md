...
OODA
- question: do practitioners actually use *judge*, *verifier* and *gate* with these three distinctions?
- roster: Hamel Husain, Shreya Shankar, Eugene Yan, Simon Willison, the Anthropic eval cookbook, Braintrust and LangSmith docs for vendor usage
- last-run: 2026-08-02
...
**Placement:** M6 deck order (2026-09-02, three-beat re-cut, slides moved and re-tiered, body text untouched pending cards): the-2-frontiers → exercise `spot-gaps-build-the-loop` (diff + rule cut) → the-loop-has-a-name → composing-the-workflow (opens on the control-loop slide) → exercise `read-your-stack` → the-handoff-prompt (titled *Agents that build agents*) → story-of-module-6 → the-map-filled-in → quality-is-grounding → Human close → agents-that-build-agents (titled *There is no last turn*, last). The survives-the-model slide and its flywheel kicker moved to `agents-that-build-agents.md`; the cadence slide moved to `composing-the-workflow.md` as *Loop instead of you starting* (Antti 2026-09-02); the placement slide was cut 2026-09-02 (Antti); the lecture is the one naming slide.
