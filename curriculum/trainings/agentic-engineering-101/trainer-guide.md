...
- **[Composing the workflow](lectures/composing-the-workflow.md)**, closer-area teacher. Read after Human close, before The loop has a name. ~3–4 min. Teaches what a workflow-of-skills IS, concretely: it re-labels the M5 passage chart with the student's own kit (session-shaper at the pier, test-strategy across a leg, verifier at a fix) so a skill's footprint follows its job, then names the four composition mechanisms (explicit load, orchestrator, `./CLAUDE.md` routing, hand-off) with the load-bearing caveat that not every workflow has an orchestrator. Keeps the practitioner-mode shift (*"there is no prompt to drill; the move is to read"*), then hands off to two reads: Dino's worked [skill-stacking system](supplementary/skill-stacking.md) (primary) and the [workflow-composition-lineages supplementary](supplementary/workflow-composition-lineages.md) (the field survey). The five-approaches walk itself now lives ONLY in that supplementary, not the lecture body.
- **[The loop has a name](lectures/the-loop-has-a-name.md)**, closer; names *eval* from the just-built second skill.
- **[The map, filled in](lectures/the-map-filled-in.md)**, penultimate closer. The consolidation beat: re-shows the whole Field Map filled from what the six modules built (near half shapes, far half corrects, verification is the sensor), the one jargon-free naming of the control loop the student drew.
...
**Pacing the close.** Lecture density means reading the room sharply. If energy is high, give the lineage conversation room. If energy is low, keep the lecture to the chart and the four mechanisms, point at the supplementary for the lineages, move to the closer pair faster. The supplementary carries the field survey; the lecture body teaches the mechanics and points.

**Composition is a reading move at M6.** If a student asks where the composition exercise is, point them at the [workflow-composition-lineages supplementary](supplementary/workflow-composition-lineages.md). The lecture teaches the composition mechanics on the chart; the supplementary carries the field lineages and the depth. *Your intuition* is the close of composing-the-workflow and the register the section asks the room to land in.
