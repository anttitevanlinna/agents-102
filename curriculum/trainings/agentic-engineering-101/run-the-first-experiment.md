...
- `[checked:2026-09-05 result:OK due:2026-08-09]` https://every.to/source-code/compound-engineering-the-definitive-guide — [practitioner direct, vendor venue] Klaassen Definitive Guide (pub 2026-02-09; due = pub+6mo, so the lapse is structural — a publication-anchored due does not move on re-reading, `check_research_claims.md` §11a). Re-opened 2026-09-05: page live and un-paywalled, byline and date unchanged, anchor thesis verbatim — *"each unit of engineering work should make subsequent units easier—not harder."* Explicit plan/work/review/compound naming is convergent-across-appearances, not verbatim-on-page — an anchor-scope guard, not a source defect. fallback: phrase as "the loop that runs through his work" — never a step count; the loop expanded 2026-05-29 and a count is what dated last time (strategy doc Woven rule #1). Treat any Every.to outcome metric as vendor-self-reported.
