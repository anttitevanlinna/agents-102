...
**STATUS:** slide-format reference page, proper-length bullets per `theory-plan.md` § Slide format.

**Proper-length pass (2026-07-02):** sections frame · mechanism · different-attacker · the move-as-governor, kicker *"Three legs. Cut one."* The different-attacker bullet is delivery-order neutral ("an access-surface map and a STRIDE pass on the feature") — keep it so; it must not assert both audits already ran when the security module is cut or reordered.

**Placement:** supplementary on the training index (`earn-the-trust` owns the security beat — surface map + STRIDE; this page extends the same muscle to the agent itself; M3 links Willison's original piece, not this page). **No forward-pointing firing exists, and none is owed (Antti, 2026-08-12):** a threat class taught once, in the module that teaches it, does not owe a reminder in the module that uses it — no pre-leash check in `earn-the-trust.md ## Next`, no copy at M4's send-off; decision note in `earn-the-trust.md`'s maintainer block. **The trifecta is not a gate on M4 and must not be described as one.**
...
Sources
- willison-trifecta `[checked:2026-07-02 result:OK due:none]` https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/ — [practitioner direct] Simon Willison, 2025-06-16. Byline, date, the three legs, and the quoted constraint sentence all verified verbatim. Dated framework piece, now standard field vocabulary by name → **`due:none`, durable-account variant**. **This is the rare case where "standard field vocabulary" is defensible rather than the convergence-verb trap:** the term is used by name, by other people, in public, and the corpus can point at that usage rather than asserting it. **Willison's guardrail-products point — that vendors claiming ~95% detection are inadequate for security — is deliberately carried in body WITHOUT the number** ("a filter that mostly works is a gate that sometimes opens"). Keep it number-free: a percentage invites the reader to treat 95% as a passing grade, which is the exact error the sentence exists to prevent. fallback: teach the three-leg frame unattributed as a field observation.
