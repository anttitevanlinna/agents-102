...
**The 3LO admin-gating clause in § *Jira: Atlassian Rovo MCP* carries no stamp of its own and is owed a live probe.** The clause — once the org's first 3LO consent is complete and the domain is allowed, individual engineers don't need per-install admin approval, admins controlling product scopes and which domains can connect rather than whether individuals can install — is a doc-derived NEGATIVE about an install affordance, which `check_platform_and_boundaries.md` §5a names its highest-risk shape. The Atlassian stamp in `Source verification` anchors the endpoint and the covered-product list only, so a cohort re-verifier can tick it OK without re-reading this sentence. Confirming it needs an Atlassian org plus admin rights, so it fails §5a's under-five-minutes test and takes §5a's stated remedy instead of a stamp nobody ran: **limitation assumed, live-probe owed**. Probe at the next cohort re-verify, from a tenant whose first consent is already done — have a non-admin `claude mcp add` the `authv2` endpoint, run `/mcp`, and record whether consent completes or bounces to an admin request; then stamp what comes back and drop this note. A technical judge re-filing §5a against this clause should read the note as the open work item rather than the section as adjudicated. Scoped to this clause: the Claude.ai-connector *admin-only on Team and Enterprise* lines are separately doc-confirmed and not in question.
