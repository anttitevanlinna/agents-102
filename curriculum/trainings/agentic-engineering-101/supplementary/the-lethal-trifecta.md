...
**STATUS:** slide-format reference page, proper-length bullets per `theory-plan.md` § Slide format. Audited — see the Quality line below. Reached from the training index's supplementary row only; M3's body and pre-read link Willison's original piece, not this page, and no forward-pointing copy exists anywhere.

**Proper-length pass (2026-07-02, unaudited):** the page holds four slide-thick sections in the reference-page shape — frame · mechanism · different-attacker · the move-as-governor — and closes on the kicker *"Three legs. Cut one."* The three legs read as one inline enumeration inside the frame bullet. The different-attacker section's wording is delivery-order neutral ("an access-surface map and a STRIDE pass on the feature"), so it does not assert that both audits already ran when the security module is cut or reordered. The legs-in-ordinary-wiring and accident-vs-adversary bullets derive from the trifecta's own mechanism plus `theory-audit.md`'s adversary-model note (composes with blast-radius) and carry no new sourced claims. Willison's constraint sentence is quoted verbatim and the filter line stays number-free.

**Placement:** supplementary on the training index (`earn-the-trust` owns the security beat — surface map + STRIDE; this page extends the same muscle to the agent itself; M3 links Willison's original piece, not this page). **No forward-pointing firing exists, and none is owed (Antti, 2026-08-12).** The rule: a threat class taught once, in the module that teaches it, does not owe a reminder in the module that uses it — so neither a pre-leash three-question check in `earn-the-trust.md ## Next` nor a second copy at M4's send-off is to be re-added. The decision note lives in `earn-the-trust.md`'s maintainer block. **The trifecta is not a gate on M4 and must not be described as one** — no *mandatory beat before the send-off*, no *fires before M4 grants the long run*. Do not re-add a copy to M4 on the strength of a sentence found on this page.
