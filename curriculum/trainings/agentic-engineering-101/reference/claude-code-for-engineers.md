...
<!--flag:module:spot-gaps-build-the-loop-->- **M6 Spot gaps, build the loop:** §§ 17–19 (the *Loop it, instead of you starting it* slide: `/loop`, Desktop local tasks, `/schedule`), 29 (compare two session transcripts)
...
**Doc verification state:** the header stamp carries the date; the page's contract is *docs win* — a live doc contradicting the page corrects the page without a maintainer call. Standing watch-items: `CLAUDE_CODE_SESSION_ID` is confirmed live (`env` inside the Bash tool) but absent from the env-vars doc — an undocumented surface the page depends on. Skills precedence (personal beats project, enterprise beats both): a stale `~/.claude/skills/` entry silently shadows a committed team skill — the M3 `test-strategy` collision mechanism.
