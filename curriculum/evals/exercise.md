...
This file is a manifest, not a megajudge template. The three class judges (writing / story / technical) each evaluate exercises against their compendium-class rules; this manifest names which judges run and at what scope.

For the per-judge prompt templates, see `curriculum/evals/judges/{writing,story,technical}.md`. To dispatch a single class, use `/eval-fire <class> <file>`. For the full ship-time audit (all three classes in parallel + Quality-state + neighbour-alignment), use `/curriculum-pre-ship-audit <file>`.
...
- All three class judges PASS (no blocking REVISE) + Quality check PASS → **APPROVE**.
- All three PASS but some non-blocking REVISE rules → **APPROVE WITH TODOs** (TODOs logged in maintainer block).
