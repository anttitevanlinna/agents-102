...
- `husain-ninety-percent-three-iterations` · detail · "better than 90% agreement after three iterations of exactly this loop" ← husain-llm-judge — anchor is the invariant fragment, not the whole sentence: the attribution clause in front of it carries §2a's body-date and will be re-tensed again, and an anchor that moves with it breaks the join silently every time.
...
Sources
- husain-llm-judge `[checked:2026-09-08 result:OK due:none]` https://hamel.dev/blog/posts/llm-judge/ — [practitioner direct] **published 2024-10-29**, verified live 2026-09-08. Durable (§2a): framework origin (calibration loop → law:eval-judge-verifier-gate, Frameworks row) + first-hand account of one product's judge. Hamel Husain (first-person byline confirmed), judge validated against a named domain expert's labels on Honeycomb's Query Assistant, agreement tracked; verbatim *"It took us only three iterations to achieve > 90% agreement between the LLM and Phillip"*, and *"You cannot write a good judge prompt until you've seen the data."* Anchors the calibrate-the-judge slide including the 90%/three-iterations number. **§2a's third condition is paid in the body, not here** — the sentence carries *"In 2024, on one product's judge"*, and a re-tensing that drops either the year or the n=1 scope puts the stamp back out of compliance whatever this line says. fallback: drop the number, keep the move — measure agreement against your own labels, iterate until you converge.
- husain-field-guide `[checked:2026-07-02 result:OK due:none]` https://hamel.dev/blog/posts/field-guide/ — [practitioner direct] Theory-construct variant (Antti 2026-08-31): error-analysis-first cited by name as Husain's discipline, acceptance demonstrable across the eval literature; any claim about current field practice owes its own dated backing. Hamel Husain: *"Error analysis - the single most valuable activity in AI development and consistently the highest-ROI activity"*; bottom-up from actual data versus top-down assumed metrics. Anchors the error-analysis-first bullet. fallback: drop the name, teach as the eval-building discipline practitioners converge on.
- husain-evals `[checked:2026-07-02 result:CAVEAT due:none]` https://hamel.dev/blog/posts/evals/ — [practitioner direct] Theory-construct variant (Antti 2026-08-31), same `due:none` case as husain-field-guide on the same claim. Corroborating second Husain source under `error-analysis-before-armchair`, never its primary: byline confirmed, judge-versus-human loop confirmed (*"iterate on the prompt of the critique model to make it sufficiently aligned"*), but the post leads with synthetic test cases, not error-analysis-first. fallback: drop this URL; the claim stands on husain-field-guide.
...
OODA
- question: has anyone besides Husain published a measured judge-versus-human agreement loop with numbers?
...
**Closing aphorism cut (2026-08-25, Antti-approved):** *"A gate is one more claim in the system. Build it, then hold it to the same bar it holds the work to."* — hanging line restating the H1 (`check_slides.md` §16 class c). Do not restore.
