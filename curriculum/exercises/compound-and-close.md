...
Also, separately from ./CLAUDE.local.md: pin the AE101 content folder into `.claude/settings.local.json` under `additionalDirectories` so future sessions auto-load it. Read the current file if it exists, add (don't replace) the absolute path of the content folder I added via /add-dir in prework. Create the file with a single-key object if it doesn't exist. Confirm .claude/settings.local.json is gitignored (it usually is by default; verify). After this, I never need to run /add-dir for this training again.
