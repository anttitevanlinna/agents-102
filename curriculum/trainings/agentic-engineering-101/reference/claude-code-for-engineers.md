...
**Two 2026-08-19 sweep findings.** (1) The laptop-sleep item reads *a session you walk away from*, per `check_student_facing.md` §22. The one remaining time-of-day hit, `/schedule tomorrow at 9am`, is the product's own command syntax being quoted and is a false positive of the same species as the rule's power-settings carve-out. Do not re-flag it. (2) The Claude:agent noun ratio, measured 244:3, is not vocabulary drift here. This is a reference page about a named product, so *Claude Code* and *Claude* are the correct nouns for the thing being documented; §21 assigns *agent* to acting behaviour, and the acting sentences on this page already use it. The ratio is what a product reference looks like, not a defect. Do not sweep it.

**Doc verification state: the whole page (all sixteen sections) is verified against live docs; the header stamp carries the date (2026-08-15).** The page's own contract is *docs win* — where a live doc contradicts the page, the page is corrected without a maintainer call. Standing watch-items: `CLAUDE_CODE_SESSION_ID` is confirmed live (`env` inside the Bash tool) but absent from the env-vars doc — an undocumented surface the page depends on. The skills precedence rule (personal beats project, enterprise beats both) means a stale `~/.claude/skills/` entry silently shadows a committed team skill — the same failure `pre-cohort-todos.md`'s `test-strategy` collision watch names.
...
- judges @324b81d7: writing REVISE (2/0 see instances/ae101--reference--claude-code-for-engineers.writing.json), story REVISE (1/0 see instances/ae101--reference--claude-code-for-engineers.story.json), technical grandfathered, behavior PASS, pedagogy grandfathered, strategy grandfathered, slides PASS
