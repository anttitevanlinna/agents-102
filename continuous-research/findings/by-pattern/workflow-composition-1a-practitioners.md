...
**What they're composing:** A repeatable four-step loop — **Plan → Work → Assess → Compound** — packaged as a coding-agent plugin. **Correction from a 2026-09-06 current-code audit:** Compound writes qualifying learnings under `docs/solutions/`; later CE planning and ideation search that store. A discoverability pointer may be added to `AGENTS.md` or `CLAUDE.md`, but the learning itself is not written there, and arbitrary future sessions do not automatically load it. [Current `ce-compound`](https://github.com/EveryInc/compound-engineering-plugin/blob/b159e1fa4c70efa995742269d38269bcc7524dd2/skills/ce-compound/SKILL.md) [practitioner direct]; [current `ce-plan` research path](https://github.com/EveryInc/compound-engineering-plugin/blob/b159e1fa4c70efa995742269d38269bcc7524dd2/skills/ce-plan/references/research.md) [practitioner direct].
**Specific moves:**
...
**What they're composing:** **Feedback-loopable playgrounds + sub-agents** — composition primitives that solve specific named failure modes (untestable agent output, single-agent context overflow). **Correction (2026-08-01):** this section originally also named a context-preserving handoff primitive as solving "lossy compaction." Amp itself retired that primitive on 2026-05-06 in favor of automatic compaction — see the Handoff entry below. Do not read this section as evidence that Amp still avoids compaction.
**Specific moves:**
- **Handoff — RETIRED, historical record only (corrected 2026-08-01).** Amp shipped `/handoff <new goal>` on 2025-10-23 to avoid lossy compaction: *"Compaction… always had downsides. It's lossy, for one."* — https://ampcode.com/news/handoff [practitioner direct, vendor venue]. It extracted what mattered for the next task into a user-editable draft in a new thread, so work moved between threads "without losing your built-up context." **Amp reversed this on 2026-05-06:** *"So handoff is out. Compaction is in… Handoff is gone. As described above, compaction made it obsolete."* — https://ampcode.com/news/neo [practitioner direct, vendor venue]. Amp now auto-compacts when the context window is 90% full, and per https://ampcode.com/news/read-bigger-threads [practitioner direct, vendor venue] (2026-07-02), Amp's longest thread has been compacted 68+ times. Keep the 2025-10-23 description only as a dated record of what Amp believed then — it is no longer Amp's mechanism or position. (https://ampcode.com/guides/context-management is marked Archived and still describes Handoff as live; never cite it for current behaviour.)
- **Feedback Loopable** — three-part composition: Playground (shared agent-human env) + Experiments (URL-encoded reproducible state) + Verification Loop (headless CLI text feedback over screenshots). — https://ampcode.com/notes/feedback-loopable [practitioner direct, vendor venue] Lewis Metcalf (2026-02-05). Unaffected by the handoff reversal.
...
**Notes:** Vendor-venue, but practitioner bylines (Metcalf, Charlesworth, Slack, Hashimoto). Mechanism > metrics here. Handoff — once flagged here as the composition primitive most likely to converge with Klaassen-Compound and Cherny-pipeline — was retired by Amp on 2026-05-06 in favor of automatic compaction; that convergence prediction did not hold. Feedback Loopable and the specification→reference→verification→iteration chain remain the durable Amp composition primitives.
...
6. **Context-handoff as named primitive — WEAKENED (corrected 2026-08-01).** Originally counted 3 practitioners (Amp `/handoff`; Pocock `handoff` skill; Cherny `teleport` web-to-local) as L2 early convergence (N=3). **Amp itself retired `/handoff` on 2026-05-06 in favor of automatic compaction** — https://ampcode.com/news/neo [practitioner direct, vendor venue]: *"So handoff is out. Compaction is in."* Amp therefore no longer supports "solving cross-session context loss via a named handoff seam" as live practice; if anything it is now a counter-example — the vendor moved from hand-authored handoff toward automatic compaction, the opposite direction. The convergence count drops to N=2 (Pocock, Cherny), below what this file treated as even L2 signal. Treat "context-handoff as a named composition primitive" as unconfirmed pending a third live practitioner, and do not cite Amp for it going forward.
