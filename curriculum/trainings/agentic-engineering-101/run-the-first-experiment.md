...
<!--flag:module:earn-the-trust-->If Lucas F. da Costa's [Backpressure Is All You Need](https://www.lucasfcosta.com/blog/backpressure-is-all-you-need) is still on your list from Module 3, read it before the send-off: today's session produces changes faster than you can read them, which is the condition the essay is about.<!--/flag:module:earn-the-trust--><!--flag:no-module:earn-the-trust-->If Simon Willison's [Designing agentic loops](https://simonwillison.net/2025/Sep/30/designing-agentic-loops/) is still on your list from Module 2, read it before the send-off: it names the thinking you do before stepping away from a session, and today you step away.<!--/flag:no-module:earn-the-trust-->
...
**Quality:** compendium-audited 2026-08-19 (writing@a7f3966 story@a7f3966 technical@a7f3966 behavior@93bb807 pedagogy@a7f3966 strategy@a7f3966 slides@a7f3966)
- judges @a7f3966: writing PASS, story PASS, technical PASS, behavior PASS, pedagogy PASS, strategy PASS, slides PASS
- cross_module @0dea491e: PASS — set=[earn-the-trust,run-the-first-experiment,learn-from-the-test,spot-gaps-build-the-loop]; 3 pairs, 0 blocking; see instances/ae101--m3-m4 + ae101--m4-m5-m6.cross_module.json
...
- **Prep / bridge timing:** Mollick essay 10 min; Uncle Bob supplementary 7 min; Entis/Klaassen interview 30 min; Klaassen compound-engineering guide 15–20 min.
...
- `[checked:2026-07-30 result:OK due:none]` https://www.lucasfcosta.com/blog/backpressure-is-all-you-need — [delegated stamp] da Costa, the `## Prework` echo of the M4 gap read. Dated check owned by `earn-the-trust.md`, which assigns it in the six-module arc. `due:none` — a delegation does not expire, the delegate's stamp does, and `source-freshness.sh` walks the delegate. fallback: drop the echo; the pre-read still assigns the piece.
- `[checked:2026-07-30 result:OK due:none]` https://simonwillison.net/2025/Sep/30/designing-agentic-loops/ — [delegated stamp] Willison, the `## Prework` send-off callback. **Renders only in cuts that drop M3.** `plan-mode-done-right.md` owns the dated check and assigns the essay in every cut: as an M2→M3 gap read in the six-module arc (a security read, not a long-running one) and as the `no-module:earn-the-trust` pre-M4 block otherwise. In the six-module arc the callback here is da Costa, so the two `## Prework` sentences sit behind matched flags and never render together. Change one and change the other. `due:none` — the delegation does not expire, the delegate's stamp does. fallback: drop the callback; the send-off stands without it.
