...
**OpenAI: Agent Operating System — enterprise decline accelerating.** Every layer from spec to governance: Open Responses spec → Agents SDK (19K stars) → Codex (Skills/Automations) → ChatGPT Agent → Frontier (multi-vendor governance) → AgentKit (visual builder). Skills = portable agent capabilities = closest to promotion path concept. Frontier manages agents from ALL vendors including Claude. **But: 7+ weeks post-Frontier-launch with zero independent deployment evidence.** Named customers use aspirational language. **Enterprise market share decline accelerating:** Ramp March 2026 shows Anthropic wins ~70% of new enterprise buyers (up from 50/50 ten weeks ago). OpenAI saw largest single-month spending decline tracked. Merging ChatGPT/Codex/browser into desktop super-app while adding consumer integrations (DoorDash, Uber, Expedia) — US/Canada only. EEA connectors partially rolling out (inconsistent feature-flag). Open Responses spec stalled, overshadowed by AAIF. IPO Q4 2026. Product strategy fragmenting across consumer/enterprise/IPO. Vision most complete, execution gap widening, enterprise ground lost. (See `openai/state.md`)
