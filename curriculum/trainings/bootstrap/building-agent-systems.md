...
- **Setup:** End Module 1's Claude Code session (scoped to `module-1/`) and start a fresh one at your training-directory root. The Module 2 starter zip unzips there. Your `module-1/CLAUDE.md` from yesterday stays where it is — scoped to Module 1. The scaffold ships a new, wider `CLAUDE.md` at the root — that's the one Module 2 onward extends.
