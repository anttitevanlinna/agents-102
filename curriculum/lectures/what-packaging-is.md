...
**Quality:** compendium-audited 2026-08-02 (story@d1b6f2c technical@d1b6f2c behavior@d1b6f2c pedagogy@d1b6f2c strategy@d1b6f2c slides@d1b6f2c)
- judges @d1b6f2c: writing REVISE (1/1 see instances/ae101--lecture--what-packaging-is.writing.json), story PASS, technical PASS, behavior PASS, pedagogy PASS, strategy PASS, slides PASS
...
- `three-pattern-is-our-combination` · vision · "We combined them into one kit and gave them names." ← none-owed
...
- amp-handoff `[checked:2026-08-01 result:CORRECT due:asap]` https://ampcode.com/news/handoff — [practitioner direct, vendor] Amp (2025-10-23): rejected auto-compaction, bet on focused threads plus manual handoff — *"Compaction… always had downsides. It's lossy, for one."* The launch post stands as an accurate record of what they believed then. **The stance did not hold: they reversed it on 2026-05-06 and discontinued the feature.** The prior stamp's fallback said "escalate to a fresher Amp post if the stance shifts" — it shifted, and nothing escalated for three months, because a `due:none` on a dated-historical source stopped anyone re-opening the question. `due:none` is right for *what they said in October 2025* and wrong for *whether the camp still exists*, and the body had been leaning on the second. fallback: keep the October position as dated history and say what happened to it; never present the camp as current.
- amp-200k `[checked:2026-08-01 result:CAVEAT due:none]` https://ampcode.com/notes/200k-tokens-is-plenty — [practitioner direct, vendor venue] Lewis Metcalf (2025-12-09), the original short-threads doctrine: *"Agents get drunk if you feed them too many tokens."* **Citable only as dated history, and the page now disowns itself** with an appended editorial note: *"This note was written in December 2025 for an older model and context-window era. Now, auto-compaction makes longer threads work well, and it's fine and productive to go beyond 200k tokens."* Do not cite the "agents get drunk" line as a current position. fallback: cite it as what the camp believed in late 2025, and say what happened next.
- amp-neo `[checked:2026-08-01 result:CORRECT due:asap]` https://ampcode.com/news/neo — [practitioner direct, vendor venue] Amp rebuilt on 2026-05-06 and killed the feature: *"So handoff is out. Compaction is in."* Also *"Handoff is gone. As described above, compaction made it obsolete"*, and the reversal is total on the very habit this module teaches: *"No More Manual Context Management… You don't have to watch context percentages anymore, or decide when to handoff."* Compaction now runs automatically at 90% fill. GA'd 2026-05-27 (https://ampcode.com/news/drop-the-neo). By 2026-07-02 they report a thread compacted 68 times that would otherwise run 21M tokens (https://ampcode.com/news/read-bigger-threads). **An earlier stamp on this file claimed the stance was "sustained into the window." That was wrong — it read a July post as continuity without checking the May rebuild in between.** The body's counter-camp bullet, which named Amp as the live handoff camp, was cut on 2026-08-01; the handoff *shape* survives in the camps bullet as the original Ralph, without the product name. fallback: cite as one team's reversal, never as a trend — single vendor, and their later posts corroborate in time rather than independently.
- amp-context-guide `[checked:2026-08-01 result:GONE due:asap]` https://ampcode.com/guides/context-management — [vendor docs] Marked Archived, yet the body still describes Handoff as the live mechanism and never mentions compaction, while every Chronicle post footer still links it. **Amp's own docs contradict Amp's own product, three months after the reversal.** Cited here only as evidence of how unsettled the ground is: when a vendor's documentation and its shipped behaviour disagree in public for a quarter, "the field hasn't converged" is an observation rather than a hedge. Never cite for current behaviour. fallback: cite the dated news posts instead.
...
- curran-2x `[checked:2026-05-25 result:OK due:2026-10-16]` https://ideas.fin.ai/p/2x-nine-months-later — [practitioner direct, vendor venue] Curran (2026-04-16): 19.2% auto-approved, 14.6 vs 75.8 min, 86% ≤20 lines, ~473 R&D / 1,305 total. Metrics vendor-self-reported; the body carries that scoping as "Intercom published its own numbers." "(Intercom engineering)" resolves the earlier CTO title error — his title is VP Engineering. fallback: keep the numbers, attribute "Intercom's published telemetry," flag self-report.
- klaassen-already-fixed `[checked:2026-05-25 result:OK due:none]` https://every.to/source-code/my-ai-had-already-fixed-the-code-before-i-saw-it — [practitioner direct, vendor venue] Klaassen (2025-08-18): TDD *"the test fails—the natural first step in test-driven development (TDD)"* verbatim; 10-consecutive-runs + parallel feedback agents confirmed. Dated-historical, and the body inline-dates it August 2025. fallback: paraphrase the TDD-first move, drop the verbatim.
- every-compound-80-20 `[checked:2026-07-30 result:CAVEAT due:2027-01-30]` https://every.to/source-code/compound-engineering-how-every-codes-with-agents — [practitioner direct, vendor venue] Shipper & Klaassen (Dec 2025), co-bylined and first-person-plural, re-verified live 2026-07-30: *"Roughly 80 percent of compound engineering is in the plan and review parts, while 20 percent is in the work and compound."* THIS is the 80/20 primary — NOT the Definitive Guide, which lacks it. Dec 2025, outside the 6-month window: cite as framework origin, not fresh evidence. Klaassen expanded the loop past four steps on 2026-05-29 without restating the ratio, so treat it as pinned to the Dec 2025 form. Body hedges as "the ratio practitioners take from Klaassen's posture"; defensible, this URL backs it. fallback: keep the posture hedge.
- ooda-long-run-context `[checked:2026-08-01 result:CAVEAT due:cohort]` (no URL — internal OODA run, corrected 2026-08-01) kb:platform-watch/coding-agents/runs/2026-04-23-scaling-session-length-2-platform-mechanics.md — [internal research] **Not a convergent pattern:** the ~60% compaction figure traces to one unsigned vendor post, the second cited source carries no such figure, the third 403s. Subagent isolation is unaffected and stands. This slide survives the correction where the M5 opener did not, because it presents its options as **a five-camp menu the field has not settled** — a divergence claim, which is what the evidence actually supports. fallback: keep the hedged menu; a number, if one ever earns its way back, belongs here rather than in the opener.
...
- Absorption bottleneck · [borrow:none] · law:absorption-bottleneck · ← curran-2x
- Test-driven development · [borrow:software engineering] · law:none · ← klaassen-already-fixed
- Compound engineering's 80/20 · [borrow:none] · law:none · ← every-compound-80-20

Stance `[stance:2026-08-01 level:L1]`
- holds: each piece separately, and not the triad. Reference artefacts, durable plan files and automated verifiers each turn up across practitioner write-ups. **Counted strictly — all three present together as artefacts in one named practice — the independents number one:** Huntley's Ralph. Ronacher is 2-of-3 in practice and 0-of-3 in vocabulary; Klaassen's plan artefact is a pre-work spec rather than a document the agent mutates across a run, so it is a different piece wearing the same word. Against an L3 bar of 10–20 independents, the three-pattern is our combination of scattered practice — honest curriculum work, and dishonest to call convergence. The body says "we combined them" for exactly this reason.
- contested: context management for long runs, entirely. The camps bullet asserts divergence and the evidence supports divergence: Amp held the handoff position for seven months, reversed it on 2026-05-06, and its own archived guide still teaches the retired mechanism three months later.
- would-move-it: a second named practitioner publishing all three pieces together as one practice, or anyone naming the triad as a triad. Either turns "we combined them" into "practitioners converge," and slide 1's lead has to change. **It moves the other way too** — Huntley abandoning or restructuring Ralph leaves the count at zero, and the slide would owe a different sentence again.
