...
**Charles runs Ramp's Dojo** as the team-scale version: a shared marketplace of skills that any engineer can publish to and any engineer can pull from. The marketplace pulls personal moves into shared standards, which is the same accretion you get on one team, run at company scale ([Peter Yang's *Inside Ramp* interview](https://creatoreconomy.so/p/inside-ramp-the-32b-company-ai-agents-geoff-charles)).
...
- Peter Yang on Geoff Charles, [*Inside Ramp: how a $32B company runs on AI agents*](https://creatoreconomy.so/p/inside-ramp-the-32b-company-ai-agents-geoff-charles) — a shared skill marketplace run at company scale.
- Charity Majors on why agents need fast production observability to validate their own changes. (Her 2026-03-09 post at charity.wtf now returns 404; cite the argument, not the link, until a live URL is found.)
...
- charles-rachitsky `[checked:2026-08-01 result:CORRECT due:none]` https://creatoreconomy.so/p/inside-ramp-the-32b-company-ai-agents-geoff-charles — [practitioner interview] **Verified live 2026-08-01, and the page does not contain the figures it was cited for.** Peter Yang interviewing Geoff Charles, 2026-03-15. No count of shared skills, no 99.5% employee-adoption figure, no 84%. What it does carry: *"50% of Ramp's code is written by AI. It'll probably be 80% soon"*, which is a different claim the body does not make. **This is the swapped-URL failure in its pure form:** the numbers came from Charles's April 2026 X thread, the thread was auth-walled, an accessible write-up was substituted as the citation, and nobody checked whether the substitute carried the numbers. It did not, for four months. Direct fetch 402s and the documented oEmbed workaround 402s as of 2026-08-01. **A third route remains untried:** `memory/reference_x_content_fetch_workaround.md` route 0 (search-index title of the canonical URL) reportedly still worked on 2026-08-01, but this session had exhausted its web-search budget, so the thread is unverified rather than proven unreachable. **All three figures cut from body 2026-08-01;** the marketplace shape stands on this interview. fallback: none needed, the correction is the fallback. Do not restore the figures without a live primary.
- majors-charity-wtf `[checked:2026-08-01 result:GONE due:asap]` https://charity.wtf/2026/03/09/your-data-is-made-powerful-by-context-so-stop-destroying-it-already-xpost/ — [practitioner direct] **The URL returns HTTP 404 as of 2026-08-01,** so the verbatim quotation attributed to Charity Majors cannot be checked by anyone. A quotation attributed to a named person behind a dead link is the exact failure M6's own opener is a memo about, and it was sitting in a shipped supplementary. **Quotation marks removed from body 2026-08-01;** the point now stands as a paraphrase, which needs no link. fallback: if a live URL turns up, the quote can be restored after checking it; until then attribute the argument, never the words.
- wolff-qcon `[checked:2026-08-01 result:OK due:none]` https://www.infoq.com/presentations/engineering-ai/ — [practitioner talk] **Verified live 2026-08-01.** Adam Wolff, *Engineering at AI Speed: Lessons from the First Agentically Accelerated Software Project*, page dated 2026-05-07. The quote holds verbatim: *"when the implementation cost goes to zero, the feedback loop becomes everything"* — note **becomes**, not *is*, and body already carries the correct form. The SQLite passage is a **failure story, not a build story**: *"This is a story of 15 days that I'm never getting back… We're now just over two weeks taking this feature out that we just shipped."* Body already tells it as shipped-then-removed, which is right; the retired TODO described it as a build-in-two-weeks anecdote, which would have inverted it. Durable talk recording, so `due:none`. fallback: paraphrase the principle, keep the reversal story.
...
Flagged
- `[found:2026-08-01]` **Resolved by opening all twelve.** Nine held, two failed, one was right all along. The two failures carry the lesson: **Ramp's three figures are not on the page that cited them.** The numbers came from Charles's April 2026 X thread; the thread was auth-walled; an accessible write-up was substituted as the citation; nobody checked whether the substitute carried the numbers. It did not, for four months, including a 99.5%-of-a-whole-company adoption claim. **A swapped URL is a new citation and owes a fresh check** — the swap is the moment a claim silently loses its source. Figures cut from body.
- `[found:2026-08-01]` **Majors' URL returns 404**, so a verbatim quotation attributed to a named person sat behind a dead link in a shipped file. Quotation marks removed and the argument kept as a paraphrase → if a live URL turns up, the words can return after a check.
- `[found:2026-08-01]` Ramp is **unverified, not disproven.** Two of three X routes are down (direct 402, oEmbed 402, both already recorded in `memory/reference_x_content_fetch_workaround.md`); the third, search-index title, was untried because this session had spent its search budget → try route 0 before concluding the figures are unrecoverable. Cutting them from body was still right: an unverifiable figure does not ship, whatever the reason it could not be checked.
