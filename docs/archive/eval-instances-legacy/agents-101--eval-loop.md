# Eval Instance — Agents 101 / Eval loop exercise

Filled-in instance of `curriculum/evals/exercise.md` for the "Eval loop runs itself" exercise in Module 6 (Evaluations) of the Agents 101 training.

**Target exercise file:** `curriculum/exercises/eval-loop.md`
**Module file:** `curriculum/trainings/agents-101/evaluations.md`
...
Enables these LOs from `trainings/agents-101/evaluations.md` (verbatim):
...
**Capability-dependent:** walk-away duration is the load-bearing plug point. If an actual end-to-end run on a representative training directory takes 12 min, the walk-away is fake; if 45 min, the exercise overruns the Agents 101 module slot. Must be timed before first delivery (named in maintainer section).
