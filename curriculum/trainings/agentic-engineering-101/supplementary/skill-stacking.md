...
Sources
- dino-stack `[checked:2026-07-05 result:ATTESTED due:none]` attested:maintainer 2026-07-05 read the in-repo skill files and the `/ship` orchestrator directly — [maintainer-attested] One engineer's in-repo skill stack read first-hand; no URL owed, no date stales it. **`/ship` is Dino's own skill, NOT a Claude Code built-in** — `lectures/composing-the-workflow.md`'s Dino figure draws it as this kit's orchestrator and its footprint chart keeps *the pilot* generic; any edit reading `/ship` as a product feature is wrong. fallback: none; the worked example is the evidence and its scope is stated in the page's first line.
