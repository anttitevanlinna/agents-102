# Bootstrap Training Architecture

Bootstrap uses a compounding working-directory model. The student is not just reading modules; they are building a local agent system that carries memory, sources, agents, judges, outputs, and operating rules forward.

## Platform

Bootstrap runs on Claude Code (CLI + Desktop) and Cowork. Not Claude.ai chat.
...
Cloud/remote features are out of scope for Bootstrap exercises. Remote tasks need a cloud source for the working directory, typically a cloud Git repo. Bootstrap uses a local training directory.
...
Bootstrap installs all trainer-provided working material during prework. Do not design module-day downloads or zip patches.
...
The Bootstrap starter is maintained in Git at `curriculum/scaffolds/training-starter/`. Participants never see Git. Export the starter per cohort and host it on the customer's site or SharePoint.

## Skill Invocation

Bootstrap should inline method moves unless a module actually ships the skill it asks students to invoke.
...
- No Bootstrap scaffold should require `crux`, `assumption-test`, or `pre-mortem` skills to exist.
