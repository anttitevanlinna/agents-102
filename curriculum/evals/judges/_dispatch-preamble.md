...
- A numbered rule the prefill resolved is in the sidecar, and it is absent from your brief. **A numbered rule that is not in your brief gets no row from you.** Leave it, do not re-derive it, do not rewrite its evidence. This is the one instruction here a judge can disobey without anything looking wrong afterwards — the merged ledger is identical either way, and only the clock knows. **Still emit every explicit judge-owned criterion in your class template.** Those criteria are not compendium rules, so neither the brief nor the sidecar can carry them; treating brief absence as resolution dropped the strategy judge's Big Idea row on a live warm run.
