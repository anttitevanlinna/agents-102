...
- `[checked:2026-08-02 result:OK due:cohort]` https://github.com/sirmalloc/ccstatusline — [capability] The line 41 aside. Third-party status-line formatter for the Claude Code CLI shipping Context % and Context Length widgets — the continuous read of what `/context` reports once. Active at check: v2.2.27, 361 commits, not archived. `due:cohort` because a third-party tool runs on somebody else's release schedule. The claim is owned in full by `exercises/orient-and-introspect.md`, which teaches the beat; this stamp exists because the module names the URL in its own body. fallback: cut the parenthetical — `/context` alone carries the point.
