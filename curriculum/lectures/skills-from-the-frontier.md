...
**Emphasis pass (2026-07-09, Antti-directed "go very lightly on the bold"):** bold = two handles — **skill** at its slide-1 definition and **Don't make general what you don't practice yourself** on *Two curated, one authored*; every other slide carries zero bold; "Go." kicker plain. Per `theory-plan.md § Slide format — emphasis budget` + `check_slides.md §9`.

**Slides-only pass (2026-07-02, unaudited):** covered regions DELETED (Path A — prose superseded by the slides; git carries it). Per-passage verdicts: opening "Two moves today" agenda CUT (slide titles carry the agenda; the Tuesday-afternoon claim FOLDED into *Two curated, one authored*) · "What's a skill, one breath" CONVERTED to slide 1 — §3 fix: "in Module 2" / "landed as a plain prompt in M2" de-sequenced to content phrasing · three-voices prose CONVERTED to one slide, one bullet per voice — §3 fix: "M1 was `CLAUDE.local.md` … M3 is your first authored skill" de-sequenced to kit-pattern phrasing (rules file personal + gitignored; first authored skill lands the same way) · "Why this proportion matters" CONVERTED (the two flips became two bullets) · "What earn the trust means" CONVERTED; "end of 1h45" → "by the module's end" (runtime stays in maintainer Meta) · "What to watch for" CONVERTED to the arming slide; Ex1/Ex2/Ex3 labels dropped (bullets name the move — keeps the lecture re-placeable) · INSTALL PATH: the authored skill lands at `~/.claude/skills/test-strategy/`, personal-first per voice three and the module's watch-for — do not point it at a team-kit path · "Don't make general …" KEPT as slide-3 bullet · "Go." KEPT as closing kicker. §3 grep above the fence: clean.

**Lecture meta:** *Primes the security exercises that follow it in Module 3 (Earn the trust) — order in that module's `## Start here`. Runtime lives on the Meta Time line below, which `scripts/calculate-time.js` reads.*
**Meta (trainer):**
- **Time:** 12 minutes
...
- klaassen-compound-engineering `[checked:2026-09-05 result:CAVEAT due:2026-06-11]` https://every.to/source-code/compound-engineering-how-every-codes-with-agents — [practitioner direct, vendor venue] Shipper + Klaassen, co-bylined, pub 2025-12-11; four-step naming verbatim and in printed order — the page prints **Plan → Work → Review → Compound** under its own *Compound engineering loop* heading, byline and pub date holding, URL live. CAVEAT, two of them: Dec 2025 is outside the 6-mo window, so cite as the framework's origin rather than fresh evidence; and Klaassen expanded the loop on 2026-05-29 (`every.to/p/compound-engineering-gets-an-upgrade`) to *"Ideate → brainstorm → plan → work → review → polish → compound → repeat"*, which moves `compound` off fourth place. Name, mechanism and attribution all hold — only the ordinal moved. Never attach a step number to this framework without re-counting first. **The past `due` is the design, not a finding — accept-note for `check_research_claims.md` §2.** Due is publication-anchored (pub 2025-12-11 + 6mo) and §11a bars a checked-anchored due, so no re-verification moves this field and the audit WARN is permanent by construction. §2 is discharged the way it asks of an out-of-window source: cited as dated origin, which this stamp and the body both do. Neither `due:none` escape reaches here — the anchor is a count (the four-step naming), which fails `source-freshness-format.md` § Theory-construct variant condition (a), and the one-source `due:none` ruling names `compound-engineering-the-definitive-guide`, not this URL; widening it to this URL is Antti's call. A §2 judge meeting this line re-opens the URL if the `checked:` date has aged, and files nothing on the `due` alone. The other body claiming this source is `lectures/where-the-rule-could-live.md`, whose stamp anchors on the 2026-05-29 expansion and therefore reads `result:OK due:2026-11-29`: different anchor, different clock, and its "in step with `skills-from-the-frontier.md`" clause does not make the two interchangeable. Edit either anchor and the other owes a re-read.
...
Flagged
- `[found:2026-08-02]` **L3 trigger unfired, second consecutive cycle.** Vercel's newest coverage (**Aman Gupta on Andrew Qu, TechInformed**, Jun 29 2026 — `[domain trade publication]`, a write-up of a public Q&A with Anthropic) gives *"the firm's data science agent now runs on roughly 100 discrete skills"* with non-developers *"among the most-downloaded skill authors"* — on **skills.sh, a public registry**, not an internal kit; no contributor count, no workforce share. Ramp unchanged at 350+ Dojo skills, no contributor number. Stance stays L2; the concentrate-vs-spread hypothesis is in Stance › contested.
- `[found:2026-08-02]` **Counterweight unit — body conversation.** The flagship kit's own maintainer reports per-skill cruft as routine (scanlan-good-bad-skills) while two cycles have found no kit that decayed. Does the counterweight become *"kits don't die, skills do — so who owns yours?"*, sourced today, rather than an org-scale decay story that may not exist to be found?

<!-- /backing -->

`check_lectures §3` accept-notes: *by the module's end* = within-module scoping, not cross-module sequencing; *the next engineer* = an engineer, not a module.
