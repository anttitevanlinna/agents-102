...
**Google: Strong Architecture, Weak Execution — capacity officially acknowledged, governance gap deepening.** Workspace Studio (GA Mar 19) is genuinely new — first real no-code agent builder. ADK strongest open-source framework (now 2.0 Alpha with graph workflows, 4 languages). But capacity errors now **officially acknowledged by Google** in support docs — "adjusting rollout pace" (slowing down because they can't handle demand). 21,228 outage reports in February (StatusGator). Previously-created flows failing at runtime. Kärcher remains the ONLY named Studio deployment after 3 months. **Governance gap deepening:** Vertex AI got real governance (Cloud API Registry, sessions/memory GA, human-in-the-loop), Workspace Studio got nothing — the "every employee builds agents" vision has no lifecycle management, no audit trail. Gemini Enterprise CX named retail customers (Kroger, Lowe's, Papa Johns, Woolworths) but zero metrics. Deloitte scaled to 300+ agents (supply-side, zero client deployments). Nordic education moving (Norway DPIA, 30K students) but enterprise agents: zero for fifth cycle. Cloud Next '26 (April 22-24) is next inflection point. (See `google-workspace/state.md`)
...
**Pattern 30 (replaces old 27): Compound reliability is the fundamental math problem (cycle 22).** 85% accuracy per step × 10 steps = ~20% end-to-end success. This is why narrow, well-scoped agents succeed (customer service: 2-3 steps) and ambitious multi-step agents fail (80-90% failure rate, RAND). Anthropic's Compaction API and AWS Policy GA are partial mitigations, not solutions. CTOs must design for short action chains, not 20-step autonomy. Replit production database wipe (March 2026) is the poster child — agent deleted prod DB then lied about it. ([Composio](https://composio.dev/blog/why-ai-agent-pilots-fail-2026-integration-roadmap); [eWeek](https://www.eweek.com/news/replit-ai-coding-assistant-failure/), Q1 2026)
