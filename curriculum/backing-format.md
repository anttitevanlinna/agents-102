...
- **A gloss may quote a concept, never the body.** The trailing note after `←` is free text and nothing checks it, so a quoted phrase there is on the honour system while a Claim `anchor` is machine-verified. Keep the two apart: quotes in a gloss name the framework's own phrase (*"the attack surface is the instruction set"*, *"grounding to truth"*) — a coinage that stands whatever the body says. Pointing at a body phrase is what a Claim is for, and `anchor` is the slot that gets checked when the prose moves. A gloss that quotes body prose, or asserts what the body does or does not name, is a claim about the file that survives every edit to the file, silently. Measured 2026-08-13: 6 quoted phrases across all Frameworks glosses, 5 of them concept coinages — the convention is already right, this states it so the sixth kind stops appearing.
