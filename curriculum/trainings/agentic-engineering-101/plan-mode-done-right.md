...
Two reads on one plan catch different misses. Yours goes first, because your codebase experience is what spots the soft item and the step that does not fit this repo. The agent's read goes wide where yours goes deep, and you stop when another answer would no longer sharpen the plan.
...
**Argyris appears twice in M2, and both stay.** `how-instructions-grow.md` credits him with the link where the distinction is taught; this Key Concepts bullet names the concept at module close as the recall handle for the second loop. Two placements, two jobs — `check_writing.md` §11's carve-out, whose test is whether the second mention does different work rather than whether it is the second. One mention apiece is the budget; a third fires §11 normally. **This bullet owes no citation** (Antti 2026-09-10): double-loop learning is learning-organisation canon, the `cultural-vocab` case, so naming Argyris here is ordinary vocabulary and not a claim needing a source. The stamp on the lecture covers the taught treatment.
...
**The soft-compound and Boy Scout closes moved into `exercises/extract-the-task-shaping-rule.md` as Phases 4–5 (2026-08-25, Antti-directed).** The module body carries includes only between the exercises and the closing lectures. Their design notes (fence acceptance, Boy Scout attribution call, `push-back-on-the-plan-4` register) live in that file's maintainer block.

**M2 issues no homework.** Do not add a `## Homework` section. The ticket read is not M1's alone: `extract-the-task-shaping-rule` runs the same move on a **story** ticket, where M1's `close-the-ticket` runs it on a **bug** ticket (2026-08-12, maintainer-directed). The split is by ticket kind, and the M2 instance is scoped to one short slide inside the existing exercise — not a second exercise, and not a homework beat.
...
- `[checked:2026-08-15 result:CAVEAT due:cohort]` https://code.claude.com/docs/en/claude-code-on-the-web — [platform docs] Cloud sessions run on Anthropic-managed infrastructure and can be opened and steered from the Claude mobile app. CAVEAT: research preview, not GA — Pro/Max/Team, and Enterprise only with premium or Chat + Claude Code seats; a room will not all have it (in step with `lectures/where-the-rule-could-live.md`). fallback: replace the cloud-and-mobile challenge with a local Remote Control handoff.
...
**Quality:** compendium-audited 2026-09-05 (writing@324b81d7 story@324b81d7 technical@324b81d7 behavior@17446703 pedagogy@324b81d7 strategy@324b81d7 slides@191ea58f)
- judges @324b81d7: writing PASS (2 findings see instances/ae101--module--plan-mode-done-right.writing.json), story PASS (1 finding see instances/ae101--module--plan-mode-done-right.story.json), technical PASS (1 finding see instances/ae101--module--plan-mode-done-right.technical.json), behavior PASS, pedagogy PASS (1 finding see instances/ae101--module--plan-mode-done-right.pedagogy.json), strategy PASS (1 finding see instances/ae101--module--plan-mode-done-right.strategy.json), slides PASS
...
**Decision points:**
- **Exercise runs to 70 min:** the second read went deep (common on real codebases). That is 13 over its 57, and there is no spare beat to take it from — this module has no buffer transition, and the Slot line already runs past the 120 cap before this happens, so that give is spent (`npm run audit:timings` for the current figure). Take it from the Bridge (5), then compress the Debrief (7) — in that order, and keep the pattern-naming step. Below that, the give is the second exercise, not the close.
