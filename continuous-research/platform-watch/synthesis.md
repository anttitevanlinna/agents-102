...
Last updated: 2026-03-21 (cycle 25 update)
Total OODA cycles: 25 (dev sweep + business agent + architecture deep dives + trajectory analysis + vertical SaaS + open-source frameworks + multi-platform update + OpenAI market dynamics + Microsoft Wave 3/GTC deep dive + Google post-GA reality check + OpenAI enterprise decline acceleration + Google cycle 22 + landscape update + OpenAI PE pivot + AgentCore deep dive + agent governance convergence + cycles 18-22 synthesis + cycle 24: Microsoft Cowork/Agent 365 + cross-platform scan + cycle 25: Google post-GA + vertical SaaS counter-evidence + standards convergence + agent identity crisis)
...
**Microsoft: The Azure of Agents — governance teeth, zero deployment evidence post-GA.** Own the governance/identity/security layer, not the apps. Nadella: "SaaS will dissolve into agents on CRUD databases." **Foundry Agent Service GA (March 16):** private networking, multi-model (Claude + GPT + Grok + Nemotron), built on Responses API. Prompt + Workflow agents GA; Hosted Preview. First named customer: Corvus Energy (Norwegian, no metrics). **But 5 days post-GA, zero new customers.** **Agent 365** (May 1) is the strongest differentiator — shadow agent detection found 500K agents inside Microsoft. 29% of enterprise agents operate without IT approval (Cyber Pulse). But Agent 365 governance is Microsoft-ecosystem-only (independently confirmed by Entro Security). **DLP security bug (Jan-Feb 2026):** DLP policy failed, exposing confidential email for 3 weeks. **CVE-2026-26144:** Excel bug weaponizes Copilot Agent for data exfiltration. Claude now available for ALL Copilot Studio agent types (not just CUA). **Copilot Cowork:** 21+ days past "late March" Frontier access promise, zero independent reviews. E7 ($99/user) bundle math improves as E3/E5 prices rise July 1. 15M Copilot seats but Copilot market share contracted 39% (18.8% to 11.5%) among paid AI subscribers. Five Level 3 convergence patterns from practitioner analyst community: governance incomplete at launch, agent sprawl real, adoption ~3%, DLP/security gaps real, licensing unclear. (See `microsoft-365/state.md`)

**OpenAI: Agent Operating System — enterprise decline accelerating, pivoting to PE distribution.** Every layer from spec to governance: Open Responses spec → Agents SDK (19K stars) → Codex (Skills/Automations) → ChatGPT Agent → Frontier (multi-vendor governance) → AgentKit (visual builder). Skills = portable agent capabilities = closest to promotion path concept. Frontier manages agents from ALL vendors including Claude. **But: 7+ weeks post-Frontier-launch with zero independent deployment evidence.** Named customers use aspirational language. **Enterprise market share decline accelerating:** Ramp March 2026 shows Anthropic wins ~70% of new enterprise buyers (24.4% of Ramp businesses now pay for Anthropic). EpochAI projects revenue crossover August 2026. **PE joint venture ($10B with TPG/Advent/Bain/Brookfield)** — Forward Deployed Engineers at $10M minimum. Implicit admission Frontier can't self-serve. **Hybrid computer-use architecture converging:** TinyFish (90% Mind2Web), Stagehand v3, Skyvern independently arrive at ~80% deterministic + 20% AI — outperforming vanilla computer use. Merging ChatGPT/Codex/browser into desktop super-app. Five strategic fronts simultaneously (super-app + Frontier + PE subsidiary + AWS deal + IPO). EEA connectors partially rolling out (feature-flag lottery, reportedly accelerated by Anthropic's unrestricted European MCP). Open Responses stalled, overshadowed by AAIF. **Agent identity crisis:** 93% of agent projects use unscoped API keys (AGAT audit); OpenAI has no identity management comparable to Entra. Vision most complete, execution gap widest, enterprise ground accelerating loss. (See `openai/state.md`)

**Anthropic: Standards + Best Engine — and quietly building the full stack.** Three-layer play becoming **four layers**: (1) open standards competitors adopt (MCP + Agent Skills), (2) model+runtime powering partners (Copilot Cowork), (3) own business surface (Cowork + plugins + marketplace), (4) **hosting/runtime (Antspace)**. Antspace is Anthropic's hidden PaaS — discovered Mar 18, 2026 by AprilNEA reverse-engineering Claude Code's Firecracker MicroVM binary. Full deployment protocol built from scratch (not wrapping Vercel). **Two runtime surfaces, not one:** (a) natural language → Baku (Vite+React+TS app builder) → Supabase auto-provisioned → Antspace deploy → live app (the consumer/business user path); (b) **API-based agents running in the sandbox** — agents built with Claude API / Agent SDK can run in the same Firecracker MicroVM infrastructure (Anthropic direct evidence). This makes Antspace a **general-purpose managed agent runtime**, not just a web app host. Direct competitor to Bedrock AgentCore and Foundry Agent Service — but with a critical differentiator: **BYOC (Bring Your Own Cloud)** with Kubernetes integration. Compute comes to the data, not data to the cloud. This could solve two of three F-Secure walls simultaneously: runtime (#2) and data access (#1, because BYOC runs inside the enterprise network perimeter). The full stack if it ships: model → Agent SDK → sandbox runtime → BYOC enterprise deployment. No other platform owns this end-to-end. Still staging. Computer Use (72.7% OSWorld) = legacy integration. B2B marketplace launched Mar 6. $100M partner network. **But: infrastructure fragility is now Level 3 convergence — 109 incidents in 90 days.** API uptime 99.34% = ~6 hours downtime/month. MCP enterprise governance filled by third-party gateways, not Anthropic. Agent Teams = coding only, not business users. ([AprilNEA analysis](https://aprilnea.me/en/blog/reverse-engineering-claude-code-antspace); [The Register](https://www.theregister.com/2026/03/19/anthropic_claude_sre/), Mar 2026). (See `claude-anthropic/state.md`)

**Google: Strong Architecture, Weak Execution — capacity officially acknowledged, hard limits documented, governance gap deepening.** Workspace Studio (GA Mar 19) is genuinely new — first real no-code agent builder. ADK strongest open-source framework (now 2.0 Alpha with graph workflows, 4 languages). But capacity errors now **officially acknowledged by Google** in support docs — "adjusting rollout pace" (slowing down because they can't handle demand). 21,228 outage reports in February (StatusGator). Previously-created flows failing at runtime. **Hard limits confirmed:** 50 executions/day (Business), 500/day (Enterprise), 20-step maximum per flow, webhooks limited preview only. Promotional limits expire March 31 — capacity may worsen. Kärcher remains the ONLY named Studio deployment after 3 months. **Governance gap deepening:** Vertex AI got real governance (Cloud API Registry, sessions/memory GA, human-in-the-loop), Workspace Studio got nothing — the "every employee builds agents" vision has no lifecycle management, no audit trail. Gemini Enterprise CX named retail customers (Kroger, Lowe's, Papa Johns, Woolworths) but zero metrics. Deloitte scaled to 300+ agents (supply-side, zero client deployments). Nordic education moving (Norway DPIA, 30K students) but enterprise agents: zero for fifth cycle. Cloud Next '26 (April 22-24) is next inflection point. (See `google-workspace/state.md`)

**Vertical SaaS: Ahead in Their Lanes — and Agentforce pulling away.** Salesforce Agentforce ($800M ARR, 29K deals, 12K deployed customers — FY2026 full year). 60%+ of Q4 bookings from expansion buying, not new experiments. UK police "Bobby" agent: 20% reduction in human agent demand. Zendesk (51% auto-resolution at SeatGeek, Forethought acquisition closing Mar 2026), ServiceNow ($600M+ ACV), SAP Joule (deepest ERP agents). Customer service is the breakthrough domain with Level 3 convergence evidence. **Agentforce is the only platform with convergence-level deployment evidence for business agents.** The vertical advantage: they own the data context. The limitation: strong only in their lane, no cross-enterprise capability. Nordic signal: Finnair (80% chat resolution) and reMarkable are global showcases. ([Subscription Insider](https://www.subscriptioninsider.com/article-type/news/salesforce-fy2026-results-show-subscription-led-revenue-base-as-agentforce-becomes-a-fast-growing-layer); [CX Today](https://www.cxtoday.com/crm/agentforce-becomes-salesforces-fastest-growing-product-ever/), FY2026). (See `vertical-saas/state.md`)
