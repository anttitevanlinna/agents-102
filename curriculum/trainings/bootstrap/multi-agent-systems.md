...
## Meta
- **Prework:** [Before Module 3](exercises/module-3-prework.md). Five practitioners automating their life with Claude Code and OpenClaw (pick three, read one piece each). Plus a plain-language primer on how Claude Code launches helper agents (sometimes called subagents): what they are, how to start one, what you'll see.
...
- **Three stances beat one summarizer.** The synthesizer's three subagents (a backward-from-end planner, a *what-would-have-to-be-true* experimentator, and a counterintuitive reframer — the Rory seat) each interrogate the retrieved material from a stance one agent won't hold at once. The main session makes the call.
