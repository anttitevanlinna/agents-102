...
*What classical security still owns and you still need.* The network perimeter. The WAF in front of the LLM API. Egress controls. mTLS to the model provider. The IAM-scoped service principal the agent runs under. Audit logging. Secrets management. Patch hygiene on the agent's host. Threat modelling for the parts of your system that are still deterministic code. The CTO who concludes *"agent mitigations replace network controls"* ships a breach. The floor stays.
