...
<path d="M866,244 C858,230 998,244 1034,260 C1070,276 1110,340 1082,338 C1054,335 874,257 866,244 Z" stroke="url(#fh-wall)" stroke-width="2.4" opacity="0.8"/>
</g>
<g fill="#2f6b6b">
...
<rect x="12" y="56" width="588" height="490" fill="#efe6d2" opacity="0.55"/>
<g text-anchor="middle" font-family="ui-monospace, Menlo, Consolas, monospace" letter-spacing="2.5" fill-opacity="0.45" paint-order="stroke" stroke="#efe6d2" stroke-width="4" stroke-linejoin="round">
<text x="528" y="210" font-size="12.5" fill="#786c56">◌ THE PROTOTYPING LOOP</text>
<text x="600" y="448" font-size="12.5" fill="#2f6b6b">THE BUILD LOOP</text>
<text x="660" y="330" font-size="11" letter-spacing="2" fill="#2f6b6b">THE AGENT'S OWN LOOP</text>
</g>
<g text-anchor="middle" font-family="Inter, sans-serif" font-size="11" fill="#4a4234" fill-opacity="0.45" paint-order="stroke" stroke="#efe6d2" stroke-width="4" stroke-linejoin="round">
<text x="528" y="225" fill="#786c56">spin a throwaway to finish the plan</text>
<text x="600" y="463">run · let it fail · shape the verifier · re-run</text>
<text x="660" y="344" font-size="10">act · read the result · correct</text>
</g>
...
- would-move-it: a published account of a team running long agent work where **no human absorbs the output at all** and the outcome holds — that would move Absorption from a human phase to a place a machine can stand in, and this lecture's closing question (*"what makes you trust the result at the end?"*) would need a different answer. In the other direction, a practitioner publishing a *fourth* far-half phase our three cannot hold; the standing candidate is provenance (see Flagged).
...
**Flagged**
- `[found:2026-07-31]` **The per-run framing is the personal-tier reading, and the slide does not say so.** Osmani puts the human in *"the constraints loop… the sampling loop (how much output should we sample and review?), the audit loop"* — a sampling loop means some runs are never read by a person. This slide teaches Verification and Absorption as phases every output passes through. → At a student's scale the per-run reading is the true one, so nothing here is wrong for its audience. The question is whether the far half should say that its per-run shape is what these phases look like at personal scale, and that they become policy at team scale. `the-map-filled-in` at M6 is where the climb is drawn, so that is the natural place to land it rather than loading the M4 container.
- `[found:2026-07-31]` **A fourth far-half phase candidate: provenance.** Osmani's fourth loop is *"the audit loop (what evidence should we keep and how do we make sure our audit log is effective?)"* — governed by a question none of our three quite asks: can we reconstruct and justify this later? Verification asks *is it good now*, Absorption *how much do we take up*, Outcome *what survives*. → Is auditability inside Outcome on a broad reading of "what survives the run", or is it a phase we do not draw? One practitioner, so L1–L2; not enough to move a map, enough to watch. Cost-of-a-run and rollback were also tested as candidates and **not** found named as phases by anyone reached — recorded as absence in reached sources only, since Klaassen/Shipper and the fuller Willison and Horthy archives were not substantively opened.
