...
- Take-home (Close): a stealable prompt that names the four moves (parallel generation, scoring, meta-agent rule edits, rounds + dashboard) and asks Claude to set up the loop around any judge the student already owns. Short, move-first, no M6-specific scaffolding. Operationalizes "the loop is a pattern, not a one-off."
