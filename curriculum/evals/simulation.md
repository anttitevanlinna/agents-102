...
HOW TO READ THE TARGET FILE: source `.md` files use `{{prompt:<key>}}` markers in place of inline `**Prompt**` fenced blocks (the curriculum-prompts registry refactor). Run `node scripts/expand-md.js [file path]` and use the EXPANDED output as the exercise under simulation — that's what students and the workbook see post-render. Files with inline blocks pass through unchanged.
...
4. **Mood score 1–10** at phase-end + one-line note on what the student is feeling (rate against the module mood contract using the Mood scale at `curriculum/evals/simulation.md` §Mood scale; flag drift) (`mood_score`, `mood_note`)
...
The release signal for a run is `node curriculum/evals/scripts/sim-freshness.js --gate --file <target>` for each file it touched: every trace those files' verdicts rely on must be fresh, and a missing one fails. The corpus-wide report is cache state, not a gate. A stale trace keeps its still-valid per-phase and per-prompt entries, and the next judge fire regenerates the rest.
