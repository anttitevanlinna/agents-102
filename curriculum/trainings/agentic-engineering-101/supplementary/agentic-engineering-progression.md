...
The engineer uses AI for assistance.

The engineer works with it interactively.

The engineer delegates bounded outcomes.

The engineer directs several independent sessions.

The engineer encodes recurring work into checked loops.

The engineer improves the context, memory, skills, and verification that make every subsequent session stronger.

Each stage follows because the engineer encounters a new constraint. Trust enables delegation. Durable context enables longer work. Verification enables greater reach. Independent work enables parallelism. Parallelism demands coordination. Repetition reveals what belongs in a loop. Working loops make the surrounding system the highest-value thing to improve.
...
**Meta:** *AE101 supplementary, ~4-minute read. Part 1 gives the engineer-active progression in one section. Part 2 gives the leverage model. First linked from M1's pre-reads; linked again from M3's `## Next` section.*
...
**Placement:** Primary link from M1 (`getting-going.md`) under `## Pre-reads before Module 2`. Reappears after M3 (`earn-the-trust.md`) as an optional forward look.

**Voice:** Direct, compact, engineer-active. The progression and its causal chain stay together; the leverage model gets the page's only diagram.
...
- contested: the sequence compresses several branches into one line. Parallel sessions and recurring loops can arrive in either order. The page claims that each gain exposes a new constraint, not a universal chronology.
