...
- `[checked:2026-07-02 result:CAVEAT due:2027-01-02]` https://www.anthropic.com/engineering/multi-agent-research-system (2025-06-13) — [practitioner direct, vendor venue] — "multi-agent systems use about 15× more tokens than chats." Re-verified live 2026-07-02: 15× quote verbatim on page; "fewer truly parallelizable tasks than research" verbatim on page. DATED (~12mo, outside 6-mo window) + effectively single-source + the SAME post says fan-out is wrong for most coding ("fewer truly parallelizable tasks than research"). Body frames 15x as a research-style ceiling, not a coding default, and flags it as a year old. The "×10x → 150x compounding" is NOT in the primary — do not use. fallback: drop the 15x figure, keep "many parallel agents multiply the bill, and fan-out is the wrong shape for most coding."

Two guards not in the stamps above: the "90 percent" is presented AS folklore on purpose (not asserted); the bloat-cost calc re-derives from the Opus pricing stamp — 100K = 0.1 MTok x $5 x 20 = $10 naive; cached = 1 write at $6.25/MTok + 19 reads at $0.50/MTok → a bit over $1. Do NOT reintroduce two numbers that failed Sweep-2: "caching saves ~81%" (vendor arithmetic on one task) and any auto-compaction trigger threshold ("83.5% / 33K") — docs give only "approaching context limits."

Live-test before cohort (capability claims, `due:cohort`): `/context` category breakdown; `/clear` wipes vs `/compact` summarizes; subagent runs in its own window and returns only its summary; `./CLAUDE.md` loads every session; skills load on demand; WebFetch reaches the Anthropic pricing page (live-fetched 2026-06-03, Opus 4.5–4.8 all $5/$25), so the cost-calc prompt's "use the pricing page" instruction is fulfillable, not a phantom fetch.
