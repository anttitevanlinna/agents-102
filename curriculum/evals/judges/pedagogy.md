...
- **Look-lines name the object and the tell** (`check_student_facing.md §9`, sibling of the actor-and-mechanism rule for do-lines). Every line asking the student to read, watch, check, notice or judge owes: precisely WHAT is under inspection — in this corpus usually *the agent's account of X* rather than X, and getting it wrong promises direct access to a fact where only a generated answer exists — and WHAT a good or bad instance looks like, so the student knows when they have found it. **Mechanical cross-check, run it every time:** read the file's own per-phase failure rows (§47) and leap tests in the maintainer block, then ask of each whether the student could catch that failure themselves in the moment. Yes → its tell is owed in BODY and its absence is a REVISE; the maintainer row keeps only the trainer's recovery move. This is the highest-yield check on this surface because the knowledge is nearly always already written and merely filed below the fence, where no student and no in-room trainer reads it. A step that survives as pure procedure (*"read the diff"*, *"let the read finish"*) with no object or tell is the signature of a generator filling a slot the design brief never sourced.
