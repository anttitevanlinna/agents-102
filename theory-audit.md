...
**⚠ Source status.** Canon candidate URLs are AS-REPORTED BY LENS AGENTS — unverified at audit time. The adopted Tier-1 set is now verified where it shipped: each consuming file's backing block carries the full stamp (Wei → `supplementary/verification-asymmetry.md` · Willison trifecta → `supplementary/the-lethal-trifecta.md` · Husain ×3 → `lectures/the-gate-is-a-claim.md`). Per `check_research_claims §11`: any FURTHER candidate's URL gets opened before its claim lands in student-facing body; single-lens candidates are one agent's judgment (§4 — hold loose); convergence is marked where ≥2 independent lenses proposed the same piece.
...
**Coverage state (verified against the shipped corpus, 2026-08-31):** convergence-triad COVERED (`what-packaging-is.md` three-pattern close + `supplementary/verification-asymmetry.md`) · comparative-advantage COVERED with the mechanism fixed to verification cost (`verification-asymmetry.md`; supplementary-tier — no in-room M4 Intent lecture names it) · dont-tamper COVERED (`the-gate-is-a-claim.md`, among the deck's five laws) · l0-l3-ladder SETTLED design-side (Antti, 2026-09-02: not student-facing wording; the climb stays enacted via `what-packaging-is.md`'s candidate → tested-here → durable sections, the ladder stays the internal diagnostic the strategy doc records as "borrow the posture, not the label", and the instrument's "L3 agentic engineer" wording was de-jargoned in the same ruling) · individual-not-institutional COVERED implicitly as the row proposed (`agents-that-build-agents.md` Cross personal → team bullet) · access-trust-gap / competence-first-sequence / pattern-layer-ontology OPEN by the table's own not-AE101 call. The three not-AE101 rows are ruled maybes, held in `theory-plan.md` § Coverage audit — not todos (Antti, 2026-09-02).
...
- Cross-links: groundwork business mat already carries `design-the-filter` (reviewer → system-designer, absorption) — same law, other mat. Costa ships a `backpressured` skill — evaluated 2026-08-31 (`@lucasfcosta/backpressured` 0.1.10, repo-metadata + README read, not installed): a Claude Code plugin (slash-commands + skills) running a plan → implement → verify → ship loop, every step gated on real checks (lint, tests, benchmarks, manual testing, independent review, PR monitoring), project-customized via a plain-English `BACKPRESSURE.md`; stops on all-gates-pass or a named blocker. 63 stars, active through May 2026. Mechanically it IS the M5 verifier-kit territory packaged — a live third-party instance of what `diagnose-and-resend` has students build. Tie-in adoption is a maintainer call; the evaluation is on record.

**Tier 1 — ≥2 independent lenses, or already enacted in-course (adopt into bank; wire during edit pass):**

1. **verification-asymmetry / verifier's rule** (practitioner + syllabus, both high; ALREADY IN M2 BODY as "Find is easier than judge", push-back-on-the-plan.md:39). The economic engine of the whole far half — why delegation pays where verification is cheap. Sharpens convergence-triad's "independently verifiable" leg into a cost ratio. Src: Jason Wei, jasonwei.net (July 2025) [practitioner direct] — verified: full stamp in `supplementary/verification-asymmetry.md` (durable, due:none; Wei's own heading reads "Verifier's rule", not "law").
2. **lethal trifecta** (practitioner + syllabus, both high/missing). The agentic adversary model — private data + untrusted content + outbound channel; M3 does classic STRIDE but nothing on the agent AS attack surface / instructions-vs-data inseparability. Src: simonwillison.net/2025/Jun/16 [practitioner direct] — verified: full stamp in `supplementary/the-lethal-trifecta.md` (durable, due:none). Home landed: that supplementary, wired from the M4 send-off territory.
3. **Goodhart / eval-gate-decay** (3 lenses: control, econ, syllabus). Gates are perishable; the optimizer games the measure; hold-out + integrity-check countermoves. Bank has principal-agent naming reward-hacking but not the decay mechanism. Home landed at M5: `the-gate-is-a-claim.md` § Gates decay carries Goodhart plus the hold-out/integrity countermoves; `when-the-score-stops-moving.md` (an Agents 101 file) carries only the flatline/blind-spot half.
...
5. **calibrate-the-judge + error-analysis-first** (syllabus high/missing + practitioner medium; one discipline). A judge is a claim requiring verification against human labels; evals come from reading real traces, not armchair taxonomy. Direct upgrade to eval-judge-verifier-gate's missing validity condition. Src: hamel.dev [practitioner direct] — verified: three stamps in `lectures/the-gate-is-a-claim.md` (llm-judge anchors the 90%/three-iterations number · field-guide anchors error-analysis-first · evals is supporting-only).
...
- **good-regulator (3 lenses).** Conant–Ashby is a narrow formal theorem; "the theorem proves CLAUDE.md works" is analogy-theatre. Candid form: intuition pump ("model-based control beats model-free under variety"); actual mechanism = relevance conditioning, already carried by context-bandwidth-and-rot.
...
- **agent-loop-closed-loop-controller** (control): necessary-but-not-sufficient — loop QUALITY (observability, sensor noise, delay) decides convergence; a flaky test suite is a closed loop that still diverges. Guards against "I added tests, therefore controlled" theatre. *Worth adopting: it's the candid version and it generates the M5 move.*
...
- MOVE: Schedule externalization routines — show-your-prompts reviews, pairing on agent sessions, protected skill-writing time. Judge a context file or skill by one bar: can a colleague re-internalize it without you in the room? Treat 'it works for me but I can't say why' as capability that has not crossed and will not.
