...
A role colors answers the same way a fact does. One more session. The first prompt: *"I'm a cardiologist preparing dinner for my patients."* Then the same second prompt about dinner. The suggestion shifts again: heart-healthy, low-sodium, vegetable-forward. Not because Claude knows medicine better than Italian cooking. You told it who you are, and every answer after took that into account.
...
**Quality:** compendium-audited 2026-08-19 (writing@efe8d58d story@1480362 technical@2b5a60a behavior@1480362 pedagogy@1480362 strategy@1480362 slides@efe8d58d)
- judges @efe8d58d: writing PASS, story PASS, technical PASS, behavior PASS, pedagogy PASS, strategy PASS, slides PASS
...
- Third session: cardiologist-preparing-dinner-for-patients → heart-healthy suggestion. Context is whatever you tell it.
...
**Demo watch-fors:**
- **Establish that the sessions are independent before you deliver.** The whole bit depends on session 2 not knowing about session 1. **Standing instruction: three fresh Claude Code sessions, each started in an empty directory outside any repo** — no `CLAUDE.md`, no project memory, no rules file loads. Not one session with `/clear` between beats. Dry-run once before the cohort on the Claude Code version the room will meet. Salvage if session 2 answers Finnish-adjacent anyway: *"It remembered — which is the lecture's point one step early: context is what you tell it, and now you've seen it telling itself"*; then run the third session clean.
...
- demo-fires-live `[checked:2026-08-23 result:NEEDED due:cohort]` (no URL — live dry-run owed) — [capability] The demo runs in Claude Code, and session independence rests on what a fresh session loads: `CLAUDE.md` files on the path, rules files, and project memory are all keyed to the working directory, so a session started in an empty directory outside any repo starts with none of them. That is the standing instruction in Demo watch-fors. Owed before each cohort: three fresh sessions in an empty directory on the Claude Code version the room will use, the dinner question run through, then `result:ATTESTED`. The earlier stamp here tracked claude.ai cross-chat search and incognito; that surface was never the demo's (Antti, 2026-08-23) and its findings do not transfer. fallback: none. If the demo stops firing, the opener needs rebuilding, not rewording.
...
- holds: the mechanism is foundational and not in doubt — prior text in the window conditions the next output, with no weight update (Brown et al. 2020). Note what that does and does not cover. The literature on file backs the *mechanism*. What backs the *specific* effect the demo shows, an unrelated earlier exchange colouring a later question, is the demo. For a claim the room watches happen in 60 seconds, a live check is the stronger warrant anyway, which is why this is taught as a demo and not as a citation. The product side is a standing precondition, not a risk: the demo runs in fresh Claude Code sessions in an empty directory outside any repo, so nothing directory-keyed loads (instruction + salvage in Demo watch-fors).
...
**Flagged**
- `[resolved:2026-08-23]` **Surface corrected.** The 2026-07-30 flag and its 2026-08-01 resolution were about claude.ai cross-chat search and incognito; the demo runs in Claude Code (Antti, 2026-08-23). Isolation instruction rewritten in Demo watch-fors; `demo-fires-live` re-stamped `NEEDED` pending a Claude Code dry-run.
