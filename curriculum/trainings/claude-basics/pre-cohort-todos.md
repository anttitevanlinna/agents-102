...
| `curriculum/exercises/find-the-wrong-claims.md` | story REVISE (Phase 1 push-back criteria in body); behavior REVISE (P1/P3/P4 self-audit-charity carve-out) | Phase 1 push-back criteria moved into prompt fence | re-fire four class-judges |
| `curriculum/exercises/organisers-prepare-claude-basics.md` | technical REVISE (missing CLAUDE.md in SharePoint workshop folder root, rule 7a); behavior REVISE (rehearsal order and readout flattening risk) | System setup now includes the folder tree and root `CLAUDE.md`; rehearsal now creates fake first-name output, runs the group-driver prompt into `shared/`, creates two more group syntheses, then runs the organiser synthesis exercise | re-fire four class-judges |
...
- `homework-build-and-verify.md`, `build-your-system.md`, `find-the-wrong-claims.md`: leap tests + per-phase failure-mode rows (story-judge rules 45 + 47).
