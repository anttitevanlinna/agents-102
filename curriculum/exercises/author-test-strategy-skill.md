...
- **Team-kit substrate your sponsor named** — if the pre-engagement contract pinned a shared Git repo, monorepo `.claude/` path, or other substrate, use that. Trainer has it on the board in the cohort room.
- **Self-study or sponsor-unresolved** — ship to `.claude/skills/test-strategy/SKILL.md` in your own repo for now. The skill lives with the code it's tuned to until the team kit substrate arrives. Takes one move to relocate later.
...
```
Confirm the skill file's current content at its team-kit path, then commit it with a one-line message: "Author test-strategy skill tuned to [feature area] — M3 AE101 first authored skill."

If the team-kit substrate is a separate git repo, push. If it's a path in my main repo's .claude/ directory, commit in this repo.
...
- **P4 ship-confusion.** Team-kit substrate ambiguous. Nerd checks: *"is the substrate a separate repo or a path in this repo's .claude/? the pre-engagement contract should name it; if unresolved, use the training-start default the trainer spun up."*
...
- Team-kit substrate was not resolved in the pre-engagement contract. Training-start default should be live; if not, Ops has a problem, flag at Debrief. Don't block the exercise on it — land the file in `.claude/skills/test-strategy/SKILL.md` in the student's repo and move the substrate question to Debrief.

**Plug points:**
- Student's feature (Ex1 + Ex2 carried forward) — Phase 3 invocation target
- Sponsor-stated team-kit substrate — Phase 1 authoring path + Phase 4 ship destination
...
- Team-kit substrate Ops runbook — who creates the default repo if sponsor answered "we don't have one," when, how the students get access. Cross-cutting TODO, not M3-specific.
