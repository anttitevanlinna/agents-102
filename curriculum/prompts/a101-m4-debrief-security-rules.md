...
Review this session and compound 1-5 security operating rules into the agent system. Read the root ./{{artifact:root-instructions}} if it exists. Read outputs/policy-report-raw.md if it exists, outputs/policy-report.md, and outputs/security-report.md. Look back over the session: which rows should future sessions remember, which "I can't tell" rows need standing evidence checks, where did the mitigation reduce one risk but shift another, and what residuals should future sessions not forget?
