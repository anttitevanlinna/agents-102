...
**Slides-only pass (2026-07-02, unaudited):** lede + six slides + two kickers (post-slide-1 line; post-prompt keeper-criterion line). Per-passage verdicts: "Pause for a second" recap lede COMPRESSED to a one-line artifact-state setup lede (pause-command shape also grazed the contemplative-pause ban) · three-pattern prose CONVERTED to slide 1 with the piece↔failure-mode mapping paragraph FOLDED into the per-piece bullets (separate mapping paragraph would be restatement) · verifier-shapes prose CONVERTED to slide 2, Cherny/Kim attribution hedge preserved verbatim ("the menu form is the synthesis") · "Hooks always fire" CONVERTED to slide 3; `{{prompt:what-packaging-is-1}}` kept in place inside the slide section, "read what comes back" trimmed to the keeper-criterion kicker · "What you didn't build today" + "The counter-camp" MERGED into one slide (both are edges of the pattern; the camps ARE the context-for-long-runs murk) · Intercom section CONVERTED; "CTO" title claim fixed in body to "(Intercom engineering)" per the 2026-05-25 stamp caveat (was flagged soften-at-delivery; now softened at source) · "Bridge to M6" section DELETED — sequencing content moved to the module file's `## Next` (verifier-gets-its-full-name already there; the did-my-run-pass/do-all-our-runs-pass scale line added there this pass); "kit grows by accretion" survived as a non-sequencing teaching bullet in the Intercom slide, bold claim recast to "Review infrastructure grows by accretion" (its "the kit" antecedent lived in the deleted bridge; cold reader needs no upstream file); "measure the reliability... the number is the gap" line died (covered by the module's Klaassen pre-read blurb).
...
- `[checked:2026-08-01 result:OK due:none]` https://lucumr.pocoo.org/2026/1/14/minijinja-go-port/ — [practitioner direct] Ronacher MiniJinja Rust→Go port, 14 Jan 2026: snapshot tests as verifier, 10 h / 2.2M tokens, three pieces cleanly inferable. **Durable account, `due:none`** (`source-freshness-format.md` § Durable-account variant, added 2026-08-01): a completed run reported first-hand does not expire, so the previous "exits the window 2026-07-14, swap at a Sep-2026 cohort" instruction is withdrawn — swapping a perfect example purely on its date traded accuracy for freshness. Figures re-verified verbatim on page 2026-08-01: *"Agent run duration: 10 hours (3 hours supervised)"*, *"Total tokens: 2.2 million"*. **The condition that still binds is the date in body** — cite it as *"in January 2026"* so the account cannot read as current practice. "Three pieces cleanly inferable" stays: that is our reading of his description, not his naming. **And the naming scope is tighter than "not his naming" conveys — he does not use the words at all.** Full-text check 2026-08-01: *reference* occurs once, as an adjective for test files (*"Parse the reference insta `.snap` snapshots and compare output"*); *verify* occurs once as an ordinary verb (*"using insta to verify they match"*); *verifier* occurs zero times. Plan files appear only as the approach he **rejected** — *"Without switching branches, I would probably just make new sessions and have more plan files lying around"* — so the post is 2-of-3 as practice and 0-of-3 as vocabulary. A 14-post sweep of his archive (Nov 2025–Jul 2026, the full window) returns zero uses of *verifier* anywhere, including the three posts that discuss agent harnesses at length. His nearest statement of the idea names nothing: *"In an ideal world the agent has one command, that lints and compiles and it tells the agent if all worked out fine"* (*A Language For Agents*, 2026-02-09).
- `[checked:2026-08-01 result:OK due:none]` https://ghuntley.com/ralph/ — [practitioner direct] **Huntley's Ralph, as he documented it: all three pieces present together.** **§2b split, applied 2026-08-01 — the account is durable, the count is not.** `due:none` covers only what he wrote and how; re-reading it next year will not change it. *"The ONE documented practice running all three"* is a different claim, about the current state of the field rather than about his post, and it rests on an eleven-practitioner sweep dated 2026-08-01. **That half carries `due:2027-02-01` and must be re-counted before a cohort.** It stops being true the day anyone else publishes a triad, and nothing in the `due:none` above would notice. Plan: *"create/update a @fix_plan.md which is a bullet point list sorted in priority… ALWAYS KEEP @fix_plan.md up to do date with your learnings using a subagent"* (note the filename is `@fix_plan.md`; our *plan.md* is a generic label for the shape, not his). Reference: *"study specs/\* to learn about the compiler specifications"*, diffed explicitly. Verifier: type-system backpressure plus *"run the tests for that unit of code that was improved"*. Still active per *everything is a ralph loop* (2026-01-17), which cites the original rather than restating the pieces. fallback: none; if the triad needs one named home, it is this one.
- `[checked:2026-08-01 result:CAVEAT due:asap]` https://every.to/source-code/stop-coding-and-start-planning — [practitioner direct, vendor venue] Klaassen (2025-11-06) carries a plan artefact but a **structurally different one**: a pre-work spec created once and reviewed, then implemented and stored in GitHub — not a living document the agent mutates across a run to fight context rot, which is what our *plan.md* piece does. No reference-diff and no verifier language anywhere in his corpus (full Every.to bibliography checked, 20 articles, nothing newer on this topic). Outside the window. fallback: cite him for planning-first posture only; do not count him toward the triad.
- `[checked:2026-08-01 result:CORRECT due:asap]` https://ampcode.com/news/neo — [practitioner direct, vendor venue] **The handoff camp reversed, and the body still teaches it as live.** Amp rebuilt on 2026-05-06 and killed the feature: *"So handoff is out. Compaction is in."* Also *"Handoff is gone. As described above, compaction made it obsolete"*, and the reversal is total on the very habit this module teaches: *"No More Manual Context Management… You don't have to watch context percentages anymore, or decide when to handoff."* Compaction now runs automatically at 90% fill. GA'd 2026-05-27 (https://ampcode.com/news/drop-the-neo). By 2026-07-02 they report a thread compacted 68 times that would otherwise run 21M tokens (https://ampcode.com/news/read-bigger-threads). **An earlier stamp on this same file, written 2026-08-01, claimed the stance was "sustained into the window." That was wrong — it read a July post as continuity without checking the May rebuild in between.** fallback: the two-camp framing needs re-cutting, not re-stamping; card owed on the body.
- `[checked:2026-08-01 result:CAVEAT due:none]` https://ampcode.com/notes/200k-tokens-is-plenty — [practitioner direct, vendor venue] Lewis Metcalf (2025-12-09), the original short-threads doctrine: *"Agents get drunk if you feed them too many tokens."* **Citable only as dated history, and the page now disowns itself** with an appended editorial note: *"This note was written in December 2025 for an older model and context-window era. Now, auto-compaction makes longer threads work well, and it's fine and productive to go beyond 200k tokens."* Do not cite the "agents get drunk" line as a current position. fallback: cite it as what the camp believed in late 2025, and say what happened next.
- `[checked:2026-08-01 result:GONE due:asap]` https://ampcode.com/guides/context-management — [vendor docs] Marked Archived, yet the body still describes Handoff as the live mechanism and never mentions compaction, while every Chronicle post footer still links it. **Amp's own docs contradict Amp's own product.** Do not cite for current behaviour. fallback: cite the dated news posts instead.
...
- `[checked:2026-05-25 result:OK due:2026-10-16]` https://ideas.fin.ai/p/2x-nine-months-later — [practitioner direct, vendor venue] Curran (2026-04-16): 19.2% auto-approved, 14.6 vs 75.8 min, 86% ≤20 lines, ~473 R&D / 1,305 total. Metrics vendor-self-reported. Body now says "(Intercom engineering)" — the earlier "CTO" title claim is resolved (his title is VP Engineering). fallback: keep the numbers, attribute "Intercom's published telemetry," flag self-report.
- `[checked:2026-05-25 result:OK due:none]` https://ghuntley.com/ralph/ — [practitioner direct] Huntley (2025-07-14): Ralph is greenfield-only ("no way in heck would I use Ralph in an existing code base"), a Bash loop over durable markdown; NO ticket-triage. Dated-historical (body cites as technique-origin). Body says "greenfield projects." fallback: cite as origin of the loop-prompt-over-durable-state technique, drop any application claim.
- `[checked:2026-08-01 result:CORRECT due:asap]` https://ampcode.com/news/handoff — [practitioner direct, vendor] Amp (2025-10-23): rejected auto-compaction, bet on focused threads plus manual handoff — *"Compaction… always had downsides. It's lossy, for one."* The launch post stands as an accurate record of what they believed then. **The stance did not hold: they reversed it on 2026-05-06 and discontinued the feature.** The prior stamp's fallback said "escalate to a fresher Amp post if the stance shifts" — it shifted, and nothing escalated for three months, because a `due:none` on a dated-historical source stopped anyone re-opening the question. That is the real defect here: `due:none` is right for *what they said in October 2025* and wrong for *whether the camp still exists*, and the body was leaning on the second. fallback: keep the October position as dated history and say what happened to it; do not present the camp as current.
...
- `[checked:2026-07-30 result:CAVEAT due:2027-01-30]` https://every.to/source-code/compound-engineering-how-every-codes-with-agents — [practitioner direct, vendor venue] Shipper & Klaassen (Dec 2025), co-bylined and first-person-plural, re-verified live 2026-07-30: "Roughly 80 percent of compound engineering is in the plan and review parts, while 20 percent is in the work and compound." THIS is the 80/20 primary — NOT the Definitive Guide, which lacks it. Dec 2025, outside the 6-mo window — cite as framework origin, not fresh evidence. Klaassen expanded the loop past four steps on 2026-05-29; the 80/20 ratio is not restated there, so treat the ratio as pinned to the Dec 2025 form. Body hedges as "the ratio practitioners take from Klaassen's posture"; defensible, this URL backs it. fallback: keep the posture hedge.
- `[checked:2026-08-01 result:CAVEAT due:cohort]` (no URL — internal OODA `2026-04-23-scaling-session-length-2-platform-mechanics.md`, corrected 2026-08-01) — **not a convergent pattern**: the ~60% figure traces to one unsigned vendor post, the second cited source carries no such figure, the third 403s. Subagent isolation is unaffected and stands. This slide survives the correction where the M5 opener does not, because it presents 60% as **one camp in a five-camp menu the field has not settled** — a divergence claim, which is what the evidence actually supports. fallback: keep the hedged menu; if the number goes anywhere, it goes here rather than in the opener.
...
- **There is no counter-camp bullet, and that is deliberate.** It named Sourcegraph's Amp as the live handoff camp; Amp reversed on 2026-05-06, retired handoff and now compacts automatically. The bullet was also the weaker duplicate of the camps bullet above it (two camps announced directly after five), and its "tools change, the method doesn't" reading belongs to `the-loop-has-a-name` at M6, not here. Do not restore it. The handoff shape survives in the camps bullet as *"the original Ralph, rebuilding fresh sessions continuously from durable state"*, which is the practice without the product name.

**Philosophy callouts:** none. The naming is the beat; further philosophy tagging dilutes.

**Vision vs. detail:**
- Vision layer: the three-pattern as **our synthesis** of scattered practice — assembling a real pattern from what practitioners do is legitimate curriculum work; presenting it as convergence is not, because one practitioner (Huntley) carries all three and none present them as a triad. The context-for-long-runs camps as a divergence menu (the field has not converged, and the evidence says so: eleven practitioners checked 2026-08-01, zero convergence on compaction). Review-infrastructure-grows-by-accretion close. **Ronacher is scoped to the practice, never to the vocabulary** — he backs *diff against a reference implementation* and *gate on an automated snapshot test*; he does not name either piece, and the body must not say he does.
