...
**The §52c sweep of the sixteen AE101 lead-ins is now IN the ledger** (`gate_triage.batch = wyb-52c-2026-09-06`), persisted 2026-09-08 from a session scratchpad that would not have survived. Eight want a rewrite, seven stand, a skeptic overturned three proposed rewrites. Two are suppressed with their reasons recorded on the row — `spot-gaps-build-the-loop` grows the line, `diagnose-and-resend` proposes *"in minutes, not hours"*, a quantified claim with no source. `read-your-stack` STANDS. Still open: `author-test-strategy-skill` and `threat-model-with-stride`. Both survive the tangibility boundary, keeping a concrete object and adding the *for what* — **but the sweep's proposed wording for `author-test-strategy-skill` does not.** It trades `SKILL.md` in a named folder for *"a skill"*, which is the direction §52c bans; the card owes a rewrite that keeps the filename. `fix-tests-first` was declined and `set-the-markers-send-it-off` withdrawn under the tangibility ruling. `push-back-on-the-plan` and `compound-and-close` were applied, and they share the batch's most answerable shape: the build slot restating the do-slot, which is the one defect a reader can confirm without knowing the exercise.
