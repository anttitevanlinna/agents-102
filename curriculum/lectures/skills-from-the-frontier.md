...
- pocock-grill-me `[checked:2026-07-29 result:CAVEAT due:2027-01-29]` https://github.com/mattpocock/skills — [practitioner direct] Pocock's own repo ("Skills For Real Engineers. Straight from my .agents directory"), MIT © 2026 Matt Pocock — both confirmed. CAVEAT on two points: `grill-me/SKILL.md` is a 4-line pointer; the method lives in the sibling `skills/productivity/grilling/SKILL.md` — cite that path. And "Socratic" is our gloss, not the source's: the skill recommends an answer per question and waits for confirmation ("For each question, provide your recommended answer… Do not act on it until I confirm"), where classic Socratic method surfaces contradictions rather than proposing. Defensible as a loose gloss; not a quote. fallback: name the skill and the one-at-a-time decision-tree interview without the word Socratic.
...
- saltzer-citation-count `[checked:2026-07-29 result:OK due:none]` https://www.semanticscholar.org/paper/10.1109/PROC.1975.9939 — [academic/research] 2,962 citations, 178 influential. Backs "still cited half a century on" and nothing stronger. The superlative this replaced ("the most-cited piece of computer security writing") is unsupported — 2,962 is a landmark but an order of magnitude below the field's most-cited, and no source asserts the claim. Shostack's own phrasing is the hedged ceiling if a stronger line is ever wanted: "one of the most cited, least read works in computer security history" (https://shostack.org/blog/the-security-principles-of-saltzer-and-schroeder) `[practitioner direct]`. **Do not restore the superlative.** fallback: drop the clause; the provenance sentence stands alone.
...
**Flagged**
- `[found:2026-07-29]` **Citation path wrong, gloss loose.** `grill-me/SKILL.md` is a 4-line pointer; the method lives in `skills/productivity/grilling/SKILL.md`. And "Socratic" is our word — the skill recommends an answer per question and waits for confirmation, where Socratic method surfaces contradictions. → Decide: keep "Socratic" as an acknowledged loose gloss (defensible, maintainer-side now recorded), or reword. Body says "on his GitHub" with no path, so no URL fix is owed above the divider.
- `[found:2026-07-29]` **Stance holds at L2 — the hunt came back empty, deliberately.** Ten named practitioners' own recent output read; no third org has published a contribution-spread number. Vercel is the near-miss: ~100 skills behind one internal agent, non-developers authoring, but the contributor count is exactly the number they do not publish. → No lecture change. If Vercel or Every ever publishes a percentage, that is the L3 trigger.
- `[found:2026-07-29]` **The falsifier is absent, and that is now recorded.** No published account anywhere of a shared skill kit decaying, losing its maintainer, or contribution flattening. What exists is practitioners naming rot as a *risk* to design against (Scanlan's Intercom post and its comments; Karlsson on skill drift after model updates), never as an observed case. → Worth knowing the counterweight slide has no evidence behind it yet. Do not add one.
