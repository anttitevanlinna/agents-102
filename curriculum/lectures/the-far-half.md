...
**STATUS:** slide-format draft (proper-length bullets per `theory-plan.md` § Slide format, grown from the 2026-07-01 skeleton on 2026-07-02), WIRED into `run-the-first-experiment.md § Start here` (M4 open), after the opening question and before `[Lecture: Test and learn]`. No Quality line by design (unaudited). Three slides (one `##` = one slide); the map figure IS the slide.
