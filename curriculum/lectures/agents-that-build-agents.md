...
**Quality:** compendium-audited 2026-09-03 (writing@289b45a3 story@289b45a3 technical@4e0370bc behavior@4e0370bc pedagogy@4e0370bc strategy@4e0370bc slides@289b45a3)
- judges @289b45a3: writing PASS (1 todo see instances/ae101--lecture--agents-that-build-agents.writing.json), story PASS (1 todo see instances/ae101--lecture--agents-that-build-agents.story.json), technical PASS (2 todos see instances/ae101--lecture--agents-that-build-agents.technical.json), behavior PASS (1 todo see instances/ae101--lecture--agents-that-build-agents.behavior.json), pedagogy PASS (3 todos see instances/ae101--lecture--agents-that-build-agents.pedagogy.json), strategy PASS (2 todos see instances/ae101--lecture--agents-that-build-agents.strategy.json), slides PASS
...
**`## The agent stops where your judgement begins` (Antti 2026-09-02: *"This concept is right. But just wrongly written now."*):** first slide; the body is the module's frame (the agent runs on what you have expressed; expression sets the frontier; judgement becomes expression one piece at a time). The old three paragraphs (not-the-agent-writing-its-own-skills / evidence-stops-at-the-disk / build-the-flywheel-to-run-exactly-that-far) are in git at `109ca335`. Do not restore them.

**`## The right information grounds and makes quality` (title and bridge sentences Antti's, 2026-09-02):** second slide, between the judgement-boundary slide and *You make agentic happen*. The 500K-lines paragraph stays as the example; the slide's job is to bridge expression into action: spec and tests were written down first, the goal is the right information, and it is found one run at a time.
...
Frameworks
- Grounding as the deeper quality move · [borrow:none] · law:none · ← none — curriculum framing, no single-practitioner attribution; the slide title is Antti's
- Principal–agent · [borrow:economics] · law:principal-agent · ← cultural-vocab — the slide header *The agent stops where your judgement begins* names the delegation boundary
- The compound ladder · [borrow:none] · law:the-compound-ladder · ← none — fix → memory → skill → system is the loop this lecture runs one turn further

Stance `[stance:2026-08-01 level:L1]`
- holds: that the agent runs exactly as far as what you have expressed, and that the kit outlives the model. Our framing, carried as a position; the lecture makes no claim about the field.
