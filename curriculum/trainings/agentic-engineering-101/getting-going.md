...
**TODO:**
- **Three-persona simulation on the three new exercises** (Maija mid-competent / Greg opinionated senior / Jin fast operator). Focus: Ex1 introspection feel, Ex2 tests-first pressure + diff push-back, Ex3 compound move + MCP gate realism.
...
- **Three-persona simulation archive.** Archive per-persona sim reports under `curriculum/evals/instances/sims/agentic-engineering-101--<exercise-slug>--<persona>.md` (one set per Ex1 / Ex2 / Ex3) so the next reshape pass can see what prior personas caught.
- **Eval instance refactor.** `curriculum/evals/instances/agentic-engineering-101--getting-going.md` was authored against the single `ship-trivial-bug.md` mega-exercise. Split into three per-exercise instances (`agentic-engineering-101--orient-and-introspect.md`, `--fix-tests-first.md`, `--compound-and-close.md`) at the next eval pass.
