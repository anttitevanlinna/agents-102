...
The enforceable design separates identities: observe, prepare, verify, authorize, execute. Autonomy expands per narrow action class only after shadow results, correct abstentions, a one-target canary, independent postconditions, and rollback evidence. The human gate also serves as the learning interface: accept, reject, edit, and rationale ground the next candidate rule in operator judgment. Central promotion then requires provenance, scope, backtest, review, versioning, and expiry.
