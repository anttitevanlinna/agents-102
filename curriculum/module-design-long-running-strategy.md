...
- **The plan must specify which file-layer each artifact targets.** *"Student compounds to `CLAUDE.md`"* in a plan reads fine but ships a social-dynamics bug: in multi-engineer repos, `./CLAUDE.md` is PR-gated. Every plan-level artifact gets a file-layer declared: team (`./CLAUDE.md`), personal (`./CLAUDE.local.md`), user (`~/.claude/CLAUDE.md`), or team-kit (sponsor-stated). Default compound target for session Debriefs: personal. Landed 2026-04-23 late after Antti caught 15+ unqualified references across M1–M4; sweep cost was six compendium amendments + 10-file prose sweep. Plan-time check: *"could a staff engineer on a 20-person team actually do this compound every session without the PR gate blocking them?"*
- **Platform-assertion verification happens before the assertion, not before drafting.** The existing curriculum/CLAUDE.md rule scoped to "BEFORE drafting" missed the strategic-conversation-mode moment when I proposed `CLAUDE.local.md` as an official primitive to Antti. Rule rewritten 2026-04-23 from activity-scoped ("drafting") to trigger-scoped ("asserting"). Practical shape: every plan that makes a platform claim names the verification source (agent + URL) at plan time. Trust-but-verify the capability-check agent — WebFetch the URL yourself when the assertion is load-bearing.
