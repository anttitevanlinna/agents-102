...
- osmani-outer-loop `[checked:2026-07-31 result:OK due:2027-01-31]` https://addyosmani.com/blog/own-the-outer-loop/ — [practitioner direct] Osmani, 15 Jul 2026 — same author, same week, different vocabulary, which is why it is corroboration and not a second vote. *"The bottleneck moves from 'can we build this?' to 'should this exist, can we answer for it?'"* *"Generation moved faster than control."* Inner loop (agent-owned): *"investigation, implementation, verification, repeat."* **Cite this one for what it costs us, too:** his human-owned outer loop fuses checking-it-is-correct with deciding-to-take-it-up into a single step, and his Intent equivalent (*"decide relevance"*) is applied downstream to work already generated. Both are live challenges to the map's ordering.
...
**Flagged**
- `[found:2026-07-31]` **Ronacher's harness loop takes the human out of Absorption.** *The Coming Loop* (23 Jun 2026) names *"the harness level loop: the loop outside the agent loop"* and says plainly what it costs him: *"Even the 'done' signal loses all meanings and just becomes communicated to yet another machine that judges. My role is reduced to that of a messenger."* This file defines Absorption around a person — *"how much of the result **you** can actually take up."* → Does Absorption stay human by definition, or is it a place a machine can stand in? Whatever this gets, the far-half lectures inherit it, and `the-map-filled-in` at M6 is where the answer would have to be visible. Same piece already sits in `the-machine-you-just-met`'s Flagged for the sibling worry about the check leaving the run — one source, two columns, likely one decision.
- `[found:2026-07-31]` **Osmani draws the review gate at the org tier, not the run.** *Software Factories* (20 Jul 2026): *"The Factory — Multiple harnessed loops running in parallel, feeding a shared review gate."* One gate, many loops. This map draws Absorption as a column every run passes through, and shows the climb separately in the team zone. → One read is that these agree, and Osmani is naming the altitude the map already draws — does that match? It matters because if Absorption only binds at team scale, the M2 column is teaching a personal-scale phase that a student cannot yet feel, which is the same charge the ghost checking loop already carries in the eyeball queue.
