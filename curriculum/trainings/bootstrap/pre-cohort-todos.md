# Pre-cohort punchlist — Bootstrap

Everything that needs to land before the first paying Bootstrap cohort. Grouped by kind of work, not by module.

Delete items when done. Git is the history; this file is current state.

## 1. Three-persona sim sweep

Run the Bootstrap sim set after the current polish pass settles. Use the same shape as AE101: mid-competent business practitioner, opinionated senior leader, fast operator, plus targeted facilitator persona where module timing is the risk.
...
- Bootstrap module eval instances for Modules 3–8 are missing or stale after the recent reshapes. Fill after the module spines settle and before three-persona sim.
